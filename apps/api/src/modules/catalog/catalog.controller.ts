...
export class CatalogController {
  ...
  @Get('uom-classes')
  async uomClasses(): Promise<{ uomClasses: UomClassRecord[] }> {
    return { uomClasses: await this.service.getUomClasses() };
  }

  @Get('uoms')
  async uoms(@Query('lang') lang?: string): Promise<{ uoms: UomRecord[] }> {
    return { uoms: await this.service.getUoms(parseLang(lang)) };
  }
  ...
  async category(
    ...
    @Query('lang') lang?: string,
  ): Promise<{ category: ItemCategoryRecord }> {
  ...
  @Get('items')
  async items(@Query('lang') lang?: string): Promise<{ items: ItemRecord[] }> {
    return { items: await this.service.getItems(parseLang(lang)) };
  }
  ...
  @Get('uom-conversions')
  async uomConversions(
    @Query('itemId') itemId?: string,
  ): Promise<{ uomConversions: UomConversionRecord[] }> {
  ...
  ): Promise<{ uomConversion: UomConversionRecord }> {

...
const itemColumns = {
  ...
  shelfLifeInDays: item.shelfLifeInDays,
...
function toUomRecord(row: UomRow, name: string): UomRecord {
  return {
    ...
    status: row.status as UomStatus,
...
function toCategoryRecord(row: CategoryRow, name: string): ItemCategoryRecord {
  return {
    ...
    status: row.status as ItemCategoryStatus,
...
function toItemRecord(row: ItemRow, name: string): ItemRecord {
  return {
    ...
    hasBatchNo: row.hasBatchNo,
    hasSerialNo: row.hasSerialNo,
    hasExpiryDate: row.hasExpiryDate,
    shelfLifeInDays: row.shelfLifeInDays,
    createdAt: row.createdAt.toISOString(),
    updatedAt: row.updatedAt.toISOString(),
  };
}
...
export class CatalogRepository {
  ...
  async listUomClasses(): Promise<UomClassRecord[]> {
    return this.database.db
      .select({ code: uomClass.code, name: uomClass.name, description: uomClass.description })
      .from(uomClass)
      .orderBy(asc(uomClass.code));
  }
  async findUomClass(code: string): Promise<UomClassRecord | null> {
    const rows = await this.database.db
      .select({ code: uomClass.code, name: uomClass.name, description: uomClass.description })
      .from(uomClass)
      .where(eq(uomClass.code, code))
      .limit(1);
    return rows[0] ?? null;
  }
  ...
  private async uomTranslationMap(language: Language): Promise<Map<string, string>> {
    const rows = await this.database.db
      .select({ uomId: uomTranslation.uomId, name: uomTranslation.name })
      .from(uomTranslation)
      .where(eq(uomTranslation.language, language));
  ...
  async insertUom(input: CreateUomInput & { id: string }): Promise<UomRecord> {
    ...
    if (input.nameAr)
      await this.database.db
        .insert(uomTranslation)
        .values({ uomId: inserted.id, language: 'ar', name: input.nameAr });
    if (input.nameEn)
      await this.database.db
        .insert(uomTranslation)
        .values({ uomId: inserted.id, language: 'en', name: input.nameEn });
  ...
  private async categoryTranslationMap(language: Language): Promise<Map<string, string>> {
    const rows = await this.database.db
      .select({
        categoryId: itemCategoryTranslation.categoryId,
        name: itemCategoryTranslation.name,
      })
      .from(itemCategoryTranslation)
      .where(eq(itemCategoryTranslation.language, language));
  ...
  async findCategoryByCode(code: string): Promise<ItemCategoryRecord | null> {
    const rows = await this.database.db
      .select(categoryColumns)
      .from(itemCategory)
      .where(eq(itemCategory.code, code))
      .limit(1);
  ...
  async insertCategory(
    input: CreateItemCategoryInput & { id: string },
    ...
    if (input.nameAr)
      await this.database.db
        .insert(itemCategoryTranslation)
        .values({ categoryId: inserted.id, language: 'ar', name: input.nameAr });
    if (input.nameEn)
      await this.database.db
        .insert(itemCategoryTranslation)
        .values({ categoryId: inserted.id, language: 'en', name: input.nameEn });
  ...
  private async itemTranslationMap(language: Language): Promise<Map<string, string>> {
    const rows = await this.database.db
      .select({ itemId: itemTranslation.itemId, name: itemTranslation.name })
      .from(itemTranslation)
      .where(eq(itemTranslation.language, language));
  ...
  async insertItem(input: CreateItemInput & { id: string }): Promise<ItemRecord> {
    const rows = await this.database.db
      ...
      .values({
        ...
        hasBatchNo: input.hasBatchNo ?? false,
        hasSerialNo: input.hasSerialNo ?? false,
        hasExpiryDate: input.hasExpiryDate ?? false,
        shelfLifeInDays: input.shelfLifeInDays ?? null,
      })
    ...
    if (input.nameAr)
      await this.database.db
        ...
        .values({ itemId: inserted.id, language: 'ar', name: input.nameAr });
    if (input.nameEn)
      await this.database.db
        .insert(itemTranslation)
        .values({ itemId: inserted.id, language: 'en', name: input.nameEn });
  ...
  async upsertItemTranslation(itemId: string, language: 'ar' | 'en', name: string): Promise<void> {
    await this.database.db
      ...
      .onConflictDoUpdate({
        target: [itemTranslation.itemId, itemTranslation.language],
        set: { name },
      });
  }
  ...
  async listUomConversions(itemId?: string): Promise<UomConversionRecord[]> {
    ...
    const rows = itemId ? await query.where(eq(uomConversion.itemId, itemId)) : await query;
    return rows.map(toUomConvRecord);
  }

  async findUomConversion(
    itemId: string,
    fromUnitId: string,
    toUnitId: string,
  ): Promise<UomConversionRecord | null> {
    const rows = await this.database.db
      .select(uomConvColumns)
      .from(uomConversion)
      .where(
        and(
  ...
  async insertUomConversion(
    input: CreateUomConversionInput & { id: string },
  ): Promise<UomConversionRecord> {
    const rows = await this.database.db

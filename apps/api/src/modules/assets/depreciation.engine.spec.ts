...
describe('Depreciation schedules — four methods (plan item 46)', () => {
  const base = { startValue: 12000, salvage: 2000, periods: 5, frequencyMonths: 12, firstDate: '2026-12-31' };
  const sum = (rows: Array<{ amount: number }>): number => Math.round(rows.reduce((s, r) => s + r.amount, 0) * 100) / 100;
  ...
  it('1. straight line: equal rows, ends at salvage', () => {
    ...
    expect(s.at(-1)).toMatchObject({ bookValueAfter: 2000, accumulated: 10000, date: '2030-12-31' });
  });
  ...
  it('3. written down value: yearly rate, per period for quarterly', () => {
    const s = buildSchedule({ ...base, method: 'written_down_value', annualRatePercent: 20, frequencyMonths: 3, periods: 8 });
  ...
  it('4. manual amounts must add up to the depreciable value', () => {
    expect(() => buildSchedule({ ...base, method: 'manual', manualAmounts: [1000, 1000, 1000, 1000, 1000] })).toThrow(/10000/);
    const s = buildSchedule({ ...base, method: 'manual', manualAmounts: [4000, 3000, 1500, 1000, 500] });
  ...
  it('5. month-end cadence and continued numbering (recalculation)', () => {
    ...
    const s = buildSchedule({ ...base, method: 'straight_line', startValue: 8000, periods: 3, openingAccumulated: 4000, firstRowNumber: 3 });
    expect(s.map((r) => [r.rowNumber, r.amount, r.accumulated])).toEqual([[3, 2000, 6000], [4, 2000, 8000], [5, 2000, 10000]]);
  });
});

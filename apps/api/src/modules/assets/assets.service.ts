...
} from '../accounting/accounting.errors';
...
export class AssetsService {
  ...
  async createCategory(input: {
    ...
    const accounts = [
      input.fixedAssetAccountId,
      input.accumulatedDepreciationAccountId,
      input.depreciationExpenseAccountId,
      input.cwipAccountId,
    ].filter((a): a is string => Boolean(a));
    if (new Set(accounts).size !== accounts.length)
      throw new AssetsValidationError('the category accounts must be different accounts');
    ...
    for (const id of accounts) {
      ...
      if (a.orgNodeId !== input.orgNodeId)
        throw new AssetsValidationError(`الحساب ${a.code} تبع شركة تانية`);
    ...
    return (
      await this.database.db
        ...
        .values({
          ...
          cwipAccountId: input.cwipAccountId ?? null,
          defaultMethod: input.defaultMethod ?? 'straight_line',
          defaultPeriods: input.defaultPeriods ?? 60,
  ...
  async create(input: {
    ...
    if (input.isComposite && !input.isCwip)
      throw new AssetsValidationError('الأصل المركّب بيتبني كأصل تحت التنفيذ (isCwip)');
    if (input.isCwip && !cat.cwipAccountId)
      throw new AssetsValidationError(`الفئة ${cat.code} ملهاش حساب أصول تحت التنفيذ`);
    const gross = Number(input.grossValue ?? 0);
    if (!input.isCwip && !(gross > 0))
      throw new AssetsValidationError(
        'grossValue must be positive for an asset that is not under construction',
      );
    const frequency = input.frequencyMonths ?? cat.defaultFrequencyMonths;
    if (![1, 3, 6, 12].includes(frequency))
      throw new AssetsValidationError('frequencyMonths must be 1, 3, 6 or 12');
    return (
      await this.database.db
        .insert(asset)
        .values({
          ...
          name: input.name.trim(),
          orgNodeId: cat.orgNodeId,
          categoryId: cat.id,
          status: input.isCwip ? 'cwip' : 'draft',
          isCwip: input.isCwip ? 'yes' : 'no',
          grossValue: n2(input.isCwip ? 0 : gross),
          salvageValue: n2(Number(input.salvageValue ?? 0)),
          openingAccumulated: n2(Number(input.openingAccumulated ?? 0)),
          accumulatedDepreciation: n2(Number(input.openingAccumulated ?? 0)),
          method: input.method ?? (cat.defaultMethod as DepreciationMethod),
          periods: input.periods ?? cat.defaultPeriods,
          frequencyMonths: frequency,
          annualRatePercent: input.annualRatePercent ?? null,
          manualAmounts: input.manualAmounts ?? null,
          costCenterId: input.costCenterId ?? null,
          isComposite: input.isComposite ? 'yes' : 'no',
  ...
  async addCwipCost(
    id: string,
    input: { amount: string; contraAccountId: string; date?: string; note?: string },
  ): Promise<AssetRow> {
    ...
    const je = await this.post(
      a,
      input.date ?? today(),
      `تكلفة أصل تحت التنفيذ ${a.assetCode}${input.note ? ` — ${input.note}` : ''}`,
      'asset_cwip',
      [
        [cat.cwipAccountId!, amount, 0],
        [input.contraAccountId, 0, amount],
      ],
    );
    await this.database.db.insert(assetValueEvent).values({
      assetId: a.id,
      eventType: 'cwip_cost',
      eventDate: input.date ?? today(),
      amount: n2(amount),
      journalEntryId: je,
  ...
  async capitalise(
    id: string,
    input: { availableForUseDate: string },
  ): Promise<AssetRow & { schedule: Array<typeof depreciationSchedule.$inferSelect> }> {
    const a = await this.mustAsset(id);
    if (a.status !== 'cwip') throw new AssetsValidationError(`الأصل ${a.assetCode} مش تحت التنفيذ`);
    const amount = Number(a.cwipAmount);
    if (!(amount > 0))
      throw new AssetsValidationError(`مفيش تكاليف متسجلة على الأصل ${a.assetCode} تحت التنفيذ`);
    const cat = await this.category(a.categoryId);
    const je = await this.post(
      a,
      input.availableForUseDate,
      `رسملة الأصل ${a.assetCode}`,
      'asset_capitalisation',
      [
        [cat.fixedAssetAccountId, amount, 0],
        [cat.cwipAccountId!, 0, amount],
      ],
    );
    await this.database.db.insert(assetValueEvent).values({
      assetId: a.id,
      eventType: 'capitalisation',
      eventDate: input.availableForUseDate,
      amount: n2(amount),
      journalEntryId: je,
    });
  ...
  async adjustValue(
    id: string,
    input: { newBookValue: string; differenceAccountId: string; date?: string; note?: string },
  ): Promise<AssetRow & { schedule: Array<typeof depreciationSchedule.$inferSelect> }> {
    const a = await this.mustAsset(id);
    if (a.status !== 'in_use')
      throw new AssetsValidationError(`الأصل ${a.assetCode} مش في الاستخدام`);
    const book = Number(a.grossValue) - Number(a.accumulatedDepreciation);
    const target = Number(input.newBookValue);
    if (!(target >= Number(a.salvageValue)))
      throw new AssetsValidationError(`القيمة الجديدة لازم تكون ≥ قيمة الخردة ${a.salvageValue}`);
    ...
    const je = await this.post(
      a,
      date,
      `تعديل قيمة الأصل ${a.assetCode}${input.note ? ` — ${input.note}` : ''}`,
      'asset_value_adjustment',
      diff > 0
        ? [
            [cat.fixedAssetAccountId, diff, 0],
            [input.differenceAccountId, 0, diff],
          ]
        : [
            [input.differenceAccountId, -diff, 0],
            [cat.fixedAssetAccountId, 0, -diff],
          ],
    );
    await this.database.db.insert(assetValueEvent).values({
      assetId: a.id,
      eventType: 'value_adjustment',
      eventDate: date,
      amount: n2(diff),
      journalEntryId: je,
      note: input.note ?? null,
  ...
  async depreciateDue(
    asOf = today(),
  ): Promise<Array<{ assetCode: string; rowNumber: number; amount: string }>> {
    const db = this.database.db;
    const due = await db
      .select()
      .from(depreciationSchedule)
      .where(
        and(isNull(depreciationSchedule.postedAt), lte(depreciationSchedule.scheduleDate, asOf)),
      )
      .orderBy(asc(depreciationSchedule.scheduleDate), asc(depreciationSchedule.rowNumber));
    const done = [];
    for (const row of due) {
      ...
      const je =
        amount > 0
          ? await this.post(
              a,
              row.scheduleDate,
              `قسط إهلاك ${row.rowNumber} للأصل ${a.assetCode}`,
              'asset_depreciation',
              [
                [cat.depreciationExpenseAccountId, amount, 0],
                [cat.accumulatedDepreciationAccountId, 0, amount],
              ],
              `asset-dep-${row.id}`,
            )
          : null;
      await db
        ...
        .set({ journalEntryId: je, postedAt: new Date() })
        .where(eq(depreciationSchedule.id, row.id));
      const accumulated = Number(a.accumulatedDepreciation) + amount;
      const left = await db
        .select({ id: depreciationSchedule.id })
        .from(depreciationSchedule)
        .where(and(eq(depreciationSchedule.assetId, a.id), isNull(depreciationSchedule.postedAt)));
      await this.set(a.id, {
        accumulatedDepreciation: n2(accumulated),
        ...(left.length === 0 ? { status: 'fully_depreciated' } : {}),
      });
  ...
  async list(orgNodeId?: string): Promise<AssetRow[]> {
    ...
    return orgNodeId
      ? q.where(eq(asset.orgNodeId, orgNodeId)).orderBy(asc(asset.assetCode))
      : q.orderBy(asc(asset.assetCode));
  }

  async get(id: string): Promise<
    AssetRow & {
      schedule: Array<typeof depreciationSchedule.$inferSelect>;
      bookValue: string;
      events: Array<typeof assetValueEvent.$inferSelect>;
    }
  > {
    const a = await this.mustAsset(id);
    const schedule = await this.database.db
      .select()
      .from(depreciationSchedule)
      .where(eq(depreciationSchedule.assetId, id))
      .orderBy(asc(depreciationSchedule.rowNumber));
    const events = await this.database.db
      .select()
      .from(assetValueEvent)
      .where(eq(assetValueEvent.assetId, id))
      .orderBy(asc(assetValueEvent.createdAt));
    return {
      ...a,
      schedule,
      events,
      bookValue: n2(Number(a.grossValue) - Number(a.accumulatedDepreciation)),
    };
  }

  // --- helpers ---
  private async putInUse(
    id: string,
    availableForUseDate: string,
  ): Promise<AssetRow & { schedule: Array<typeof depreciationSchedule.$inferSelect> }> {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(availableForUseDate))
      throw new AssetsValidationError('availableForUseDate must look like 2026-01-31');
  ...
  private async rebuildSchedule(id: string): Promise<void> {
    ...
    await db
      ...
      .where(and(eq(depreciationSchedule.assetId, id), isNull(depreciationSchedule.postedAt)));
    ...
    const first = new Date(
      Date.UTC(y!, m! - 1 + (lastPosted ? a.frequencyMonths : a.frequencyMonths - 1) + 1, 0),
    )
      .toISOString()
      .slice(0, 10);
    let schedule;
    try {
      schedule = buildSchedule({
        ...
        startValue,
        salvage: Number(a.salvageValue),
        periods: remaining,
        frequencyMonths: a.frequencyMonths,
        firstDate: first,
        openingAccumulated: accumulated,
        annualRatePercent: a.annualRatePercent === null ? undefined : Number(a.annualRatePercent),
        manualAmounts:
          a.method === 'manual' ? (a.manualAmounts ?? []).slice(posted.length) : undefined,
        firstRowNumber: posted.length + 1,
      });
    } catch (err) {
  ...
  private async post(
    ...
    try {
      const e = await this.accounting.createEntry({
        ...
        entryDate: `${date}T12:00:00Z`,
        isAutoGenerated: true,
        sourceEventType: source,
        idempotencyKey: idempotencyKey ?? `${source}-${a.id}-${Date.now()}`,
        lines: lines.map(([accountId, dr, cr]) => ({
          accountId,
          debitAmount: n2(dr),
          creditAmount: n2(cr),
      ...
      if (err instanceof AccountingValidationError || err instanceof AccountingNotFoundError)
        throw new AssetsValidationError(err.message);
      throw err;
    }
  }
  ...
  private async category(id: string): Promise<CategoryRow> {
    const c = (
      await this.database.db.select().from(assetCategory).where(eq(assetCategory.id, id)).limit(1)
    )[0];

...
export class AssetLifecycleService {
  ...
  async move(
    assetId: string,
    input: {
      purpose: 'transfer' | 'issue' | 'receipt';
      toLocation?: string;
    ...
    if (a.status === 'merged' || a.status === 'scrapped')
      throw new AssetsValidationError(`الأصل ${a.assetCode} "${a.status}"`);
    let toCustodian = a.custodianEmployeeId;
    if (input.purpose === 'issue') {
      if (!input.toCustodianId)
        throw new AssetsValidationError('التسليم لموظف محتاج toCustodianId');
      await this.checkEmployee(input.toCustodianId);
      toCustodian = input.toCustodianId;
    } else if (input.purpose === 'receipt') {
      if (!a.custodianEmployeeId)
        throw new AssetsValidationError(`الأصل ${a.assetCode} مش في عهدة حد`);
      toCustodian = null;
    ...
    if (toLocation === a.location && toCustodian === a.custodianEmployeeId)
      throw new AssetsValidationError('مفيش تغيير في المكان ولا العهدة');
    await this.database.db.insert(assetMovement).values({
      assetId,
      purpose: input.purpose,
      fromLocation: a.location,
      toLocation,
      fromCustodianId: a.custodianEmployeeId,
    ...
    return (
      await this.database.db
        .update(asset)
        .set({ location: toLocation, custodianEmployeeId: toCustodian, updatedAt: new Date() })
  ...
  async movements(assetId: string): Promise<Array<typeof assetMovement.$inferSelect>> {
    ...
    return this.database.db
      ...
      .orderBy(asc(assetMovement.movementDate));
  }
  ...
  ): Promise<typeof assetInsurance.$inferSelect> {
    await this.mustAsset(assetId);
    if (!(Number(input.insuredValue) > 0))
      throw new AssetsValidationError('insuredValue must be positive');
    if (input.endDate <= input.startDate)
      throw new AssetsValidationError('endDate must be after startDate');
    return (
      await this.database.db
        .insert(assetInsurance)
        .values({
          assetId,
          insurer: input.insurer.trim(),
          policyNumber: input.policyNumber.trim(),
          insuredValue: n2(Number(input.insuredValue)),
          premium: input.premium ? n2(Number(input.premium)) : null,
  ...
  async insuranceAlerts(
    ...
    expiring: Array<typeof assetInsurance.$inferSelect & { assetCode: string }>;
    uninsured: string[];
  }> {
    const db = this.database.db;
    const until = new Date(new Date(`${on}T00:00:00Z`).getTime() + withinDays * 86_400_000)
      .toISOString()
      .slice(0, 10);
    const expiring = await db
      .select({ p: assetInsurance, code: asset.assetCode })
      .from(assetInsurance)
      .innerJoin(asset, eq(asset.id, assetInsurance.assetId))
      .where(and(gte(assetInsurance.endDate, on), lte(assetInsurance.endDate, until)))
      .orderBy(asc(assetInsurance.endDate));
    const inUse = await db.select().from(asset).where(eq(asset.status, 'in_use'));
    const uninsured: string[] = [];
    for (const a of inUse) {
      const running = await db
        .select({ id: assetInsurance.id })
        .from(assetInsurance)
        .where(
          and(
            eq(assetInsurance.assetId, a.id),
            lte(assetInsurance.startDate, on),
            gte(assetInsurance.endDate, on),
          ),
        )
        .limit(1);
  ...
  async addComponents(
    ...
    input: {
      assetIds?: string[];
      stockItems?: Array<{ itemId: string; warehouseId: string; quantity: string }>;
    },
  ): Promise<AssetRow> {
    const c = await this.mustAsset(compositeId);
    if (c.isComposite !== 'yes' || c.status !== 'cwip')
      throw new AssetsValidationError(`الأصل ${c.assetCode} مش أصل مركّب تحت التنفيذ`);
    ...
    for (const id of new Set(input.assetIds ?? [])) {
      ...
      if (part.orgNodeId !== c.orgNodeId)
        throw new AssetsValidationError(`الأصل ${part.assetCode} تبع شركة تانية`);
      if (!['draft', 'in_use', 'fully_depreciated'].includes(part.status))
        throw new AssetsValidationError(`الأصل ${part.assetCode} "${part.status}" — مينفعش يتضم`);
      const pc = await this.category(part.categoryId);
      const gross = Number(part.grossValue);
      const acc = Number(part.accumulatedDepreciation);
      const book = Math.round((gross - acc) * 100) / 100;
      const lines: Array<[string, number, number]> = [
        [cat.cwipAccountId!, book, 0],
        [pc.fixedAssetAccountId, 0, gross],
      ];
      if (acc > 0) lines.push([pc.accumulatedDepreciationAccountId, acc, 0]);
      const je = await this.post(
        c,
        `ضم الأصل ${part.assetCode} للأصل المركّب ${c.assetCode}`,
        lines.filter(([, dr, cr]) => dr > 0 || cr > 0),
      );
      await this.database.db
        .delete(depreciationSchedule)
        .where(
          and(eq(depreciationSchedule.assetId, part.id), isNull(depreciationSchedule.postedAt)),
        );
      await this.database.db
        .update(asset)
        .set({ status: 'merged', parentAssetId: c.id, updatedAt: new Date() })
        .where(eq(asset.id, part.id));
      await this.database.db.insert(assetValueEvent).values({
        assetId: c.id,
        eventType: 'component_asset',
        eventDate: today(),
        amount: n2(book),
        journalEntryId: je,
        note: part.assetCode,
    ...
    for (const s of input.stockItems ?? []) {
      ...
      const mv = await this.inventory.createMovement({
        itemId: s.itemId,
        ...
        sourceModule: 'asset',
        sourceId: c.id,
        note: `مكوّن للأصل ${c.assetCode}`,
      });
      const value = Number(mv.totalValue ?? 0);
      let je: string | null = null;
      const accounts = this.postingEngine
        ? await this.postingEngine.resolveStockMovementAccounts(
            c.orgNodeId,
            s.warehouseId,
            'issue',
            'asset',
        ...
        : null;
      if (value > 0 && accounts)
        je = await this.post(c, `تكلفة مخزون مكوّن للأصل ${c.assetCode}`, [
          [cat.cwipAccountId!, value, 0],
          [accounts.contraAccountId, 0, value],
        ]);
      await this.database.db.insert(assetValueEvent).values({
        assetId: c.id,
        eventType: 'component_stock',
        eventDate: today(),
        amount: n2(value),
        journalEntryId: je,
        note: `${s.quantity} × ${s.itemId}`,
      });
      added += value;
    }
    ...
    return (
      await this.database.db
        .update(asset)
        .set({ cwipAmount: n2(Number(c.cwipAmount) + added), updatedAt: new Date() })
        .where(eq(asset.id, c.id))
        .returning()
    )[0]!;
  }
  ...
  async components(
    compositeId: string,
  ): Promise<{ assets: string[]; events: Array<typeof assetValueEvent.$inferSelect> }> {
    const parts = await this.database.db
      .select({ code: asset.assetCode })
      .from(asset)
      .where(eq(asset.parentAssetId, compositeId));
    const events = await this.database.db
      .select()
      .from(assetValueEvent)
      .where(eq(assetValueEvent.assetId, compositeId));
    return {
      assets: parts.map((p) => p.code),
      events: events.filter((e) => e.eventType.startsWith('component_')),
    };
  }
  ...
  private async post(
    ...
    try {
      const e = await this.accounting.createEntry({
        ...
        sourceEventType: 'asset_composition',

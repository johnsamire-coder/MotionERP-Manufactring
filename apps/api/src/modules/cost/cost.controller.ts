...
import type { AllocationBase, CostEntryType, PoolType } from './cost.types';
...
export class CostController {
  ...
  async createComponentType(
    @Body() dto: CreateComponentTypeDto,
  ): Promise<{ componentType: CostComponentTypeRecord }> {
  ...
  ): Promise<{ costSheet: JobCostSheetRecord }> {
    return {
      costSheet: await this.svc.getOrCreateCostSheet(dto.jobOrderReference, dto.currencyCode),
    };
  }
  @Post('entries')
  @HttpCode(201)
  async addCostEntry(@Body() dto: CreateCostEntryDto): Promise<{ costEntry: CostEntryRecord }> {
    return {
      costEntry: await this.svc.addCostEntry({
        costSheetId: dto.costSheetId,
        componentTypeId: dto.componentTypeId,
        entryType: dto.entryType as CostEntryType,
        amount: dto.amount,
  ...
  async addMaterialCost(
    ...
    return {
      costEntry: await this.svc.addMaterialCost(
        ref,
        dto.amount,
        dto.description,
        dto.sourceReference,
  ...
  async addLaborCost(
    ...
    return {
      costEntry: await this.svc.addLaborCost(ref, dto.amount, dto.description, dto.sourceReference),
    };
  }
  ...
  async createPool(@Body() dto: CreateOverheadPoolDto): Promise<{ pool: OverheadPoolRecord }> {
    return {
      pool: await this.svc.createPool({
        code: dto.code,
        name: dto.name,
  ...
  @Post('pools/:id/entries')
  @HttpCode(201)
  async addPoolEntry(
    @Param('id', ParseUUIDPipe) id: string,
    @Body() dto: AddPoolEntryDto,
  ): Promise<{ entry: OverheadPoolEntryRecord }> {
    return {
      entry: await this.svc.addPoolEntry(id, {
  ...
  @Post('pools/:id/activate')
  @HttpCode(200)
  async activatePool(
    @Param('id', ParseUUIDPipe) id: string,
  ): Promise<{ pool: OverheadPoolRecord }> {
  ...
  ): Promise<{ policy: AllocationPolicyRecord }> {
    return {
      policy: await this.svc.createPolicy({
        code: dto.code,
        name: dto.name,
        poolId: dto.poolId,

...
const sheetColumns = {
  id: jobCostSheet.id, jobOrderReference: jobCostSheet.jobOrderReference, currencyCode: jobCostSheet.currencyCode, status: jobCostSheet.status
};
...
interface SheetRow { id: string; jobOrderReference: string; currencyCode: string; status: string; }
...
function toSheetRecord(row: SheetRow): JobCostSheetRecord { return { id: row.id, jobOrderReference: row.jobOrderReference, currencyCode: row.currencyCode, status: row.status as any }; }
...
export class CostRepository {
  ...
  async insertCostSheet(input: CreateJobCostSheetInput & { id: string }): Promise<JobCostSheetRecord> {
    const rows = await this.database.db.insert(jobCostSheet).values({
      id: input.id, jobOrderReference: input.jobOrderReference, currencyCode: input.currencyCode ?? 'EGP'

 ...
 */
export const overheadPool = costSchema.table('overhead_pool', {
  id: uuid('id').primaryKey().defaultRandom(),
  code: text('code').notNull().unique(),
  name: text('name').notNull(),
  ...
  check('overhead_pool_type_valid', sql`${t.poolType} in ('manufacturing_overhead', 'administrative', 'selling_marketing')`),
  check('overhead_pool_status_valid', sql`${t.status} in ('draft', 'active', 'allocated', 'closed')`),
 ...
 */
export const overheadPoolEntry = costSchema.table('overhead_pool_entry', {
  id: uuid('id').primaryKey().defaultRandom(),
  poolId: uuid('pool_id').notNull().references(() => overheadPool.id, { onDelete: 'cascade' }),
 ...
 */
export const allocationPolicy = costSchema.table('allocation_policy', {
  id: uuid('id').primaryKey().defaultRandom(),
  code: text('code').notNull().unique(),
  name: text('name').notNull(),
  poolId: uuid('pool_id').notNull().references(() => overheadPool.id, { onDelete: 'cascade' }),
  ...
  check('allocation_policy_base_valid', sql`${t.allocationBase} in ('units_produced', 'direct_labor_hours', 'direct_labor_cost', 'machine_hours', 'direct_material_cost', 'sales_revenue')`),
  check('allocation_policy_percentage_valid', sql`${t.percentage} > 0 AND ${t.percentage} <= 100`),
 ...
 */
export const allocationResult = costSchema.table('allocation_result', {
  id: uuid('id').primaryKey().defaultRandom(),
  policyId: uuid('policy_id').notNull().references(() => allocationPolicy.id, { onDelete: 'cascade' }),

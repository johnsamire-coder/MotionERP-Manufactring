...
export class PrintingNotFoundError extends Error {
  constructor(m: string) {
    super(m);
    this.name = 'PrintingNotFoundError';
  }
}
export class PrintingValidationError extends Error {
  constructor(m: string) {
    super(m);
    this.name = 'PrintingValidationError';
  }
}
...
export class PrintingService {
  ...
  async createLetterhead(input: {
    orgNodeId: string;
    name: string;
    headerHtml?: string;
    footerHtml?: string;
    isDefault?: boolean;
  }): Promise<typeof letterhead.$inferSelect> {
    try {
      checkTemplate(input.headerHtml ?? '');
      checkTemplate(input.footerHtml ?? '');
    } catch (e) {
      throw new PrintingValidationError((e as Error).message);
    }
    if (input.isDefault)
      await this.database.db
        .update(letterhead)
        .set({ isDefault: false })
        .where(eq(letterhead.orgNodeId, input.orgNodeId));
    return (
      await this.database.db
        .insert(letterhead)
        .values({
  ...
  }): Promise<typeof printFormat.$inferSelect> {
    try {
      checkTemplate(input.template);
    } catch (e) {
      throw new PrintingValidationError((e as Error).message);
    }
    if (input.css && /<\/?style|<script/i.test(input.css))
      throw new PrintingValidationError('css must be plain CSS');
    if (input.isDefault)
      await this.database.db
        .update(printFormat)
        .set({ isDefault: false })
        .where(eq(printFormat.documentType, input.documentType));
    return (
      await this.database.db
        .insert(printFormat)
        .values({
          ...
          isDefault: input.isDefault ?? false,
          allowDraft: input.allowDraft ?? false,
          allowCancelled: input.allowCancelled ?? false,
        })
        .returning()
    )[0]!;
  }
  ...
  async listFormats(documentType?: string): Promise<Array<typeof printFormat.$inferSelect>> {
    ...
    return documentType
      ? q.where(eq(printFormat.documentType, documentType)).orderBy(asc(printFormat.code))
      : q.orderBy(asc(printFormat.code));
  }
  ...
  async render(input: {
    ...
    const format = input.formatId
      ...
      : (
          await db
            ...
            .where(
              and(
                eq(printFormat.documentType, input.documentType),
                eq(printFormat.isDefault, true),
              ),
            )
            .limit(1)
        )[0];
    if (!format) throw new PrintingNotFoundError(`no print format for "${input.documentType}"`);
    if (format.documentType !== input.documentType)
      throw new PrintingValidationError(`format ${format.code} is for "${format.documentType}"`);
    const doc = input.documentId
      ? await this.load(input.documentType, input.documentId)
      : input.data;
    if (!doc) throw new PrintingValidationError('documentId or data is required');
    if (doc.status === 'draft' && !format.allowDraft)
      throw new PrintingValidationError('مينفعش طباعة مستند مسودة — رحّله الأول');
    if (doc.status === 'cancelled' && !format.allowCancelled)
      throw new PrintingValidationError('مينفعش طباعة مستند ملغي');

    const company =
      doc.orgNodeId && this.settings
        ? await this.settings.getCompanyProfile(doc.orgNodeId).catch(() => null)
        : null;
        ...
        : doc.orgNodeId
          ? (
              await db
                .select()
                .from(letterhead)
                .where(and(eq(letterhead.orgNodeId, doc.orgNodeId), eq(letterhead.isDefault, true)))
                .limit(1)
            )[0]
          : undefined;
    }
    ...
    await db.insert(printLog).values({
      documentType: input.documentType,
...
<body>${header ? `<header>${header}</header>` : ''}<main>${body}</main>${footer ? `<footer>${footer}</footer>` : ''}</body></html>`;
    ...
    for (const l of lines) {
      const item =
        typeof l['itemId'] === 'string'
          ? await this.catalog.getItem(l['itemId']).catch(() => null)
          : null;
  ...
  private async load(documentType: string, id: string): Promise<Doc> {
    if (!this.finance)
      throw new PrintingValidationError('document sources are not available — pass data instead');
    try {
      if (documentType === 'sales_invoice')
        return this.withItemNames((await this.finance.getSalesInvoice(id)) as unknown as Doc);
      if (documentType === 'purchase_invoice')
        return this.withItemNames((await this.finance.getPurchaseInvoice(id)) as unknown as Doc);
    ...
    throw new PrintingValidationError(
      `no built-in source for "${documentType}" — pass the document as data`,
    );
  }
}

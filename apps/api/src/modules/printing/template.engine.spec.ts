...
describe('Print template engine (plan item 47)', () => {
  ...
  it('1. fields, dotted paths and HTML escaping', () => {
    expect(renderTemplate('{{invoiceNumber}} / {{customer.name}}', data)).toBe(
      'SI-1 / مستشفى &lt;النيل&gt;',
    );
  });
  ...
  it('2. loops with index and parent access', () => {
    expect(
      renderTemplate(
        '{{#each lines}}<tr>{{@index}}-{{item}}x{{qty}} ({{../invoiceNumber}})</tr>{{/each}}',
        data,
      ),
  ...
  it('3. if / else, nested blocks', () => {
    expect(renderTemplate('{{#if notes}}N:{{notes}}{{else}}no notes{{/if}}', data)).toBe(
      'no notes',
    );
    expect(renderTemplate('{{#each lines}}{{#if qty}}[{{item}}]{{/if}}{{/each}}', data)).toBe(
      '[سرير][شاشة]',
    );
  });
  ...
  it('5. filters format dates and money; unknown filters are refused', () => {
    expect(
      renderTemplate('{{d | date}} {{m | money}} {{q | number}}', {
        d: '2026-09-24T00:00:00.000Z',
        m: '1234.5',
        q: '2.000000',
      }),
    ).toBe('2026-09-24 1,234.50 2');

...
export class PrintingController {
  constructor(
    private readonly service: PrintingService,
  ...
  ): Promise<{ letterheads: Awaited<ReturnType<Svc['listLetterheads']>> }> {
    return { letterheads: await this.service.listLetterheads(orgNodeId || undefined) };
  }
  ...
  ): Promise<{ letterhead: Awaited<ReturnType<Svc['createLetterhead']>> }> {
    return { letterhead: await this.service.createLetterhead(dto) };
  }

  @Get('formats')
  async formats(
    @Query('documentType') documentType?: string,
  ): Promise<{ formats: Awaited<ReturnType<Svc['listFormats']>> }> {
    return { formats: await this.service.listFormats(documentType || undefined) };
  }
  ...
  async format(
    @Body() dto: FormatDto,
  ): Promise<{ format: Awaited<ReturnType<Svc['createFormat']>> }> {
    return { format: await this.service.createFormat(dto) };
  }
  ...
  async render(@Body() dto: RenderDto): Promise<string> {
    return this.service.render(dto);
  }

  @Get('printers')
  async printers(): Promise<{ printers: Awaited<ReturnType<Jobs['printers']>> }> {
    return { printers: await this.jobs.printers() };
  }
  ...
  async printer(
    @Body() dto: PrinterDto,
  ): Promise<{ printer: Awaited<ReturnType<Jobs['addPrinter']>> }> {
    return { printer: await this.jobs.addPrinter(dto) };
  }

  /** Bulk printing in the background: returns the job at once (202). */
  @Post('jobs')
  @HttpCode(202)
  async queue(@Body() dto: BulkDto): Promise<{ job: Awaited<ReturnType<Jobs['queue']>> }> {
    return { job: await this.jobs.queue(dto) };
  }

  @Get('jobs')
  async list(): Promise<{ jobs: Awaited<ReturnType<Jobs['list']>> }> {
  ...
  ): Promise<{ job: Awaited<ReturnType<Jobs['get']>> }> {
    return { job: await this.jobs.get(id) };
  }

  @Get('jobs/:id/output')
  @Header('Content-Type', 'text/html; charset=utf-8')
  async output(@Param('id', ParseUUIDPipe) id: string): Promise<string> {
    return this.jobs.output(id);
  }
}

...
export class TechnicalController {
  ...
  ): Promise<{ documents: TechnicalDocumentRecord[] }> {
  ...
  ): Promise<{ document: TechnicalDocumentRecord }> {
  ...
  async createBom(@Body() dto: CreateBomDto): Promise<{ bom: BomRecord }> {
    const created = await this.service.createBom({
      ...
      isPhantomBom: dto.isPhantomBom,
      allowAlternativeItem: dto.allowAlternativeItem,
      qualityInspectionRequired: dto.qualityInspectionRequired,
      consumeComponentsBasedOn: dto.consumeComponentsBasedOn,
      defaultSourceWarehouseId: dto.defaultSourceWarehouseId,
      defaultTargetWarehouseId: dto.defaultTargetWarehouseId,
  ...
  @Get('bom-creators/:id')
  async bomCreatorById(
    @Param('id', ParseUUIDPipe) id: string,
  ): Promise<{ bomCreator: BomCreatorRecord }> {
    return { bomCreator: await this.service.getBomCreator(id) };
  }
  @Post('bom-creators')
  @HttpCode(201)
  async createBomCreator(
    @Body() dto: CreateBomCreatorDto,
  ): Promise<{ bomCreator: BomCreatorRecord }> {
    const created = await this.service.createBomCreator({
      ...
      items: dto.items.map((it) => ({
        ...
        isSubAssembly: it.isSubAssembly,
      })),
    });
    return { bomCreator: created };
  }
  @Post('bom-creators/:id/create-boms')
  @HttpCode(200)
  async createBoms(
    @Param('id', ParseUUIDPipe) id: string,
  ): Promise<{ bomCreator: BomCreatorRecord }> {

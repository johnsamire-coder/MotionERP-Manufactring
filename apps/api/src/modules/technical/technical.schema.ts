...
export const technicalDocument = technicalSchema.table(
  'technical_document',
  {
  ...
  (t) => [
    check(
      'technical_document_type_valid',
      sql`${t.documentType} in ('shop_drawing', 'cutting_list', 'other')`,
    ),
 ...
 */
export const bom = technicalSchema.table(
  'bom',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    defaultSourceWarehouseId: uuid('default_source_warehouse_id').references(() => warehouse.id, {
      onUpdate: 'cascade',
      onDelete: 'restrict',
    }),
    defaultTargetWarehouseId: uuid('default_target_warehouse_id').references(() => warehouse.id, {
      onUpdate: 'cascade',
      onDelete: 'restrict',
    }),
  ...
  (t) => [
    ...
    check(
      'bom_consume_based_on_valid',
      sql`${t.consumeComponentsBasedOn} in ('bom', 'material_transferred_for_manufacture')`,
    ),
    index('bom_item_idx').on(t.productItemId),
    index('bom_org_node_idx').on(t.orgNodeId),
  ],
);
export const bomLine = technicalSchema.table(
  'bom_line',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    bomId: uuid('bom_id')
 ...
 * state).
 */
export const bomCreator = technicalSchema.table(
  'bom_creator',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    orgNodeId: uuid('org_node_id')
      .notNull()
      .references(() => orgNode.id, { onUpdate: 'cascade', onDelete: 'restrict' }),
    quantityToProduce: numeric('quantity_to_produce', { precision: 24, scale: 6 })
      .notNull()
      .default('1'),
    allowAlternativeItem: boolean('allow_alternative_item').notNull().default(false),
...
export const bomCreatorItem = technicalSchema.table(
  'bom_creator_item',
  {
    id: uuid('id').primaryKey().defaultRandom(),

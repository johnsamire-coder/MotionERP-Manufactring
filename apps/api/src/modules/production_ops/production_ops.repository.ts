...
import type { MaterialTransferMode } from './production_ops.types';

const wcColumns = {
  id: workCenter.id,
  code: workCenter.code,
  name: workCenter.name,
  orgNodeId: workCenter.orgNodeId,
  ratePerMinute: workCenter.ratePerMinute,
...
const woColumns = {
  ...
  orgNodeId: workOrder.orgNodeId,
  jobOrderReference: workOrder.jobOrderReference,
  qtyToManufacture: workOrder.qtyToManufacture,
  sourceWarehouseId: workOrder.sourceWarehouseId,
  wipWarehouseId: workOrder.wipWarehouseId,
  finishedGoodsWarehouseId: workOrder.finishedGoodsWarehouseId,
  ...
  useMultiLevelBom: workOrder.useMultiLevelBom,
  considerScrapItems: workOrder.considerScrapItems,
  materialConsumptionPercentage: workOrder.materialConsumptionPercentage,
  materialTransferMode: workOrder.materialTransferMode,
...
const stepColumns = {
  ...
  jobOrderReference: productionStep.jobOrderReference,
  workOrderId: productionStep.workOrderId,
  orgNodeId: productionStep.orgNodeId,
  workCenterId: productionStep.workCenterId,
  operationName: productionStep.operationName,
  standardTimeMinutes: productionStep.standardTimeMinutes,
  actualTimeMinutes: productionStep.actualTimeMinutes,
  forQuantity: productionStep.forQuantity,
  completedQuantity: productionStep.completedQuantity,
  processLossQuantity: productionStep.processLossQuantity,
  allowOverproduction: productionStep.allowOverproduction,
  overproductionPercentage: productionStep.overproductionPercentage,
  operatorEmployeeId: productionStep.operatorEmployeeId,
  sequence: productionStep.sequence,
  status: productionStep.status,
};

const logColumns = {
  id: productionStepTimeLog.id,
  productionStepId: productionStepTimeLog.productionStepId,
  fromTime: productionStepTimeLog.fromTime,
  toTime: productionStepTimeLog.toTime,
  timeInMinutes: productionStepTimeLog.timeInMinutes,
  completedQuantity: productionStepTimeLog.completedQuantity,
  processLossQuantity: productionStepTimeLog.processLossQuantity,
};

const wtColumns = {
  id: workstationType.id,
  code: workstationType.code,
  name: workstationType.name,
  status: workstationType.status,
};
const opColumns = {
  id: operation.id,
  code: operation.code,
...
const dtColumns = {
  id: downtimeEntry.id,
  workCenterId: downtimeEntry.workCenterId,
  operatorEmployeeId: downtimeEntry.operatorEmployeeId,
  stopReason: downtimeEntry.stopReason,
  startTime: downtimeEntry.startTime,
  stopTime: downtimeEntry.stopTime,
  stoppageMinutes: downtimeEntry.stoppageMinutes,
  remarks: downtimeEntry.remarks,
};
...
const wooColumns = {
  ...
  plannedStartTime: workOrderOperation.plannedStartTime,
  plannedEndTime: workOrderOperation.plannedEndTime,
  processLossQuantity: workOrderOperation.processLossQuantity,
  sequentialOrder: workOrderOperation.sequentialOrder,
};

const _matColumns = {
  id: productionStepMaterial.id,
  productionStepId: productionStepMaterial.productionStepId,
  itemId: productionStepMaterial.itemId,
  requiredQuantity: productionStepMaterial.requiredQuantity,
  consumedQuantity: productionStepMaterial.consumedQuantity,
  warehouseId: productionStepMaterial.warehouseId,
  lineNumber: productionStepMaterial.lineNumber,
};

const scoColumns = {
  id: subcontractingOrder.id,
  voucherNumber: subcontractingOrder.voucherNumber,
  orgNodeId: subcontractingOrder.orgNodeId,
  supplierId: subcontractingOrder.supplierId,
  workOrderId: subcontractingOrder.workOrderId,
  postingDate: subcontractingOrder.postingDate,
...
const sciColumns = {
  id: subcontractingItem.id,
  subcontractingOrderId: subcontractingItem.subcontractingOrderId,
  itemId: subcontractingItem.itemId,
  warehouseId: subcontractingItem.warehouseId,
  quantity: subcontractingItem.quantity,
...
export class ProductionOpsRepository {
  ...
  async listSteps(jobOrderReference?: string): Promise<ProductionStepRecord[]> {
    ...
    const rows = jobOrderReference
      ? await query
          .where(eq(productionStep.jobOrderReference, jobOrderReference))
          .orderBy(asc(productionStep.sequence))
      : await query.orderBy(asc(productionStep.sequence));
  ...
  async countSteps(jobOrderReference: string): Promise<number> {
    const rows = await this.database.db
      .select({ id: productionStep.id })
      .from(productionStep)
      .where(eq(productionStep.jobOrderReference, jobOrderReference));
    return rows.length;
  }
  async insertStep(
    input: CreateProductionStepInput & { id: string; sequence: number; orgNodeId: string | null },
  ): Promise<ProductionStepRecord> {
    const rows = await this.database.db
      .insert(productionStep)
      .values({
        id: input.id,
        ...
        forQuantity: input.forQuantity ?? null,
        allowOverproduction: input.allowOverproduction ?? false,
        overproductionPercentage: input.overproductionPercentage ?? null,
  ...
  async recordActualTime(id: string, actualTimeMinutes: string): Promise<ProductionStepRecord> {
    const rows = await this.database.db
      .update(productionStep)
      .set({ actualTimeMinutes, status: 'done', updatedAt: new Date() })
      .where(eq(productionStep.id, id))
      .returning(stepColumns);
    return { ...rows[0]!, status: rows[0]!.status as ProductionStepStatus };
  }
  ...
  async addTimeLog(input: {
    ...
    processLossQuantity?: string;
  }): Promise<ProductionStepTimeLogRecord> {
    const rows = await this.database.db
      .insert(productionStepTimeLog)
      .values({
        id: input.id,
        productionStepId: input.productionStepId,
        fromTime: new Date(input.fromTime),
        toTime: input.toTime ? new Date(input.toTime) : null,
        timeInMinutes: input.timeInMinutes ?? null,
        completedQuantity: input.completedQuantity ?? null,
        processLossQuantity: input.processLossQuantity ?? null,
      })
    ...
    return {
      ...
      fromTime: r.fromTime.toISOString(),
      toTime: r.toTime ? r.toTime.toISOString() : null,
      timeInMinutes: r.timeInMinutes,
      completedQuantity: r.completedQuantity,
      processLossQuantity: r.processLossQuantity,
    };
  }
  async listTimeLogs(productionStepId: string): Promise<ProductionStepTimeLogRecord[]> {
    const rows = await this.database.db
      .select(logColumns)
      .from(productionStepTimeLog)
      .where(eq(productionStepTimeLog.productionStepId, productionStepId));
    return rows.map((r) => ({
      id: r.id,
      productionStepId: r.productionStepId,
      fromTime: r.fromTime.toISOString(),
  ...
  async incrementCompletedQuantity(productionStepId: string, qty: string): Promise<void> {
    await this.database.db.execute(
      sql`UPDATE ${productionStep} SET completed_quantity = completed_quantity + ${qty}::numeric WHERE id = ${productionStepId}`,
    );
  }
  ...
  async listWorkOrders(): Promise<WorkOrderRecord[]> {
    ...
    return rows.map((r) => ({
      ...
      qtyToManufacture: r.qtyToManufacture,
      sourceWarehouseId: r.sourceWarehouseId,
      wipWarehouseId: r.wipWarehouseId,
      finishedGoodsWarehouseId: r.finishedGoodsWarehouseId,
      ...
      useMultiLevelBom: r.useMultiLevelBom,
      considerScrapItems: r.considerScrapItems,
      materialConsumptionPercentage: r.materialConsumptionPercentage,
      materialTransferMode: r.materialTransferMode as MaterialTransferMode,
      trackOperations: r.trackOperations,
  ...
  async insertWorkOrder(
    input: CreateWorkOrderInput & { id: string; workOrderNumber: string },
    ...
    const rows = await this.database.db
      ...
      .values({
        ...
        qtyToManufacture: input.qtyToManufacture,
        ...
        useMultiLevelBom: input.useMultiLevelBom ?? false,
        considerScrapItems: input.considerScrapItems ?? false,
        materialConsumptionPercentage: input.materialConsumptionPercentage ?? '100',
        materialTransferMode: input.materialTransferMode ?? 'transfer',
  ...
  async recordActualStart(id: string): Promise<WorkOrderRecord> {
    await this.database.db
      .update(workOrder)
      .set({ status: 'in_progress', actualStartDate: new Date(), updatedAt: new Date() })
      .where(eq(workOrder.id, id));
  ...
  async recordActualEnd(id: string, status: WorkOrderStatus): Promise<WorkOrderRecord> {
    await this.database.db
      .update(workOrder)
      .set({ status, actualEndDate: new Date(), updatedAt: new Date() })
      .where(eq(workOrder.id, id));
    return (await this.findWorkOrderById(id))!;
  }
  ...
  async listWorkOrderOperations(workOrderId: string): Promise<WorkOrderOperationRecord[]> {
    const rows = await this.database.db
      ...
      .where(eq(workOrderOperation.workOrderId, workOrderId));
    return rows.map((r) => ({
      id: r.id,
      ...
      processLossQuantity: r.processLossQuantity,
      sequentialOrder: r.sequentialOrder,
    }));
  }
  ...
  async findWorkstationTypeByCode(code: string): Promise<WorkstationTypeRecord | null> {
    const rows = await this.database.db
      ...
      .where(eq(workstationType.code, code))
      .limit(1);
    return rows[0] ? { ...rows[0], status: rows[0].status as WorkstationTypeStatus } : null;
  }
  async insertWorkstationType(
    input: CreateWorkstationTypeInput & { id: string },
  ): Promise<WorkstationTypeRecord> {
  ...
  async listDowntimeEntries(): Promise<DowntimeEntryRecord[]> {
    ...
    return rows.map((r) => ({
      ...
      stopTime: r.stopTime ? r.stopTime.toISOString() : null,
      stoppageMinutes: r.stoppageMinutes,
      remarks: r.remarks,
    }));
  }
  ...
  async insertDowntimeEntry(
    input: CreateDowntimeEntryInput & { id: string },
  ): Promise<DowntimeEntryRecord> {
    const rows = await this.database.db
      .insert(downtimeEntry)
      .values({
        id: input.id,
        workCenterId: input.workCenterId,
        operatorEmployeeId: input.operatorEmployeeId ?? null,
  ...
  async closeDowntimeEntry(
    id: string,
    stopTime: Date,
    stoppageMinutes: string,
  ): Promise<DowntimeEntryRecord> {
    const rows = await this.database.db
      .update(downtimeEntry)
      .set({ stopTime, stoppageMinutes, updatedAt: new Date() })
      .where(eq(downtimeEntry.id, id))
      .returning(dtColumns);
    const r = rows[0]!;
    return {
  ...
  async replaceBomInWorkOrders(oldBomId: string, newBomId: string): Promise<number> {
    const result = await this.database.db
      .update(workOrder)
      .set({ bomId: newBomId, updatedAt: new Date() })
      .where(eq(workOrder.bomId, oldBomId));
  ...
  async listSubcontractingOrders(orgNodeId?: string): Promise<SubcontractingOrderRecord[]> {
    ...
    const rows = orgNodeId
      ? await query.where(eq(subcontractingOrder.orgNodeId, orgNodeId))
      : await query;
    const results: SubcontractingOrderRecord[] = [];
    for (const r of rows) {
      const items = await this.database.db
        .select(sciColumns)
        .from(subcontractingItem)
        .where(eq(subcontractingItem.subcontractingOrderId, r.id));
      results.push({
        id: r.id,
        ...
        items: items.map((i) => ({
          id: i.id,
          subcontractingOrderId: i.subcontractingOrderId,
          itemId: i.itemId,
  ...
  async insertSubcontractingOrder(
    ...
    for (const item of input.computedItems) {
      const itemRows = await this.database.db
        .insert(subcontractingItem)
        .values({
          id: item.id,
          subcontractingOrderId: insertedOrder.id,
          itemId: item.itemId,
      ...
      insertedItems.push({
        id: lci.id,
        subcontractingOrderId: lci.subcontractingOrderId,
        itemId: lci.itemId,
    ...
    return {
      id: insertedOrder.id,
      voucherNumber: insertedOrder.voucherNumber,
      orgNodeId: insertedOrder.orgNodeId,
      supplierId: insertedOrder.supplierId,
      workOrderId: insertedOrder.workOrderId,
      postingDate: insertedOrder.postingDate.toISOString(),
  ...
  async setSubcontractingOrderStatus(
    id: string,

...
export interface LeaveTypeRecord { id: string; code: string; name: string; maxDaysPerAllocation: string | null; }
export interface LeaveAllocationRecord {
  id: string; employeeId: string; leaveTypeId: string; fromDate: string; toDate: string; days: string; batchReference: string | null;
}
export interface BulkAllocationInput {
  leaveTypeId: string; fromDate: string; toDate: string; days: string;
  /** Filters (combined with AND). Only ACTIVE employees are ever allocated. */
  orgNodeId?: string; role?: string; employeeIds?: string[];
}
...
export class LeaveService {
  ...
  async listTypes(): Promise<LeaveTypeRecord[]> {
    ...
    return rows.map((r) => ({ id: r.id, code: r.code, name: r.name, maxDaysPerAllocation: r.maxDaysPerAllocation }));
  }

  async createType(input: { code: string; name: string; maxDaysPerAllocation?: string }): Promise<LeaveTypeRecord> {
    const code = input.code?.trim();
    if (!code || !input.name?.trim()) throw new HrValidationError('code and name are required');
    const exists = await this.database.db.select({ id: leaveType.id }).from(leaveType).where(eq(leaveType.code, code)).limit(1);
    if (exists.length > 0) throw new HrValidationError(`leave type "${code}" already exists`);
    const rows = await this.database.db.insert(leaveType).values({ code, name: input.name.trim(), maxDaysPerAllocation: input.maxDaysPerAllocation ?? null }).returning();
    const r = rows[0]!;
  ...
  async listAllocations(employeeId?: string): Promise<LeaveAllocationRecord[]> {
    ...
    const rows = employeeId ? await q.where(eq(leaveAllocation.employeeId, employeeId)).orderBy(asc(leaveAllocation.fromDate)) : await q.orderBy(asc(leaveAllocation.fromDate));
  ...
  async bulkAllocate(input: BulkAllocationInput): Promise<BulkAllocationResult> {
    const type = await this.database.db.select().from(leaveType).where(eq(leaveType.id, input.leaveTypeId)).limit(1);
    if (!type[0]) throw new HrNotFoundError(`leave type ${input.leaveTypeId} does not exist`);
    const from = new Date(input.fromDate);
    const to = new Date(input.toDate);
    if (Number.isNaN(from.getTime()) || Number.isNaN(to.getTime()) || to < from) throw new HrValidationError('invalid period');
    const days = Number(input.days);
    if (!(days > 0)) throw new HrValidationError('days must be positive');
    const max = type[0].maxDaysPerAllocation;
    if (max !== null && days > Number(max)) throw new HrValidationError(`${type[0].code} allows at most ${Number(max)} days per allocation`);
    ...
    if (input.orgNodeId) {
      ...
      const walk = (n: OrgTreeNode): void => { scope!.add(n.id); n.children.forEach(walk); };
      walk(root);
    }
    const wanted = input.employeeIds ? new Set(input.employeeIds) : null;
    const targets = (await this.employees.listEmployees()).filter((e) =>
      e.status === 'active'
    ...
    for (const e of targets) {
      const overlap = await this.database.db.select({ id: leaveAllocation.id }).from(leaveAllocation).where(and(
        eq(leaveAllocation.employeeId, e.id), eq(leaveAllocation.leaveTypeId, input.leaveTypeId),
        lte(leaveAllocation.fromDate, to), gte(leaveAllocation.toDate, from),
      )).limit(1);
      if (overlap.length > 0) {
        result.skipped.push({ employeeCode: e.code, reason: 'لديه تخصيص متداخل لنفس نوع الإجازة في نفس الفترة' });
        continue;
      }
      const rows = await this.database.db.insert(leaveAllocation).values({
        employeeId: e.id, leaveTypeId: input.leaveTypeId, fromDate: from, toDate: to, days: days.toFixed(2), batchReference,
...
function toAllocation(r: typeof leaveAllocation.$inferSelect): LeaveAllocationRecord {
  return {
    id: r.id, employeeId: r.employeeId, leaveTypeId: r.leaveTypeId, fromDate: r.fromDate.toISOString(),
    toDate: r.toDate.toISOString(), days: r.days, batchReference: r.batchReference,
  };
}

...
export const employee = hrSchema.table(
  'employee',
  {
    ...
    reportsTo: uuid('reports_to').references((): AnyPgColumn => employee.id, {
      onUpdate: 'cascade',
      onDelete: 'restrict',
    }),
  ...
  (t) => [
    ...
    check(
      'employee_probation_after_joining',
      sql`${t.probationEndDate} is null or (${t.dateOfJoining} is not null and ${t.probationEndDate} >= ${t.dateOfJoining})`,
    ),
    check(
      'employee_confirmation_after_joining',
      sql`${t.confirmationDate} is null or (${t.dateOfJoining} is not null and ${t.confirmationDate} >= ${t.dateOfJoining})`,
    ),
 ...
 * hard-coded (D37).
 */
export const commissionRule = hrSchema.table(
  'commission_rule',
  {
    id: uuid('id').primaryKey().defaultRandom(),
 ...
 */
export const commissionEntry = hrSchema.table(
  'commission_entry',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    employeeId: uuid('employee_id')
...
export const externalCommission = hrSchema.table(
  'external_commission',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const payrollEntry = hrSchema.table(
  'payroll_entry',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    employeeId: uuid('employee_id')
    ...
    totalCommissions: numeric('total_commissions', { precision: 12, scale: 4 })
      .notNull()
      .default('0'),
...
export const leaveType = hrSchema.table(
  'leave_type',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    code: text('code').notNull(),
...
export const leaveAllocation = hrSchema.table(
  'leave_allocation',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    employeeId: uuid('employee_id')
    ...
    leaveTypeId: uuid('leave_type_id')
      .notNull()
      .references(() => leaveType.id, { onUpdate: 'cascade', onDelete: 'restrict' }),
...
export const finalSettlement = hrSchema.table(
  'final_settlement',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    totalReceivable: numeric('total_receivable', { precision: 14, scale: 4 })
      .notNull()
      .default('0'),
  ...
  (t) => [
    ...
    uniqueIndex('final_settlement_one_open_per_employee')
      .on(t.employeeId)
      .where(sql`${t.status} <> 'cancelled'`),
  ],
);

export const finalSettlementLine = hrSchema.table(
  'final_settlement_line',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    settlementId: uuid('settlement_id')
      .notNull()
      .references(() => finalSettlement.id, { onUpdate: 'cascade', onDelete: 'cascade' }),
  ...
  (t) => [
    check(
      'final_settlement_line_direction_valid',
      sql`${t.direction} in ('payable', 'receivable')`,
    ),
...
export const probationEvent = hrSchema.table(
  'probation_event',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    employeeId: uuid('employee_id')
  ...
  (t) => [
    check(
      'probation_event_type_valid',
      sql`${t.eventType} in ('started', 'extended', 'confirmed')`,
    ),

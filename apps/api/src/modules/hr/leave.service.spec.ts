...
describe('Bulk leave allocation validation (plan item 20)', () => {
  const chain = (rows: unknown[]): unknown => ({ from: () => ({ where: () => ({ limit: async () => rows }) }) });
  const service = (type: unknown | null, employees: unknown[] = []): LeaveService => new LeaveService(
  ...
  it('rejects an unknown type, a reversed period, non-positive days, days above the maximum, and no match', async () => {
    ...
    await expect(service(annual).bulkAllocate({ ...base, toDate: '2025-01-01' })).rejects.toThrow(/invalid period/);
    await expect(service(annual).bulkAllocate({ ...base, days: '0' })).rejects.toThrow(/days must be positive/);
    await expect(service(annual).bulkAllocate({ ...base, days: '22' })).rejects.toThrow(/at most 21 days/);
    const onlyTerminated = [{ id: 'e', code: 'E', status: 'terminated', role: 'x', orgNodeId: 'o' }];
    await expect(service(annual, onlyTerminated).bulkAllocate(base)).rejects.toThrow(/no active employee/);
  });
});

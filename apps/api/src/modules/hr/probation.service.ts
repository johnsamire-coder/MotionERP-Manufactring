...
const parseDay = (s: string, field: string): Date => {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(s))
    throw new HrValidationError(`${field} must be a date like 2026-01-31`);
  const d = new Date(`${s}T00:00:00Z`);
  if (Number.isNaN(d.getTime()) || isoDay(d) !== s)
    throw new HrValidationError(`${field} is not a valid date`);
  return d;
};
...
export function probationState(
  p: { probationEndDate: string | null; confirmationDate: string | null },
  today: string,
  ...
  const daysLeft = Math.round(
    (parseDay(p.probationEndDate, 'date').getTime() - parseDay(today, 'date').getTime()) / DAY,
  );
...
export class ProbationService {
  ...
  async get(employeeId: string, today = isoDay(new Date())): Promise<ProbationRecord> {
    const e = (
      await this.database.db.select().from(employee).where(eq(employee.id, employeeId)).limit(1)
    )[0];
    if (!e) throw new HrNotFoundError(`employee ${employeeId} does not exist`);
    const events = await this.database.db
      .select()
      .from(probationEvent)
      .where(eq(probationEvent.employeeId, employeeId))
      .orderBy(asc(probationEvent.createdAt));
    return {
      employeeId: e.id,
      employeeCode: e.code,
      employeeName: e.name,
  ...
  async start(
    employeeId: string,
    input: { dateOfJoining: string; months?: number; probationEndDate?: string },
  ): Promise<ProbationRecord> {
    const p = await this.get(employeeId);
    if (p.status !== 'active')
      throw new HrValidationError(`employee ${p.employeeCode} is "${p.status}"`);
    if (p.probationEndDate || p.confirmationDate) {
      throw new HrValidationError(
        `الموظف ${p.employeeCode} عدّى بفترة اختبار قبل كده — فترة الاختبار مرة واحدة بس عند نفس صاحب العمل`,
      );
    }
    parseDay(input.dateOfJoining, 'dateOfJoining');
    const end =
      input.probationEndDate ??
      probationEnd(input.dateOfJoining, input.months ?? MAX_PROBATION_MONTHS);
    this.checkEnd(input.dateOfJoining, end);
    await this.database.db
      .update(employee)
      .set({ dateOfJoining: input.dateOfJoining, probationEndDate: end, updatedAt: new Date() })
  ...
  async extend(
    employeeId: string,
    input: { probationEndDate: string; reason: string },
  ): Promise<ProbationRecord> {
    const p = await this.openProbation(employeeId);
    if (!input.reason?.trim())
      throw new HrValidationError('a reason is required to extend a probation');
    if (
      parseDay(input.probationEndDate, 'probationEndDate') <= parseDay(p.probationEndDate!, 'date')
    ) {
    ...
    await this.database.db
      .update(employee)
      .set({ probationEndDate: input.probationEndDate, updatedAt: new Date() })
      .where(eq(employee.id, employeeId));
    await this.log(
      employeeId,
      'extended',
  ...
  async confirm(
    employeeId: string,
    input: { confirmationDate?: string; note?: string } = {},
    ...
    if (parseDay(day, 'confirmationDate') < parseDay(p.dateOfJoining!, 'date'))
      throw new HrValidationError('confirmationDate cannot be before the joining day');
    if (day > today) throw new HrValidationError('confirmationDate cannot be in the future');
    await this.database.db
      .update(employee)
      .set({ confirmationDate: day, updatedAt: new Date() })
      .where(eq(employee.id, employeeId));
  ...
  async due(withinDays = 14, today = isoDay(new Date())): Promise<ProbationRecord[]> {
    const rows = await this.database.db
      ...
      .from(employee)
      .where(isNotNull(employee.probationEndDate));
  ...
  private checkEnd(dateOfJoining: string, end: string): void {
    ...
    if (parseDay(end, 'probationEndDate') < parseDay(dateOfJoining, 'dateOfJoining'))
      throw new HrValidationError('probation cannot end before the joining day');
    if (end > max)
      throw new HrValidationError(
        `فترة الاختبار متزيدش عن ${MAX_PROBATION_MONTHS} شهور (قانون العمل) — آخر يوم مسموح ${max}`,
      );
  }
  ...
  private async openProbation(employeeId: string): Promise<ProbationRecord> {
    ...
    if (p.status !== 'active')
      throw new HrValidationError(`employee ${p.employeeCode} is "${p.status}"`);
    if (p.confirmationDate)
      throw new HrValidationError(
        `employee ${p.employeeCode} was already confirmed on ${p.confirmationDate}`,
      );
    if (!p.probationEndDate)
      throw new HrValidationError(`employee ${p.employeeCode} has no probation — start one first`);
    return p;
  }

  private async log(
    employeeId: string,
    eventType: 'started' | 'extended' | 'confirmed',
    eventDate: string,
    end: string | null,
    reason: string | null,
  ): Promise<void> {
    await this.database.db
      .insert(probationEvent)
      .values({ employeeId, eventType, eventDate, probationEndDate: end, reason });
  }
}

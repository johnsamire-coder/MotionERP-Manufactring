...
export class HrService {
  ...
  async setReportsTo(id: string, managerId: string | null): Promise<EmployeeRecord> {
    ...
    if (managerId !== null) {
      ...
      if (manager.status !== 'active')
        throw new HrValidationError(`manager ${manager.code} is not active`);
      ...
      while (cursor?.reportsTo && !seen.has(cursor.id)) {
        if (cursor.reportsTo === id)
          throw new HrValidationError(
            `${manager.code} already reports (directly or indirectly) to ${emp.code}; that would create a loop`,
          );
        seen.add(cursor.id);
  ...
  async terminateEmployee(
  ...
  ): Promise<EmployeeRecord & { deactivatedUsers: string[] }> {
    const emp = await this.getEmployee(id);
    if (emp.status === 'terminated')
      throw new HrValidationError(`employee ${emp.code} is already terminated`);
    ...
    if (Number.isNaN(date.getTime()))
      throw new HrValidationError('relievingDate is not a valid date');
    ...
    const deactivatedUsers = this.authService
      ? await this.authService.deactivateUsersForEmployee([emp.code, emp.id])
      : [];
  ...
  async getCommissionRules(employeeId?: string): Promise<CommissionRuleRecord[]> {
    return this.repository.listCommissionRules(employeeId);
  }
  ...
  async createCommissionRule(input: CreateCommissionRuleInput): Promise<CommissionRuleRecord> {
    ...
    if (!Number.isFinite(rate) || rate <= 0)
      throw new HrValidationError('ratePercentage must be positive');
    return this.repository.insertCommissionRule({ id: randomUUID(), ...input });
  }

  async getCommissionEntries(employeeId?: string): Promise<CommissionEntryRecord[]> {
    return this.repository.listCommissionEntries(employeeId);
  }
  ...
  async earnCommission(
    ...
    if (!Number.isFinite(base) || base <= 0)
      throw new HrValidationError('basisAmount must be positive');
  ...
  async getExternalCommissions(): Promise<ExternalCommissionRecord[]> {
    return this.repository.listExternalCommissions();
  }

  async createExternalCommission(
    input: CreateExternalCommissionInput,
  ): Promise<ExternalCommissionRecord> {
    const amount = Number(input.amount);
    if (!Number.isFinite(amount) || amount <= 0)
      throw new HrValidationError('amount must be positive');
    if (!input.beneficiaryName?.trim()) throw new HrValidationError('beneficiaryName is required');
    if (!input.basisDescription?.trim())
      throw new HrValidationError('basisDescription is required');
    return this.repository.insertExternalCommission({ id: randomUUID(), ...input });
  }

  async getPayrollEntries(employeeId?: string): Promise<PayrollEntryRecord[]> {
    return this.repository.listPayrollEntries(employeeId);
  }
  ...
  async generatePayroll(
    ...
    if (existing)
      throw new HrValidationError(
        `payroll for employee ${employeeId} in ${periodYear}-${periodMonth} already exists`,
      );

    const pendingCommissions = await this.repository.listPendingCommissionsForPeriod(employeeId);
    const totalCommissions = pendingCommissions.reduce(
...
function normalizeCode(raw: unknown): string {
  ...
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/.test(trimmed))
    throw new HrValidationError('code must match ^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$');
  return trimmed;
}

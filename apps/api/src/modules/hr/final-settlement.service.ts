...
export function suggestSettlementLines(
  ...
  for (const p of unpaidPayroll) {
    lines.push({
      direction: 'payable',
      component: 'unpaid_salary',
      description: `مرتب غير مصروف ${p.periodYear}-${p.periodMonth}`,
  ...
  if (!lastMonthHasPayroll && baseSalary > 0) {
    ...
    const daysInMonth = new Date(
      Date.UTC(relieving.getUTCFullYear(), relieving.getUTCMonth() + 1, 0),
    ).getUTCDate();
    lines.push({
      direction: 'payable',
      component: 'prorated_salary',
      description: `مرتب نسبي ${day} من ${daysInMonth} يوم`,
      amount: (baseSalary * day) / daysInMonth,
      isAuto: true,
    });
  }
  ...
  for (const a of allocations) {
    ...
    const elapsed = Math.min(
      total,
      Math.max(0, (relieving.getTime() - a.fromDate.getTime()) / DAY + 1),
    );
    ...
    if (earned > 0 && dailyRate > 0) {
      lines.push({
        ...
        description: `بدل رصيد إجازات مستحق ${earned.toFixed(2)} يوم`,
        amount: earned * dailyRate,
        isAuto: true,
      });
    }
  }
...
export class FinalSettlementService {
  ...
  async create(employeeId: string): Promise<FinalSettlementRecord> {
    ...
    if (emp.status !== 'terminated' || !emp.relievingDate) {
      throw new HrValidationError(
        `employee ${emp.code} has not left yet — terminate the service first (it sets the relieving date)`,
      );
    }
    const open = await this.database.db
      .select({ id: finalSettlement.id })
      .from(finalSettlement)
      .where(
        and(eq(finalSettlement.employeeId, employeeId), ne(finalSettlement.status, 'cancelled')),
      )
      .limit(1);
    if (open.length > 0)
      throw new HrValidationError(`employee ${emp.code} already has a final settlement`);

    const relieving = new Date(emp.relievingDate);
    const unpaid = await this.database.db
      .select()
      .from(payrollEntry)
      .where(
        and(
          eq(payrollEntry.employeeId, employeeId),
          inArray(payrollEntry.status, ['draft', 'approved']),
        ),
      );
    const lastMonth = await this.database.db
      ...
      .limit(1);
    const allocations = await this.database.db
      .select()
      .from(leaveAllocation)
      .where(
        and(
          eq(leaveAllocation.employeeId, employeeId),
          lte(leaveAllocation.fromDate, relieving),
          gte(leaveAllocation.toDate, relieving),
        ),
      );
    ...
    const n =
      Number((await this.database.db.select({ n: count() }).from(finalSettlement))[0]?.n ?? 0) + 1;
    const rows = await this.database.db
      .insert(finalSettlement)
      .values({
        settlementNumber: `FNF-${relieving.getUTCFullYear()}-${String(n).padStart(6, '0')}`,
        employeeId,
    ...
    if (lines.length > 0) {
      await this.database.db
        .insert(finalSettlementLine)
        .values(lines.map((l) => ({ ...l, settlementId: id, amount: l.amount.toFixed(4) })));
    }
  ...
  async get(id: string): Promise<FinalSettlementRecord> {
    ...
    const lines = await this.database.db
      .select()
      .from(finalSettlementLine)
      .where(eq(finalSettlementLine.settlementId, id))
      .orderBy(asc(finalSettlementLine.direction));
    return {
      id: s.id,
      settlementNumber: s.settlementNumber,
      employeeId: s.employeeId,
      relievingDate: s.relievingDate.toISOString(),
      status: s.status as FinalSettlementRecord['status'],
      ...
      lines: lines.map((l) => ({
        id: l.id,
        direction: l.direction as SettlementDirection,
  ...
  async addLine(
    ...
    if (!line.description?.trim() || !line.component?.trim())
      throw new HrValidationError('component and description are required');
    await this.database.db.insert(finalSettlementLine).values({
      settlementId: id,
      direction: line.direction,
      component: line.component.trim(),
  ...
  async removeLine(id: string, lineId: string): Promise<FinalSettlementRecord> {
    await this.assertDraft(id);
    await this.database.db
      .delete(finalSettlementLine)
      .where(and(eq(finalSettlementLine.id, lineId), eq(finalSettlementLine.settlementId, id)));
    return this.recalculate(id);
  }
  ...
  private async assertDraft(id: string): Promise<void> {
    ...
    if (s.status !== 'draft')
      throw new HrValidationError(
        `final settlement ${s.settlementNumber} is "${s.status}" and can no longer change`,
      );
  }
  ...
  private async recalculate(id: string): Promise<FinalSettlementRecord> {
    const lines = await this.database.db
      ...
      .from(finalSettlementLine)
      .where(eq(finalSettlementLine.settlementId, id));
    const sum = (d: SettlementDirection): number =>
      lines.filter((l) => l.direction === d).reduce((a, l) => a + Number(l.amount), 0);
    const payable = sum('payable');
    const receivable = sum('receivable');
    await this.database.db
      .update(finalSettlement)
      .set({
        totalPayable: payable.toFixed(4),
        totalReceivable: receivable.toFixed(4),
        netAmount: (payable - receivable).toFixed(4),
      })
      .where(eq(finalSettlement.id, id));
    return this.get(id);
  }

...
import type {
  ...
  CreateEmployeeInput,
  CreateExternalCommissionInput,
  EmployeeRecord,
  EmployeeStatus,
  ExternalCommissionRecord,
  ExternalCommissionStatus,
  PayrollEntryRecord,
  PayrollEntryStatus,
} from './hr.types';
...
const ruleColumns = {
  ...
  employeeId: commissionRule.employeeId,
  basis: commissionRule.basis,
  ratePercentage: commissionRule.ratePercentage,
  status: commissionRule.status,
};
const entryColumns = {
  id: commissionEntry.id,
  employeeId: commissionEntry.employeeId,
  jobOrderReference: commissionEntry.jobOrderReference,
  sourceReference: commissionEntry.sourceReference,
  baseAmount: commissionEntry.baseAmount,
  commissionAmount: commissionEntry.commissionAmount,
  earnedDate: commissionEntry.earnedDate,
  status: commissionEntry.status,
};
const extColumns = {
  id: externalCommission.id,
  beneficiaryName: externalCommission.beneficiaryName,
  jobOrderReference: externalCommission.jobOrderReference,
  amount: externalCommission.amount,
  basisDescription: externalCommission.basisDescription,
  dueDate: externalCommission.dueDate,
  status: externalCommission.status,
};
const payColumns = {
  id: payrollEntry.id,
  employeeId: payrollEntry.employeeId,
  periodYear: payrollEntry.periodYear,
  periodMonth: payrollEntry.periodMonth,
  baseSalary: payrollEntry.baseSalary,
  totalCommissions: payrollEntry.totalCommissions,
...
function toRuleRecord(row: RuleRow): CommissionRuleRecord {
  return {
    id: row.id,
...
function toExtRecord(row: ExtRow): ExternalCommissionRecord {
  return {
    ...
    jobOrderReference: row.jobOrderReference,
    amount: row.amount,
    basisDescription: row.basisDescription,
    dueDate: row.dueDate ? row.dueDate.toISOString() : null,
    status: row.status as ExternalCommissionStatus,
  };
}
function toPayRecord(row: PayRow): PayrollEntryRecord {
  return {
    ...
    status: row.status as PayrollEntryStatus,
  };
}
...
export class HrRepository {
  ...
  async setEmployeeReportsTo(id: string, reportsTo: string | null): Promise<EmployeeRecord> {
    const rows = await this.database.db
      .update(employee)
      .set({ reportsTo, updatedAt: new Date() })
      .where(eq(employee.id, id))
  ...
  async setEmployeeStatus(
    ...
    const rows = await this.database.db
      .update(employee)
      .set({ status, relievingDate, updatedAt: new Date() })
      .where(eq(employee.id, id))
      .returning(empColumns);
    return toEmpRecord(rows[0]!);
  }
  ...
  async listActiveSubordinates(managerId: string): Promise<EmployeeRecord[]> {
    const rows = await this.database.db
      .select(empColumns)
      .from(employee)
      .where(and(eq(employee.reportsTo, managerId), eq(employee.status, 'active')))
      .orderBy(asc(employee.code));
    return rows.map(toEmpRecord);
  }
  ...
  async listCommissionRules(employeeId?: string): Promise<CommissionRuleRecord[]> {
    const rows = employeeId
      ? await this.database.db
          .select(ruleColumns)
          .from(commissionRule)
          .where(eq(commissionRule.employeeId, employeeId))
  ...
  async findActiveRuleForEmployee(employeeId: string): Promise<CommissionRuleRecord | null> {
    const rows = await this.database.db
      .select(ruleColumns)
      .from(commissionRule)
      .where(and(eq(commissionRule.employeeId, employeeId), eq(commissionRule.status, 'active')))
      .limit(1);
    return rows[0] ? toRuleRecord(rows[0]) : null;
  }
  async insertCommissionRule(
    input: CreateCommissionRuleInput & { id: string },
  ...
  async listCommissionEntries(employeeId?: string): Promise<CommissionEntryRecord[]> {
    const rows = employeeId
      ? await this.database.db
          ...
          .where(eq(commissionEntry.employeeId, employeeId))
          .orderBy(asc(commissionEntry.earnedDate))
      : await this.database.db
          .select(entryColumns)
          .from(commissionEntry)
          .orderBy(asc(commissionEntry.earnedDate));
  ...
  async insertCommissionEntry(input: {
    id: string;
    employeeId: string;
    jobOrderReference: string;
    sourceReference: string;
  ...
  async listPendingCommissionsForPeriod(employeeId: string): Promise<CommissionEntryRecord[]> {
    const rows = await this.database.db
      ...
      .from(commissionEntry)
      .where(
        and(eq(commissionEntry.employeeId, employeeId), eq(commissionEntry.status, 'pending')),
      );
    return rows.map(toEntryRecord);
  }
  ...
  async insertExternalCommission(
    input: CreateExternalCommissionInput & { id: string },
  ): Promise<ExternalCommissionRecord> {
    const rows = await this.database.db
      ...
      .values({
        ...
        amount: input.amount,
        basisDescription: input.basisDescription,
    ...
    const rows = await this.database.db
      ...
      .where(
        and(
          ...
          eq(payrollEntry.periodYear, year),
          eq(payrollEntry.periodMonth, month),
        ),
      )
      .limit(1);
    return rows[0] ? toPayRecord(rows[0]) : null;
  }
  async insertPayrollEntry(input: {
    id: string;
    employeeId: string;
    periodYear: string;
    periodMonth: string;

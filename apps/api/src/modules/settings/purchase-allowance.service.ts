...
export class PurchaseAllowanceService {
  ...
  async resolve(orgNodeId?: string | null): Promise<PurchaseAllowances> {
    if (orgNodeId) {
      const { node, ancestors } = await this.orgNodeOrThrow(() => this.organization.getAncestors(orgNodeId));
      for (const id of [node.id, ...ancestors.map((a) => a.id).reverse()]) {
        const row = await this.find(id);
        if (row) return row;
      }
    }
    return (await this.find(null)) ?? { source: 'none', overOrderPct: 0, overReceiptPct: 0, overBillingPct: 0 };
  }

  async set(orgNodeId: string | null, values: { overOrderPct: number; overReceiptPct: number; overBillingPct: number }): Promise<PurchaseAllowances> {
    for (const v of [values.overOrderPct, values.overReceiptPct, values.overBillingPct]) {
      if (!Number.isFinite(v) || v < 0 || v > 100) throw new SettingsValidationError('allowances must be between 0 and 100 percent');
    }
    if (orgNodeId) await this.orgNodeOrThrow(() => this.organization.getNode(orgNodeId));
    const row = { overOrderPct: values.overOrderPct.toFixed(2), overReceiptPct: values.overReceiptPct.toFixed(2), overBillingPct: values.overBillingPct.toFixed(2), updatedAt: new Date() };
    await this.database.db.insert(purchaseAllowance).values({ orgNodeId, ...row })
  ...
  private async find(orgNodeId: string | null): Promise<PurchaseAllowances | null> {
    const rows = await this.database.db.select().from(purchaseAllowance)
      .where(orgNodeId ? eq(purchaseAllowance.orgNodeId, orgNodeId) : isNull(purchaseAllowance.orgNodeId)).limit(1);
    const r = rows[0];
    if (!r) return null;
    return {
      source: r.orgNodeId ?? 'global',
      overOrderPct: Number(r.overOrderPct), overReceiptPct: Number(r.overReceiptPct), overBillingPct: Number(r.overBillingPct),
    };
  }
}

 ...
 */
export const companyProfile = settingsSchema.table(
  'company_profile',
  {
  ...
  (t) => [
    ...
    check(
      'company_profile_display_name_not_blank',
      sql`${t.displayName} is null or length(btrim(${t.displayName})) > 0`,
    ),
  ],
);
 ...
 */
export const purchaseAllowance = settingsSchema.table(
  'purchase_allowance',
  {
    id: uuid('id').primaryKey().defaultRandom(),
  ...
  (t) => [
    ...
    check(
      'purchase_allowance_range',
      sql`${t.overOrderPct} between 0 and 100 and ${t.overReceiptPct} between 0 and 100 and ${t.overBillingPct} between 0 and 100`,
    ),
  ],
);

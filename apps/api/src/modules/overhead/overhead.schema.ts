 ...
 */
export const overheadPool = overheadSchema.table('overhead_pool', {
  ...
  check('overhead_pool_category_valid', sql`${t.category} in ('manufacturing', 'administrative', 'selling', 'distribution')`),
 ...
 */
export const allocationPolicy = overheadSchema.table('allocation_policy', {
  id: uuid('id').primaryKey().defaultRandom(),
  orgNodeId: uuid('org_node_id')
    .notNull()
    .references(() => orgNode.id, { onUpdate: 'cascade', onDelete: 'restrict' }),
  ...
  check('allocation_base_valid', sql`${t.allocationBase} in (
    'units_produced',
    'direct_labor_hours',
    'direct_labor_cost',
    'machine_hours',
 ...
 */
export const allocationResult = overheadSchema.table('allocation_result', {
  id: uuid('id').primaryKey().defaultRandom(),
  poolId: uuid('pool_id')
    .notNull()
    .references(() => overheadPool.id, { onUpdate: 'cascade', onDelete: 'restrict' }),

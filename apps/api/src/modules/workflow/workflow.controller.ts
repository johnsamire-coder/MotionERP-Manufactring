...
import { ArrayMinSize, IsArray, IsBoolean, IsIn, IsObject, IsOptional, IsString, IsUUID, MaxLength, ValidateNested } from 'class-validator';
...
export class TaskDto { @IsString() action!: string; @IsIn(['email', 'webhook', 'print']) taskType!: TaskType; @IsObject() config!: Record<string, unknown>; }

@Controller({ path: 'workflow', version: '1' })
@UseFilters(WorkflowExceptionFilter)
export class WorkflowController {
  constructor(private readonly service: WorkflowService, private readonly automation: WorkflowAutomationService) {}
  ...
  @Post('definitions/:id/tasks') @HttpCode(201)
  async addTask(@Param('id', ParseUUIDPipe) id: string, @Body() dto: TaskDto): Promise<{ task: Awaited<ReturnType<WorkflowAutomationService['addTask']>> }> {
    return { task: await this.automation.addTask(id, dto) };
  }

  @Get('definitions/:id/tasks')
  async tasks(@Param('id', ParseUUIDPipe) id: string): Promise<{ tasks: Awaited<ReturnType<WorkflowAutomationService['tasks']>> }> { return { tasks: await this.automation.tasks(id) }; }

  @Get('instances/:id/task-runs')
  async runs(@Param('id', ParseUUIDPipe) id: string): Promise<{ runs: Awaited<ReturnType<WorkflowAutomationService['runs']>> }> { return { runs: await this.automation.runs(id) }; }
}

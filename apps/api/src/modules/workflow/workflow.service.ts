...
export function validateDefinition(d: DefinitionInput): void {
  ...
  if (codes.size !== d.states.length)
    throw new WorkflowValidationError('state codes must be unique');
  if (!codes.has(d.initialState))
    throw new WorkflowValidationError(`initial state "${d.initialState}" is not one of the states`);
  if (!d.states.some((s) => s.isFinal))
    throw new WorkflowValidationError('at least one state must be final');
  const seen = new Set<string>();
  for (const t of d.transitions) {
    if (!codes.has(t.fromState) || !codes.has(t.toState))
      throw new WorkflowValidationError(`transition "${t.action}" uses an unknown state`);
    if (t.fromState === t.toState)
      throw new WorkflowValidationError(`transition "${t.action}" must change the state`);
    if (d.states.find((s) => s.code === t.fromState)?.isFinal)
      throw new WorkflowValidationError(
        `final state "${t.fromState}" cannot have outgoing transitions`,
      );
    const key = `${t.fromState}|${t.action}`;
    if (seen.has(key))
      throw new WorkflowValidationError(
        `action "${t.action}" appears twice from state "${t.fromState}"`,
      );
    seen.add(key);
    if (t.condition !== undefined && t.condition !== null) {
      try {
        ...
        throw new WorkflowValidationError(`condition of "${t.action}": ${(err as Error).message}`);
      }
    }
  }
  // every non-final state must lead somewhere
  for (const s of d.states) {
    if (!s.isFinal && !d.transitions.some((t) => t.fromState === s.code))
      throw new WorkflowValidationError(
        `state "${s.code}" is a dead end (not final, no transitions)`,
      );
  }
}
...
export class WorkflowService {
  ...
  async createDefinition(d: DefinitionInput): Promise<{ id: string }> {
    ...
    if (
      (
        await db
          ...
          .where(eq(workflowDefinition.code, d.code))
          .limit(1)
      )[0]
    ) {
    ...
    await db
      ...
      .set({ isActive: false })
      .where(eq(workflowDefinition.documentType, d.documentType));
    const def = (
      await db
    ...
    if (d.transitions.length > 0) {
      await db.insert(workflowTransition).values(
        d.transitions.map((t) => ({
          workflowId: def.id,
          ...
          condition: (t.condition ?? null) as object | null,
        })),
      );
    }
    return { id: def.id };
  ...
  async listDefinitions(): Promise<
    Array<
      typeof workflowDefinition.$inferSelect & {
        states: Array<typeof workflowState.$inferSelect>;
        transitions: Array<typeof workflowTransition.$inferSelect>;
      }
    >
  > {
    const db = this.database.db;
    ...
    return Promise.all(
      defs.map(async (d) => ({
        ...
        transitions: await db
          ...
          .where(eq(workflowTransition.workflowId, d.id)),
      })),
    );
  }
  ...
  async start(
    ...
    const def = (
      await db
        ...
        .where(
          and(
            eq(workflowDefinition.documentType, documentType),
    ...
    if (!def)
      throw new WorkflowNotFoundError(`no active workflow for document type "${documentType}"`);
    const existing = (
      await db
        .select({ id: workflowInstance.id })
        .from(workflowInstance)
        .where(
          and(
            eq(workflowInstance.documentType, documentType),
    ...
    if (existing)
      throw new WorkflowValidationError(`document ${documentId} is already under a workflow`);
    const inst = (
      await db
  ...
  async availableActions(
    ...
    return transitions
      .filter((t) => t.fromState === inst.currentState)
      .map((t) => {
  ...
  async act(
    instanceId: string,
    action: string,
    input: { comment?: string; context?: Record<string, unknown> } = {},
  ): Promise<InstanceView> {
    const { inst, transitions, states } = await this.load(instanceId);
    if (states.find((s) => s.code === inst.currentState)?.isFinal)
      throw new WorkflowValidationError(`المستند في حالة نهائية "${inst.currentState}"`);
    const t = transitions.find((x) => x.fromState === inst.currentState && x.action === action);
    if (!t)
      throw new WorkflowValidationError(
        `الإجراء "${action}" مش متاح من الحالة "${inst.currentState}"`,
      );
    const data = input.context ?? inst.context;
    const block = this.blockReason(t, await this.currentRoles(), data);
    if (block)
      throw block.kind === 'role'
        ? new WorkflowForbiddenError(block.message)
        : new WorkflowValidationError(block.message);
    const db = this.database.db;
    await db
      .update(workflowInstance)
      .set({ currentState: t.toState, context: data, updatedAt: new Date() })
      .where(eq(workflowInstance.id, inst.id));
    await db.insert(workflowHistory).values({
      instanceId: inst.id,
    ...
    this.automation?.dispatch({
      ...
      workflowCode: view.workflowCode,
  ...
  private blockReason(
    t: typeof workflowTransition.$inferSelect,
    roles: string[] | null,
    data: Record<string, unknown>,
  ): { kind: 'role' | 'condition'; message: string } | null {
    const allowed = t.allowedRoles ?? [];
    if (allowed.length > 0) {
      if (roles === null)
        return {
          kind: 'role',
          message: `الإجراء "${t.action}" محتاج تسجيل دخول بدور من: ${allowed.join('، ')}`,
        };
      if (!allowed.some((r) => roles.includes(r)))
        return {
          kind: 'role',
          message: `دورك مش من الأدوار المسموح لها بـ "${t.action}" (${allowed.join('، ')})`,
        };
    }
    if (!evaluateCondition(t.condition as Condition | null, data))
      return {
        kind: 'condition',
        message: `شرط الإجراء "${t.action}" مش متحقق على بيانات المستند`,
      };
    return null;
  }
  ...
  private async currentRoles(): Promise<string[] | null> {
    ...
    if (!user) return null;
    const role = (await this.auth.getRoles()).find(
      (r) => r.id === user.roleId && r.status === 'active',
    );
    return role ? [role.code] : [];
  }

  private async load(instanceId: string): Promise<{
    inst: typeof workflowInstance.$inferSelect;
    transitions: Array<typeof workflowTransition.$inferSelect>;
    states: Array<typeof workflowState.$inferSelect>;
  }> {
    const db = this.database.db;
    const inst = (
      await db.select().from(workflowInstance).where(eq(workflowInstance.id, instanceId)).limit(1)
    )[0];
    ...
    return {
      ...
      transitions: await db
        .select()
        .from(workflowTransition)
        .where(eq(workflowTransition.workflowId, inst.workflowId)),
      states: await db
        .select()
        .from(workflowState)
        .where(eq(workflowState.workflowId, inst.workflowId)),
    };
  }
  ...
  private async view(instanceId: string): Promise<InstanceView> {
    ...
    const def = (
      await db
        .select()
        .from(workflowDefinition)
        .where(eq(workflowDefinition.id, inst.workflowId))
        .limit(1)
    )[0]!;
    const history = await db
      .select()
      .from(workflowHistory)
      .where(eq(workflowHistory.instanceId, inst.id))
    ...
    return {
      ...
      isFinal: states.find((s) => s.code === inst.currentState)?.isFinal ?? false,

...
export function validateCondition(c: unknown, depth = 0): asserts c is Condition {
  ...
  if ('all' in o || 'any' in o) {
    ...
    if (!Array.isArray(list) || list.length === 0)
      throw new Error('"all" / "any" need a non-empty list');
    list.forEach((x) => validateCondition(x, depth + 1));
    return;
  }
  if ('not' in o) {
    validateCondition(o.not, depth + 1);
    return;
  }
  ...
  if ((o.op === 'in' || o.op === 'nin') && !Array.isArray(o.value))
    throw new Error(`"${String(o.op)}" needs a list value`);
}

export function evaluateCondition(
  c: Condition | null | undefined,
  ...
  switch (c.op) {
    ...
    case 'eq':
      return actual === c.value || (num(actual) !== null && num(actual) === num(c.value));
    case 'ne':
      return !(actual === c.value || (num(actual) !== null && num(actual) === num(c.value)));
    case 'in':
    ...
    case 'contains':
      return Array.isArray(actual)
        ...
        : typeof actual === 'string' && actual.includes(String(c.value));
    default: {
      const a = num(actual);
      const b = num(c.value);

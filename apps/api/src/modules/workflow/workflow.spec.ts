...
describe('Workflow engine (plan item 42)', () => {
  ...
  it('1. conditions: comparisons, nesting, dotted paths, numbers as strings', () => {
    ...
    expect(evaluateCondition({ field: 'customer.segment', op: 'in', value: ['gov', 'ngo'] }, data)).toBe(true);
    expect(evaluateCondition({ all: [{ field: 'type', op: 'eq', value: 'capex' }, { not: { field: 'amount', op: 'lte', value: 20000 } }] }, data)).toBe(false);
    expect(evaluateCondition({ any: [{ field: 'missing', op: 'exists' }, { field: 'tags', op: 'contains', value: 'urgent' }] }, data)).toBe(true);
  ...
  const def = (over: Partial<DefinitionInput> = {}): DefinitionInput => ({
    code: 'PR', name: 'Purchase request', documentType: 'purchase_request', initialState: 'draft',
    states: [{ code: 'draft', name: 'مسودة' }, { code: 'pending', name: 'مستني' }, { code: 'approved', name: 'معتمد', isFinal: true }, { code: 'rejected', name: 'مرفوض', isFinal: true }],
    transitions: [
  ...
  it('3. a sound definition passes; broken graphs are refused', () => {
    ...
    expect(() => validateDefinition(def({ transitions: def().transitions.slice(0, 1) }))).toThrow(/dead end/);
    expect(() => validateDefinition(def({ transitions: [...def().transitions, { fromState: 'approved', toState: 'draft', action: 'reopen' }] }))).toThrow(/final state/);
    expect(() => validateDefinition(def({ transitions: [...def().transitions, { fromState: 'draft', toState: 'rejected', action: 'submit' }] }))).toThrow(/twice/);
  });
});

...
describe('RfqService — request for quotation to several suppliers (plan item 7)', () => {
  ...
  let sales: { createQuotation: jest.Mock; getQuotation: jest.Mock; approveQuotation: jest.Mock; rejectQuotation: jest.Mock };

  const input: CreateRfqInput = {
    lines: [{ itemId: 'steel', quantity: '10' }, { itemId: 'paint', quantity: '4' }],
  ...
  beforeEach(() => {
    ...
    const repo = {
      ...
      insert: jest.fn().mockImplementation(async (i: CreateRfqInput & { id: string; rfqNumber: string }) => {
        const rec: RfqRecord = {
          id: i.id, rfqNumber: i.rfqNumber, orgNodeId: null, rfqDate: '', respondBy: null, status: 'draft',
          materialRequestReference: null, awardedSupplierId: null, note: null, createdAt: '', updatedAt: '',
          lines: i.lines.map((l, n) => ({ id: `l${n}`, itemId: l.itemId, quantity: l.quantity, lineNumber: n + 1 })),
          suppliers: i.supplierIds.map((s) => ({ id: s, supplierId: s, status: 'pending', quotationId: null, respondedAt: null })),
        };
        store.set(rec.id, rec);
        return rec;
      }),
      setStatus: jest.fn().mockImplementation(async (id: string, status: RfqStatus, awarded?: string) => {
        const r = store.get(id)!;
        r.status = status;
        if (awarded) r.awardedSupplierId = awarded;
      }),
      setSupplierResponse: jest.fn().mockImplementation(async (id: string, sup: string, status: RfqSupplierStatus, q?: string) => {
    ...
    sales = {
      createQuotation: jest.fn().mockImplementation(async (q) => {
        const rec = { id: `q-${q.supplierId}`, status: 'draft', lines: q.lines } as unknown as QuotationRecord;
        quotations.set(rec.id, rec);
        return rec;
      }),
      getQuotation: jest.fn().mockImplementation(async (id: string) => quotations.get(id)),
      approveQuotation: jest.fn().mockImplementation(async (id: string) => { quotations.get(id)!.status = 'approved'; }),
      rejectQuotation: jest.fn().mockImplementation(async (id: string) => { quotations.get(id)!.status = 'rejected'; }),
    };
    const crm = {
  ...
  it('2. answers are only accepted once sent, from invited suppliers, pricing exactly the RFQ items', async () => {
    ...
    const prices = { lines: [{ itemId: 'steel', unitPrice: '100' }, { itemId: 'paint', unitPrice: '50' }] };
    ...
    await expect(service.recordResponse(draft.id, 's-a', { lines: [{ itemId: 'steel', unitPrice: '1' }] })).rejects.toThrow(/exactly the RFQ items/);
    const { quotation } = await service.recordResponse(draft.id, 's-a', prices);
    expect(sales.createQuotation).toHaveBeenCalledWith(expect.objectContaining({ direction: 'incoming', supplierId: 's-a' }));
    expect(quotation.lines).toEqual([
      { itemId: 'steel', quantity: '10', unitPrice: '100' },
      { itemId: 'paint', quantity: '4', unitPrice: '50' },
    ]);
    await expect(service.recordResponse(draft.id, 's-a', prices)).rejects.toThrow(/already answered/);
  });
  ...
  it('3. compares offers side by side and flags the lowest price per line and the lowest total', async () => {
    ...
    await service.recordResponse(r.id, 's-a', { lines: [{ itemId: 'steel', unitPrice: '100' }, { itemId: 'paint', unitPrice: '60' }] });
    await service.recordResponse(r.id, 's-b', { lines: [{ itemId: 'steel', unitPrice: '110' }, { itemId: 'paint', unitPrice: '40' }] });
    ...
    expect(c.suppliers.find((s) => s.supplierId === 's-c')).toMatchObject({ status: 'declined', total: null });
  ...
  it('4. awarding approves the winner, rejects the other answers and closes the RFQ', async () => {
    const r = await createAndSend();
    const p = { lines: [{ itemId: 'steel', unitPrice: '1' }, { itemId: 'paint', unitPrice: '1' }] };

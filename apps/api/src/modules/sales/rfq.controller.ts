...
export class RfqController {
  ...
  @Get()
  async list(): Promise<{ rfqs: RfqRecord[] }> {
    return { rfqs: await this.service.list() };
  }

  @Get(':id')
  async get(@Param('id', ParseUUIDPipe) id: string): Promise<{ rfq: RfqRecord }> {
    return { rfq: await this.service.get(id) };
  }

  @Post()
  @HttpCode(201)
  async create(@Body() dto: CreateRfqDto): Promise<{ rfq: RfqRecord }> {
    return { rfq: await this.service.create(dto) };
  }

  @Post(':id/send')
  @HttpCode(200)
  async send(@Param('id', ParseUUIDPipe) id: string): Promise<{ rfq: RfqRecord }> {
    return { rfq: await this.service.send(id) };
  }

  @Post(':id/suppliers/:supplierId/response')
  @HttpCode(201)
  async respond(
  ...
  @Post(':id/suppliers/:supplierId/decline')
  @HttpCode(200)
  async decline(
    @Param('id', ParseUUIDPipe) id: string,
    @Param('supplierId', ParseUUIDPipe) supplierId: string,

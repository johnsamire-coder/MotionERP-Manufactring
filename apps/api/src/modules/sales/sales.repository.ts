...
const quotationColumns = {
  ...
  orgNodeId: quotation.orgNodeId,
  quotationDate: quotation.quotationDate,
  validUntil: quotation.validUntil,
  status: quotation.status,
  currency: quotation.currency,
  customerPoReference: quotation.customerPoReference,
  note: quotation.note,
...
const lineColumns = {
  ...
  lineNumber: quotationLine.lineNumber,
};
const jobOrderColumns = {
  ...
  financialReviewPassed: jobOrder.financialReviewPassed,
  note: jobOrder.note,
...
function toQuotationRecord(row: QuotationRow, lines: QuotationLineRecord[]): QuotationRecord {
  return {
    ...
    direction: row.direction as QuotationDirection,
...
function toJobOrderRecord(row: JobOrderRow): JobOrderRecord {
  return {
    ...
    source: row.source as JobOrderSource,
    quotationReference: row.quotationReference,
    customerId: row.customerId,
    orgNodeId: row.orgNodeId,
    status: row.status as JobOrderStatus,
    financialReviewPassed: row.financialReviewPassed === 'true',
    note: row.note,
    createdAt: row.createdAt.toISOString(),
    updatedAt: row.updatedAt.toISOString(),
  };
...
export class SalesRepository {
  ...
  async listQuotations(direction?: 'outgoing' | 'incoming'): Promise<QuotationRecord[]> {
    const quotations = direction
      ? await this.database.db
          ...
          .from(quotation)
          .where(eq(quotation.direction, direction))
          .orderBy(asc(quotation.quotationNumber))
      : await this.database.db
          .select(quotationColumns)
          .from(quotation)
          .orderBy(asc(quotation.quotationNumber));
    ...
    return quotations.map((q) =>
      toQuotationRecord(q, allLines.filter((l) => l.quotationId === q.id).map(toLineRecord)),
    );
  }
  ...
  async insertQuotation(
    input: CreateQuotationInput & { id: string; quotationNumber: string },
  ...
  async listJobOrders(): Promise<JobOrderRecord[]> {
    const rows = await this.database.db
      ...
      .from(jobOrder)
      .orderBy(asc(jobOrder.jobOrderNumber));
  ...
  async insertJobOrder(
    input: CreateJobOrderInput & { id: string; jobOrderNumber: string },
  ): Promise<JobOrderRecord> {
    const rows = await this.database.db
      .insert(jobOrder)
      .values({
        id: input.id,
        jobOrderNumber: input.jobOrderNumber,
        source: input.source,
  ...
  async setJobOrderFinancialReview(id: string, passed: boolean): Promise<JobOrderRecord> {
    const rows = await this.database.db
      .update(jobOrder)
      .set({ financialReviewPassed: passed ? 'true' : 'false' })
      .where(eq(jobOrder.id, id))
      .returning(jobOrderColumns);
    return toJobOrderRecord(rows[0]!);
  }

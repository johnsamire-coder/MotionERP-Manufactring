...
export function ruleMatches(
  ...
  if (rule.applyOn === 'item' ? rule.itemId !== itemId : rule.categoryId !== categoryId)
    return false;
  if (qty < Number(rule.minQty) || (rule.maxQty !== null && qty > Number(rule.maxQty)))
    return false;
...
export function applyPricingRules(
  ...
  lines.forEach((line, index) => {
      ...
      matching
        ...
        .sort(
          (a, b) =>
            ...
            (type === 'price' ? priceAfter(a, line.baseRate) - priceAfter(b, line.baseRate) : 0),
        )[0];
    ...
    priced.push({
      ...
      baseRate: line.baseRate.toFixed(4),
      rate: rate.toFixed(4),
      discountPerUnit: (line.baseRate - rate).toFixed(4),
      appliedRule: priceRule?.code ?? null,
    });
    ...
    if (productRule) {
      ...
      if (freeQty > 0) {
        free.push({
          itemId: productRule.freeItemId ?? line.itemId,
          quantity: freeQty.toFixed(6),
          rate: '0',
          forLine: index,
          appliedRule: productRule.code,
        });
      }
    }
  });

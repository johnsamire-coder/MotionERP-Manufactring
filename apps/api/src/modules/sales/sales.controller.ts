...
export class SalesController {
  ...
  ): Promise<{ creditStatus: CustomerCreditStatus }> {
    return { creditStatus: await this.credit.getStatus(id) };
  }

  @Get('quotations')
  async quotations(
    @Query('direction') direction?: 'outgoing' | 'incoming',
  ): Promise<{ quotations: QuotationRecord[] }> {
  ...
  @Get('job-orders')
  async jobOrders(): Promise<{ jobOrders: JobOrderRecord[] }> {
    return { jobOrders: await this.service.getJobOrders() };
  }

  @Get('job-orders/:id')
  async jobOrderById(
    @Param('id', ParseUUIDPipe) id: string,
  ): Promise<{ jobOrder: JobOrderRecord }> {
  ...
  @Post('job-orders/:id/financial-review/pass')
  @HttpCode(200)
  async passFinancialReview(
    @Param('id', ParseUUIDPipe) id: string,
  ): Promise<{ jobOrder: JobOrderRecord }> {
  ...
  @HttpCode(200)
  async approveJobOrder(
    @Param('id', ParseUUIDPipe) id: string,
  ): Promise<{ jobOrder: JobOrderRecord; creditWarning?: string }> {

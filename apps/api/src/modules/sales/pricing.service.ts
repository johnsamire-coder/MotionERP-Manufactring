...
export class PricingService {
  ...
  async create(input: CreatePricingRuleInput): Promise<PricingRuleRecord> {
    ...
    if (!code || !input.title?.trim())
      throw new SalesValidationError('code and title are required');
    if (await this.repository.findByCode(code))
      throw new SalesValidationError(`pricing rule "${code}" already exists`);
    ...
    if (input.ruleType === 'price') {
      const given = [input.discountPercentage, input.discountAmount, input.rate].filter(
        (v) => v !== null && v !== undefined,
      );
      if (given.length !== 1)
        throw new SalesValidationError(
          'a price rule needs exactly one of discountPercentage, discountAmount, rate',
        );
      const pct = input.discountPercentage;
      if (pct !== null && pct !== undefined && (Number(pct) <= 0 || Number(pct) > 100))
        throw new SalesValidationError('discountPercentage must be in (0, 100]');
    } else {
      if (!input.freeQty || Number(input.freeQty) <= 0)
        throw new SalesValidationError('a product rule needs a positive freeQty');
      if (input.recursive && !(Number(input.minQty ?? 0) > 0))
        throw new SalesValidationError('a recursive product rule needs minQty > 0');
      if (input.freeItemId) await this.itemExists(input.freeItemId);
    }
    if (input.maxQty && Number(input.maxQty) < Number(input.minQty ?? 0))
      throw new SalesValidationError('maxQty is below minQty');
  ...
  async setStatus(id: string, status: 'active' | 'disabled'): Promise<PricingRuleRecord> {
    if (!(await this.repository.findById(id)))
      throw new SalesNotFoundError(`pricing rule ${id} does not exist`);
  ...
  async apply(
    ...
    for (const l of lines) {
      ...
      let base: number | null =
        l.unitPrice !== undefined && l.unitPrice !== '' ? Number(l.unitPrice) : null;
      if (base === null) base = await this.repository.listPrice(l.itemId, appliesTo, at);
      if (base === null || !Number.isFinite(base))
        throw new SalesValidationError(
          `no ${appliesTo} price for item ${item.code}; pass unitPrice or add a price list entry`,
        );
      prepared.push({
        itemId: l.itemId,
    ...
    return applyPricingRules(prepared, await this.repository.listActive(appliesTo), {
      appliesTo,
      partyId,
      date: at,
    });
  ...
  private async itemExists(itemId: string): Promise<{ code: string; categoryId: string }> {
    try {
      ...
      if (err instanceof CatalogNotFoundError)
        throw new SalesNotFoundError(`item ${itemId} does not exist`);
      throw err;
    }
  }
}

...
function toRecord(r: typeof pricingRule.$inferSelect): PricingRuleRecord {
  return {
    ...
    code: r.code,
    title: r.title,
    appliesTo: r.appliesTo as PricingAppliesTo,
    applyOn: r.applyOn as PricingRuleRecord['applyOn'],
    itemId: r.itemId,
    ...
    priority: r.priority,
    ruleType: r.ruleType as PricingRuleRecord['ruleType'],
    ...
    status: r.status as PricingRuleRecord['status'],
  };
}
...
export class PricingRepository {
  ...
  async list(): Promise<PricingRuleRecord[]> {
    return (
      await this.database.db
        .select()
        .from(pricingRule)
        .orderBy(desc(pricingRule.priority), asc(pricingRule.code))
    ).map(toRecord);
  }
  async listActive(appliesTo: PricingAppliesTo): Promise<PricingRuleRecord[]> {
    return (
      await this.database.db
        .select()
        .from(pricingRule)
        .where(and(eq(pricingRule.appliesTo, appliesTo), eq(pricingRule.status, 'active')))
    ).map(toRecord);
  }
  async findByCode(code: string): Promise<PricingRuleRecord | null> {
    const rows = await this.database.db
      .select()
      .from(pricingRule)
      .where(eq(pricingRule.code, code))
      .limit(1);
  ...
  async listPrice(itemId: string, type: PricingAppliesTo, at: Date): Promise<number | null> {
    const rows = await this.database.db
      .select({ price: itemPrice.price })
      .from(itemPrice)
      .where(
        and(
          eq(itemPrice.itemId, itemId),
          eq(itemPrice.priceListType, type),
          lte(itemPrice.validFrom, at),
          or(isNull(itemPrice.validUntil), gte(itemPrice.validUntil, at)),
        ),
      )
      .orderBy(desc(itemPrice.validFrom))
      .limit(1);

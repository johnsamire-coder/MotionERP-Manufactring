...
import type {
  CreateInspectionFromTemplateInput, CreateInspectionTemplateInput, InspectionTemplateRecord,
  InspectionWithReadings, ParameterCriteria, ReadingRecord, RecordReadingsInput, TemplateParameterInput,
...
function describeCriteria(p: TemplateParameterInput): string {
  ...
  if (p.minValue !== undefined && p.maxValue !== undefined) return `${fmt(p.minValue)} – ${fmt(p.maxValue)}`;
...
export class QualityReadingsService {
  ...
  async createTemplate(input: CreateInspectionTemplateInput): Promise<InspectionTemplateRecord> {
    ...
    if (!input.parameters || input.parameters.length === 0) throw new QualityValidationError('a template needs at least one parameter');
    if (await this.repository.findTemplateByCode(code)) throw new QualityValidationError(`template "${code}" already exists`);
    ...
    for (const p of input.parameters) {
      ...
      if (!Number.isInteger(readings) || readings < 1 || readings > 10) throw new QualityValidationError('readingsRequired must be 1–10');
      if (p.isNumeric) {
        if (p.minValue === undefined && p.maxValue === undefined) throw new QualityValidationError(`numeric parameter "${name}" needs minValue and/or maxValue`);
        for (const v of [p.minValue, p.maxValue]) {
          if (v !== undefined && !Number.isFinite(Number(v))) throw new QualityValidationError(`limits of "${name}" must be numbers`);
        }
        if (p.minValue !== undefined && p.maxValue !== undefined && Number(p.minValue) > Number(p.maxValue)) {
    ...
    return this.repository.insertTemplate({
      ...input, code, name: input.name.trim(),
      parameters: input.parameters.map((p) => ({ ...p, parameterName: p.parameterName.trim() })),
    });
  }

  async listTemplates(): Promise<InspectionTemplateRecord[]> { return this.repository.listTemplates(); }
  ...
  async createInspectionFromTemplate(input: CreateInspectionFromTemplateInput): Promise<InspectionWithReadings> {
    const template = await this.getTemplate(input.templateId);
    const itemId = input.itemId ?? template.itemId;
    if (!itemId) throw new QualityValidationError('itemId is required (the template is not tied to an item)');
    ...
    const inspection = await this.quality.createQualityInspection({
      orgNodeId: input.orgNodeId, itemId, referenceType: input.referenceType, referenceId: input.referenceId,
      notes: input.notes ?? `قالب ${template.code}`,
      parameters: template.parameters.map((p) => ({
        parameterName: p.parameterName,
        targetValue: describeCriteria({
          parameterName: p.parameterName, isNumeric: p.isNumeric, minValue: p.minValue ?? undefined,
          maxValue: p.maxValue ?? undefined, acceptedValue: p.acceptedValue ?? undefined,
        }),
      })),
    });
    for (const p of template.parameters) {
      await this.repository.setParameterCriteria(inspection.id, p.parameterName, {
        isNumeric: p.isNumeric, minValue: p.minValue, maxValue: p.maxValue, acceptedValue: p.acceptedValue, readingsRequired: p.readingsRequired,
      });
    }
  ...
  async getInspection(id: string): Promise<InspectionWithReadings> {
    ...
    return {
      ...
      parameters: inspection.parameters.map((p) => ({
        ...p,
        ...(criteria.get(p.id) ?? { isNumeric: false, minValue: null, maxValue: null, acceptedValue: null, readingsRequired: 1 }),
  ...
  async recordReadings(id: string, input: RecordReadingsInput): Promise<InspectionWithReadings> {
    ...
    if (inspection.status !== 'pending') throw new QualityValidationError(`inspection ${inspection.inspectionNumber} is already "${inspection.status}"`);
    const byParam = new Map((input.readings ?? []).map((r) => [r.parameterId, r.values ?? []]));
    const unknown = [...byParam.keys()].filter((pid) => !inspection.parameters.some((p) => p.id === pid));
    if (unknown.length > 0) throw new QualityValidationError(`unknown parameter(s): ${unknown.join(', ')}`);

    const results: Array<{ parameterId: string; actualValue: string; status: 'pass' | 'fail'; readings: ReadingRecord[] }> = [];
    for (const p of inspection.parameters) {
      const values = (byParam.get(p.id) ?? []).map((v) => String(v).trim()).filter((v) => v.length > 0);
      if (values.length < p.readingsRequired) {
        throw new QualityValidationError(`"${p.parameterName}" needs ${p.readingsRequired} reading(s), got ${values.length}`);
      }
      if (values.length > 10) throw new QualityValidationError(`"${p.parameterName}" accepts at most 10 readings`);
      if (p.isNumeric) {
        const bad = values.find((v) => !Number.isFinite(Number(v)));
        if (bad !== undefined) throw new QualityValidationError(`"${p.parameterName}" is numeric; "${bad}" is not a number`);
      }
      const readings = values.map((value, i) => ({ readingNo: i + 1, value, withinSpec: readingWithinSpec(p, value) }));
      const actualValue = p.isNumeric
        ? `${values.join(', ')} (متوسط ${(values.reduce((a, v) => a + Number(v), 0) / values.length).toFixed(4)})`
        : values.join(', ');
      results.push({ parameterId: p.id, actualValue, status: readings.every((r) => r.withinSpec) ? 'pass' : 'fail', readings });
    }
    ...
    await this.qualityRepository.updateInspectionStatus(
      id, status, requestContext.currentUserId() ?? SYSTEM_USER_ID, input.notes,
      results.map(({ parameterId, actualValue, status: s }) => ({ parameterId, actualValue, status: s })),
    );
  ...
  private async assertItemExists(itemId: string): Promise<void> {
    ...
    try {
      ...
      if (err instanceof CatalogNotFoundError) throw new QualityNotFoundError(`item ${itemId} does not exist`);
      throw err;
    }
  }
}

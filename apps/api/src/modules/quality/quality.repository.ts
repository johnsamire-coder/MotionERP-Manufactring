...
const cpColumns = {
  ...
  orgNodeId: qualityCheckPoint.orgNodeId, name: qualityCheckPoint.name, targetDurationMinutes: qualityCheckPoint.targetDurationMinutes,
  gracePeriodMinutes: qualityCheckPoint.gracePeriodMinutes, assignedRoleId: qualityCheckPoint.assignedRoleId
};
...
const qiColumns = {
  id: qualityInspection.id, inspectionNumber: qualityInspection.inspectionNumber, orgNodeId: qualityInspection.orgNodeId,
  itemId: qualityInspection.itemId, referenceType: qualityInspection.referenceType, referenceId: qualityInspection.referenceId,
  status: qualityInspection.status, inspectedBy: qualityInspection.inspectedBy, inspectedAt: qualityInspection.inspectedAt,
  notes: qualityInspection.notes, createdAt: qualityInspection.createdAt, updatedAt: qualityInspection.updatedAt,
};

const qipColumns = {
  id: qualityInspectionParameter.id, inspectionId: qualityInspectionParameter.inspectionId,
  parameterName: qualityInspectionParameter.parameterName, targetValue: qualityInspectionParameter.targetValue,
  actualValue: qualityInspectionParameter.actualValue, status: qualityInspectionParameter.status,
...
export class QualityRepository {
  ...
  async findCheckPointById(id: string): Promise<QualityCheckPointRecord | null> {
    ...
    return rows[0] ? {
      id: rows[0].id, relatedEntityType: rows[0].relatedEntityType as any, relatedEntityId: rows[0].relatedEntityId,
      orgNodeId: rows[0].orgNodeId, name: rows[0].name, targetDurationMinutes: rows[0].targetDurationMinutes,
      gracePeriodMinutes: rows[0].gracePeriodMinutes, assignedRoleId: rows[0].assignedRoleId,
    } : null;
  }
  ...
  async insertCheckPoint(input: CreateQualityCheckPointInput & { id: string; orgNodeId: string | null }): Promise<QualityCheckPointRecord> {
    ...
    const r = rows[0]!;
    return {
      id: r.id, relatedEntityType: r.relatedEntityType as any, relatedEntityId: r.relatedEntityId,
      orgNodeId: r.orgNodeId, name: r.name, targetDurationMinutes: r.targetDurationMinutes,
      gracePeriodMinutes: r.gracePeriodMinutes, assignedRoleId: r.assignedRoleId,
    };
  }
  ...
  async updateWorkflowStatus(id: string, status: QualityWorkflowStatus, actionTakenById: string, resultNote?: string): Promise<QualityWorkflowRecord> {
  ...
  async countInspections(): Promise<number> {
    const rows = await this.database.db.select({ id: qualityInspection.id }).from(qualityInspection);
    return rows.length;
  }

  async insertInspection(input: CreateQualityInspectionInput & { id: string; inspectionNumber: string }): Promise<QualityInspectionRecord> {
    const rows = await this.database.db.insert(qualityInspection).values({
      id: input.id,
    ...
    for (const p of input.parameters) {
      const pRows = await this.database.db.insert(qualityInspectionParameter).values({
    ...
    return {
      id: r.id, inspectionNumber: r.inspectionNumber, orgNodeId: r.orgNodeId, itemId: r.itemId,
      referenceType: r.referenceType as any, referenceId: r.referenceId, status: r.status as any,
      inspectedBy: r.inspectedBy, inspectedAt: r.inspectedAt ? r.inspectedAt.toISOString() : null,
  ...
  async findInspectionById(id: string): Promise<QualityInspectionRecord | null> {
    const rows = await this.database.db.select(qiColumns).from(qualityInspection).where(eq(qualityInspection.id, id)).limit(1);
    if (!rows[0]) return null;
    const r = rows[0];

    const params = await this.database.db.select(qipColumns).from(qualityInspectionParameter).where(eq(qualityInspectionParameter.inspectionId, r.id));
    ...
    return {
      ...
      parameters: params.map((p) => ({
        id: p.id, inspectionId: p.inspectionId, parameterName: p.parameterName, targetValue: p.targetValue,
        actualValue: p.actualValue, status: p.status as any, createdAt: p.createdAt.toISOString(),
      })),
    };
  }
  ...
  async updateInspectionStatus(
    ...
    if (paramResults) {
      for (const pr of paramResults) {
        await this.database.db.update(qualityInspectionParameter)
          .set({ actualValue: pr.actualValue, status: pr.status })
          .where(eq(qualityInspectionParameter.id, pr.parameterId));
      }
    }

    await this.database.db.update(qualityInspection)
      .set({ status, inspectedBy, inspectedAt: new Date(), notes: notes ?? null, updatedAt: new Date() })

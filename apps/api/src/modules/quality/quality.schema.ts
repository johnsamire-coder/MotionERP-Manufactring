...
export const qualityCheckPoint = qualitySchema.table(
  'check_point',
  {
  ...
  (t) => [
    check(
      'valid_related_entity_type',
      sql`${t.relatedEntityType} IN ('production_step', 'material_request')`,
    ),
...
export const qualityWorkflow = qualitySchema.table(
  'workflow',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const qualityInspection = qualitySchema.table(
  'inspection',
  {
    id: uuid('id').primaryKey().defaultRandom(),
  ...
  (t) => [
    ...
    check(
      ...
      sql`${t.referenceType} IN ('purchase_receipt', 'production_step', 'delivery_order')`,
    ),
...
export const qualityInspectionParameter = qualitySchema.table(
...
export const inspectionTemplate = qualitySchema.table(
  'inspection_template',
  {
    id: uuid('id').primaryKey().defaultRandom(),
 ...
 */
export const inspectionTemplateParameter = qualitySchema.table(
  'inspection_template_parameter',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    templateId: uuid('template_id')
      .notNull()
      .references(() => inspectionTemplate.id, { onDelete: 'cascade' }),
    parameterName: text('parameter_name').notNull(),
  ...
  (t) => [
    ...
    check(
      'template_parameter_readings_required_range',
      sql`${t.readingsRequired} between 1 and 10`,
    ),
    check(
      'template_parameter_criteria',
      sql`(${t.isNumeric} and (${t.minValue} is not null or ${t.maxValue} is not null)) or (not ${t.isNumeric} and ${t.acceptedValue} is not null)`,
    ),
    check(
      'template_parameter_min_le_max',
...
export const inspectionReading = qualitySchema.table(
  'inspection_reading',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    parameterId: uuid('parameter_id')
      .notNull()
      .references(() => qualityInspectionParameter.id, { onDelete: 'cascade' }),

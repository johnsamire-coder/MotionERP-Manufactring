...
import {
  CreateInspectionFromTemplateDto,
  CreateInspectionTemplateDto,
  RecordReadingsDto,
} from './quality-readings.dto';
...
export class QualityReadingsController {
  ...
  async templates(): Promise<{ templates: InspectionTemplateRecord[] }> {
    return { templates: await this.service.listTemplates() };
  }

  @Get('inspection-templates/:id')
  async template(
    @Param('id', ParseUUIDPipe) id: string,
  ): Promise<{ template: InspectionTemplateRecord }> {
  ...
  async createInspection(
    @Body() dto: CreateInspectionFromTemplateDto,
  ): Promise<{ inspection: InspectionWithReadings }> {
  ...
  @Post('inspections/:id/readings')
  @HttpCode(200)
  async readings(
    @Param('id', ParseUUIDPipe) id: string,
    @Body() dto: RecordReadingsDto,
  ): Promise<{ inspection: InspectionWithReadings }> {

...
import type { QualityCheckPointType } from './quality.types';
...
export class QualityController {
  ...
  ): Promise<{ checkPoint: QualityCheckPointRecord }> {
    const created = await this.service.createCheckPoint({
      relatedEntityType: dto.relatedEntityType as QualityCheckPointType,
  ...
  async approveCheckPoint(
    ...
    const workflow = await this.service.approveCheckPoint(
      workflowId,
  ...
  async rejectCheckPoint(
    ...
    const workflow = await this.service.rejectCheckPoint(
      workflowId,
      '00000000-0000-0000-0000-000000000000',
      dto.resultNote,
    );

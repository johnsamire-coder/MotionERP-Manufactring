...
describe('Quality inspection with real readings (plan item 9)', () => {
  ...
  describe('templates and readings', () => {
    ...
    let updated: {
      status?: string;
      results?: Array<{ parameterId: string; status: string; actualValue: string }>;
    };
    ...
    beforeEach(() => {
      ...
      updated = {};
      const repo = {
        findTemplateByCode: jest
          .fn()
          .mockImplementation(
            async (code: string) => [...templates.values()].find((t) => t.code === code) ?? null,
          ),
        ...
        findTemplateById: jest
          .fn()
          .mockImplementation(async (id: string) => templates.get(id) ?? null),
        setParameterCriteria: jest
          .fn()
          .mockImplementation(async (_i: string, name: string, c: ParameterCriteria) => {
            const p = inspection.parameters.find((x) => x.parameterName === name)!;
            criteria.set(p.id, c);
          }),
        listParameterCriteria: jest.fn().mockImplementation(async () => criteria),
        insertReadings: jest.fn().mockImplementation(async (pid: string, r: ReadingRecord[]) => {
          readings.set(pid, r);
        }),
      ...
      const quality = {
        createQualityInspection: jest.fn().mockImplementation(async (i) => {
          inspection = {
            id: 'qi-1',
            inspectionNumber: 'QINSP-1',
            orgNodeId: i.orgNodeId,
            itemId: i.itemId,
            ...
            parameters: i.parameters.map(
              (p: { parameterName: string; targetValue: string }, n: number) => ({
                id: `p-${n}`,
                inspectionId: 'qi-1',
                parameterName: p.parameterName,
      ...
      const qualityRepo = {
        updateInspectionStatus: jest
          .fn()
          .mockImplementation(
            async (_id: string, status: string, _by: string, _n: string, results) => {
    ...
    const template = (): Promise<InspectionTemplateRecord> =>
    ...
    it('2. validates templates', async () => {
      await expect(
        service.createTemplate({
          code: 'X',
          name: 'x',
          parameters: [{ parameterName: 'a', isNumeric: true }],
        }),
      ...
      await expect(
        service.createTemplate({
          code: 'X',
          name: 'x',
          parameters: [{ parameterName: 'a', isNumeric: false }],
        }),
      ...
      await expect(
        service.createTemplate({
          code: 'X',
          name: 'x',
          parameters: [{ parameterName: 'a', isNumeric: true, minValue: '5', maxValue: '1' }],
        }),
    ...
    it('3. an inspection copies the template criteria; all readings in spec → passed (computed, not typed)', async () => {
      ...
      const qi = await service.createInspectionFromTemplate({
        templateId: t.id,
        orgNodeId: 'o',
        referenceType: 'purchase_receipt',
        referenceId: 'r',
      });
      expect(qi.parameters[0]).toMatchObject({
        targetValue: '1.15 – 1.25',
        readingsRequired: 3,
        isNumeric: true,
      });
      const done = await service.recordReadings(qi.id, {
        readings: [
    ...
    it('5. rejects missing readings, non-numbers and re-evaluation', async () => {
      ...
      await expect(
        service.recordReadings(qi.id, {
          readings: [
            { parameterId: 'p-0', values: ['1.2'] },
            { parameterId: 'p-1', values: ['سليم'] },
          ],
        }),
      ...
      await expect(
        service.recordReadings(qi.id, {
          readings: [
            { parameterId: 'p-0', values: ['1.2', 'x', '1.2'] },
            { parameterId: 'p-1', values: ['سليم'] },
          ],
        }),
      ).rejects.toThrow(/not a number/);
      await service.recordReadings(qi.id, {
        readings: [
          { parameterId: 'p-0', values: ['1.2', '1.2', '1.2'] },
          { parameterId: 'p-1', values: ['سليم'] },
        ],
      });
      await expect(service.recordReadings(qi.id, { readings: [] })).rejects.toThrow(
        /already "passed"/,
      );
    });
  });

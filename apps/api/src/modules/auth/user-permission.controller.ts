import { Body, Controller, Delete, Get, HttpCode, Param, ParseUUIDPipe, Post, Query, UseFilters } from '@nestjs/common';
...
export class UserPermissionController {
  ...
  @Get()
  async list(@Query('userId') userId?: string): Promise<{ userPermissions: UserPermissionRecord[] }> {
    return { userPermissions: await this.service.list(userId) };
  }

  @Post() @HttpCode(201)
  async add(@Body() dto: CreateUserPermissionDto): Promise<{ userPermission: UserPermissionRecord }> {

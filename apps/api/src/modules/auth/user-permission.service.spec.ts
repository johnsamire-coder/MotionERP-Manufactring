...
describe('UserPermissionService (plan item 5.1)', () => {
  ...
  beforeEach(() => {
    rows = [];
    const repo = {
      findUserById: jest.fn().mockImplementation(async (id: string) => (id === 'u-1' ? { id } : null)),
      ...
      findUserPermission: jest.fn().mockImplementation(async (u: string, t: string, v: string) =>
      ...
      listUserPermissions: jest.fn().mockImplementation(async (u?: string) => rows.filter((r) => !u || r.userId === u)),
      findUserPermissionById: jest.fn().mockImplementation(async (id: string) => rows.find((r) => r.id === id) ?? null),
      deleteUserPermission: jest.fn().mockImplementation(async (id: string) => { rows = rows.filter((r) => r.id !== id); }),
  ...
  it('2. rejects unknown user, unknown value, bad type and duplicates', async () => {
    await expect(service.add({ userId: 'u-x', allowType: 'warehouse', allowValue: 'wh-1' })).rejects.toThrow(AuthNotFoundError);
    await expect(service.add({ userId: 'u-1', allowType: 'warehouse', allowValue: 'nope' })).rejects.toThrow(/warehouse nope does not exist/);
    await expect(service.add({ userId: 'u-1', allowType: 'org_node', allowValue: 'nope' })).rejects.toThrow(/org node nope does not exist/);
    await expect(service.add({ userId: 'u-1', allowType: 'company' as never, allowValue: 'org-1' })).rejects.toThrow(AuthValidationError);
    await service.add({ userId: 'u-1', allowType: 'warehouse', allowValue: 'wh-1' });
    await expect(service.add({ userId: 'u-1', allowType: 'warehouse', allowValue: 'wh-1' })).rejects.toThrow(/already exists/);
  });
  ...
  it('3. getRestrictions: null means unrestricted on that dimension', async () => {
    ...
    expect(await service.getRestrictions('u-1')).toEqual({ orgNodeIds: null, warehouseIds: ['wh-1', 'wh-2'] });
  });

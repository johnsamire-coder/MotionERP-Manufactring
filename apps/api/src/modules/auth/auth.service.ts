...
export class AuthService {
  ...
  async deactivateUsersForEmployee(employeeRefs: string[]): Promise<string[]> {
    const users = await this.repository.listActiveUsersByEmployeeReference(
      employeeRefs.filter((r) => r.length > 0),
    );
  ...
  async createUser(input: CreateUserInput): Promise<UserRecord> {
    ...
    if (!/^[a-z0-9][a-z0-9._-]{2,63}$/.test(username))
      throw new AuthValidationError('invalid username format');
    ...
    if (existing)
      throw new AuthValidationError(`a user with username "${username}" already exists`);
  ...
  async login(username: string, password: string): Promise<UserRecord> {
    const found = await this.repository.findUserWithPasswordByUsername(
      username.trim().toLowerCase(),
    );
    if (!found) throw new AuthValidationError('invalid username or password');
    if (found.status !== 'active')
      throw new AuthValidationError(`user account is "${found.status}"`);
    ...
    return {
      ...
      roleId: found.roleId,
      employeeReference: found.employeeReference,
      status: found.status,
    };
  }
  ...
  async checkPermission(
    ...
    const permission = await this.repository.findMatchingPermission(
      user.roleId,
      action,
      resource,
      scopeOrgNodeId,
...
function normalizeCode(raw: unknown): string {
  ...
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/.test(trimmed))
    throw new AuthValidationError('code must match ^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$');
  return trimmed;
}

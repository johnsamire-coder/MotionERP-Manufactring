...
const permColumns = {
  ...
  scopeOrgNodeId: permission.scopeOrgNodeId,
...
function toRoleRecord(row: RoleRow): RoleRecord {
  return { id: row.id, code: row.code, name: row.name, status: row.status as RoleStatus };
}
function toPermRecord(row: PermRow): PermissionRecord {
  return {
    ...
    scopeOrgNodeId: row.scopeOrgNodeId,
...
function toUserPermissionRecord(row: typeof userPermission.$inferSelect): UserPermissionRecord {
  return {
    id: row.id,
    userId: row.userId,
    allowType: row.allowType as UserPermissionAllowType,
    allowValue: row.allowValue,
...
export class AuthRepository {
  ...
  async insertPermission(input: CreatePermissionInput & { id: string }): Promise<PermissionRecord> {
    const rows = await this.database.db
      ...
      .values({
        ...
        scopeOrgNodeId: input.scopeOrgNodeId ?? null,
  ...
  async findUserWithPasswordByUsername(username: string): Promise<UserWithPasswordRow | null> {
    const rows = await this.database.db
      .select(userWithPasswordColumns)
      .from(user)
      .where(eq(user.username, username))
      .limit(1);
    return rows[0] ?? null;
  ...
  async listUserPermissions(userId?: string): Promise<UserPermissionRecord[]> {
    ...
    const rows = userId
      ? await query
          ...
          .orderBy(asc(userPermission.allowType), asc(userPermission.createdAt))
      : await query.orderBy(
          asc(userPermission.userId),
          asc(userPermission.allowType),
          asc(userPermission.createdAt),
        );
    ...
    const rows = await this.database.db
      ...
      .where(
        and(
          ...
          eq(userPermission.allowType, allowType),
          eq(userPermission.allowValue, allowValue),
        ),
      )
      .limit(1);
  ...
  async insertUserPermission(
    input: CreateUserPermissionInput & { id: string },

...
export const holidayList = supportSchema.table(
  'holiday_list',
  {
...
export const holiday = supportSchema.table(
  'holiday',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    holidayListId: uuid('holiday_list_id')
      .notNull()
      .references(() => holidayList.id, { onDelete: 'cascade' }),
...
export const serviceLevelAgreement = supportSchema.table(
  'service_level_agreement',
  {
    ...
    workingHours: jsonb('working_hours')
      .$type<Array<{ weekday: number; start: string; end: string }>>()
      .notNull(),
    holidayListId: uuid('holiday_list_id').references(() => holidayList.id, {
      onDelete: 'set null',
    }),
    /** { low|medium|high|urgent: { responseMinutes, resolutionMinutes } } — working minutes. */
    priorities: jsonb('priorities')
      .$type<Record<string, { responseMinutes: number; resolutionMinutes: number }>>()
      .notNull(),
...
export const ticket = supportSchema.table(
  'ticket',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    parentTicketId: uuid('parent_ticket_id').references((): AnyPgColumn => ticket.id, {
      onDelete: 'set null',
    }),
  ...
  (t) => [
    ...
    check(
      'ticket_status_valid',
      sql`${t.status} in ('open', 'replied', 'on_hold', 'resolved', 'closed')`,
    ),
    check(
      'ticket_response_status_valid',
      sql`${t.responseStatus} in ('pending', 'fulfilled', 'failed')`,
    ),
    check(
      'ticket_resolution_status_valid',
      sql`${t.resolutionStatus} in ('pending', 'fulfilled', 'failed')`,
    ),
...
export const ticketComment = supportSchema.table(
  'ticket_comment',
  {
    id: uuid('id').primaryKey().defaultRandom(),

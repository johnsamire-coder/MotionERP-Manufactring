...
const hm = (s: string): [number, number] => {
...
function dayIntervals(at: Date, cal: Calendar): Array<[number, number]> {
  ...
  return cal.windows
    ...
    .map((w) => {
      ...
      return [
        zonedTime(y, m, d, sh, sm, cal.timeZone).getTime(),
        zonedTime(y, m, d, eh, em, cal.timeZone).getTime(),
      ] as [number, number];
    })
    .filter(([s, e]) => e > s)
    .sort((a, b) => a[0] - b[0]);
...
export function workingMinutesBetween(from: Date, to: Date, cal: Calendar): number {
  ...
  for (let guard = 0; guard < 3660 && cursor < to.getTime(); guard++) {
    for (const [s, e] of dayIntervals(new Date(cursor), cal)) {
      const a = Math.max(s, cursor);
      const b = Math.min(e, to.getTime());
      if (b > a) total += b - a;
    }
    const { y, m, d } = localParts(new Date(cursor), cal.timeZone);
    const next = localParts(
      new Date(zonedTime(y, m, d, 0, 0, cal.timeZone).getTime() + DAY + 60 * MIN),
      cal.timeZone,
    );
    cursor = zonedTime(next.y, next.m, next.d, 0, 0, cal.timeZone).getTime();
  }

...
export interface SlaInput {
  ...
  name: string;
  timeZone?: string;
  workingHours: Array<{ weekday: number; start: string; end: string }>;
  holidayListId?: string | null;
  priorities: Record<string, { responseMinutes: number; resolutionMinutes: number }>;
...
export class SupportService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    ...
    if (minutes > 0) {
      this.timer = setInterval(() => {
        this.sweep().catch((e: unknown) => this.logger.error(`support sweep failed: ${String(e)}`));
      }, minutes * 60_000);
      this.timer.unref();
    }
  }
  ...
  async createHolidayList(
    ...
    const list = (
      await this.database.db.insert(holidayList).values({ name: name.trim() }).returning()
    )[0]!;
    if (dates.length > 0)
      await this.database.db.insert(holiday).values(
        dates.map((d) => ({
          holidayListId: list.id,
          holidayDate: d.date,
  ...
  async createSla(input: SlaInput): Promise<SlaRow> {
    if (input.workingHours.length === 0)
      throw new SupportValidationError('working hours are required');
    for (const w of input.workingHours) {
      if (!Number.isInteger(w.weekday) || w.weekday < 0 || w.weekday > 6)
        throw new SupportValidationError('weekday must be 0 (Sunday) … 6 (Saturday)');
      if (!HHMM.test(w.start) || !HHMM.test(w.end) || w.end <= w.start)
        throw new SupportValidationError(`working window ${w.start}–${w.end} is not valid`);
    }
    for (const p of PRIORITIES) {
      const v = input.priorities[p];
      if (!v || !(v.responseMinutes > 0) || !(v.resolutionMinutes > 0))
        throw new SupportValidationError(
          `priority "${p}" needs positive responseMinutes and resolutionMinutes`,
        );
      if (v.responseMinutes > v.resolutionMinutes)
        throw new SupportValidationError(
          `priority "${p}": response time cannot exceed resolution time`,
        );
    }
    try {
      new Intl.DateTimeFormat('en-US', { timeZone: input.timeZone ?? 'Africa/Cairo' });
    } catch {
      throw new SupportValidationError(`unknown time zone "${input.timeZone}"`);
    }
    if (input.isDefault)
      await this.database.db.update(serviceLevelAgreement).set({ isDefault: false });
    return (
      await this.database.db
        ...
        .values({
          ...
          holidayListId: input.holidayListId ?? null,
          priorities: input.priorities,
          autoCloseAfterDays: input.autoCloseAfterDays ?? 7,
  ...
  async listSlas(): Promise<SlaRow[]> {
    return this.database.db
      .select()
      .from(serviceLevelAgreement)
      .orderBy(asc(serviceLevelAgreement.code));
  }
  ...
  async createTicket(
    ...
    if (input.customerId && this.crm) {
      await this.crm.getCustomer(input.customerId).catch((e) => {
        if (e instanceof CrmNotFoundError)
          throw new SupportNotFoundError(`customer ${input.customerId} does not exist`);
        throw e;
      });
    }
    const sla = await this.slaFor(input.slaId);
    const deadlines = sla
      ? await this.deadlines(sla, priority, now)
      : { responseBy: null, resolutionBy: null };
    const n = Number((await this.database.db.select({ n: count() }).from(ticket))[0]?.n ?? 0) + 1;
    return (
      await this.database.db
        .insert(ticket)
        .values({
          ticketNumber: `TKT-${now.getFullYear()}-${String(n).padStart(6, '0')}`,
  ...
  async getTicket(
    ...
    TicketRow & { comments: Array<typeof ticketComment.$inferSelect>; children: string[] }
  > {
    ...
    const children = (
      await this.database.db
        .select({ n: ticket.ticketNumber })
        .from(ticket)
        .where(eq(ticket.parentTicketId, id))
    ).map((r) => r.n);
  ...
  async listTickets(status?: TicketStatus): Promise<TicketRow[]> {
    ...
    return status
      ? q.where(eq(ticket.status, status)).orderBy(desc(ticket.openedAt))
      : q.orderBy(desc(ticket.openedAt));
  }
  ...
  async comment(
    ...
    await this.database.db
      ...
      .values({ ticketId: id, author, body: body.trim(), createdAt: now });
  ...
  async setStatus(
    ...
    if (t.status === 'closed' && status !== 'open')
      throw new SupportValidationError(`التذكرة ${t.ticketNumber} مقفولة — افتحها الأول`);
    ...
    if (t.status === 'on_hold' && t.onHoldSince) {
      ...
      if (cal && held > 0) {
        if (t.resolutionBy && !t.resolvedAt)
          set.resolutionBy = addWorkingMinutes(t.resolutionBy, held, cal);
        if (t.responseBy && !t.firstRespondedAt)
          set.responseBy = addWorkingMinutes(t.responseBy, held, cal);
      }
      set.totalHoldMinutes = t.totalHoldMinutes + held;
      set.onHoldSince = null;
    }
    if (status === 'on_hold') {
      if (!['open', 'replied'].includes(t.status))
        throw new SupportValidationError('only an open or replied ticket can go on hold');
      set.onHoldSince = now;
  ...
  async split(
    id: string,
    input: { subject: string; commentIds: string[] },
    now = new Date(),
  ): Promise<{ parent: string; child: TicketRow }> {
    const t = await this.mustTicket(id);
    if (t.status === 'closed') throw new SupportValidationError(`التذكرة ${t.ticketNumber} مقفولة`);
    ...
    const comments = await this.database.db
      .select()
      .from(ticketComment)
      .where(and(eq(ticketComment.ticketId, id), inArray(ticketComment.id, ids)));
    if (comments.length !== ids.length)
      throw new SupportValidationError('every comment must belong to this ticket');
    const child = await this.createTicket(
      {
        subject: input.subject,
        description: comments.map((c) => c.body).join('\n---\n'),
    ...
    await this.database.db
      ...
      .set({ ticketId: child.id })
      .where(inArray(ticketComment.id, ids));
  ...
  async sweep(
    now = new Date(),
  ): Promise<{ responseFailed: number; resolutionFailed: number; autoClosed: string[] }> {
    const db = this.database.db;
    const r1 = await db
      .update(ticket)
      .set({ responseStatus: 'failed', updatedAt: now })
      .where(
        and(
          eq(ticket.responseStatus, 'pending'),
          isNull(ticket.firstRespondedAt),
          isNull(ticket.onHoldSince),
          lt(ticket.responseBy, now),
        ),
      )
      .returning({ id: ticket.id });
    const r2 = await db
      .update(ticket)
      .set({ resolutionStatus: 'failed', updatedAt: now })
      .where(
        and(
          eq(ticket.resolutionStatus, 'pending'),
          isNull(ticket.resolvedAt),
          isNull(ticket.onHoldSince),
          lt(ticket.resolutionBy, now),
        ),
      )
      .returning({ id: ticket.id });
    ...
    const waiting = await db
      .select()
      .from(ticket)
      .where(inArray(ticket.status, ['replied', 'resolved']));
      ...
      const sla = t.slaId
        ? (
            await db
              .select()
              .from(serviceLevelAgreement)
              .where(eq(serviceLevelAgreement.id, t.slaId))
              .limit(1)
          )[0]
        : undefined;
  ...
  async slaReport(): Promise<
    Array<{
      priority: string;
      tickets: number;
      responseFulfilled: number;
      responseFailed: number;
      resolutionFulfilled: number;
  ...
  > {
    ...
    return rows.map((r) => ({
      ...
      responseFulfilled: Number(r.responseFulfilled),
      responseFailed: Number(r.responseFailed),
      resolutionFulfilled: Number(r.resolutionFulfilled),
      resolutionFailed: Number(r.resolutionFailed),
    }));
  }
    ...
    if (slaId) {
      const s = (
        await db
          .select()
          .from(serviceLevelAgreement)
          .where(eq(serviceLevelAgreement.id, slaId))
          .limit(1)
      )[0];
    ...
    return (
      (
        await db
          .select()
          .from(serviceLevelAgreement)
          .where(eq(serviceLevelAgreement.isDefault, true))
          .limit(1)
      )[0] ?? null
    );
  }
  ...
  private async deadlines(
    ...
    priority: Priority,
    from: Date,
  ): Promise<{ responseBy: Date; resolutionBy: Date }> {
    const cal = await this.calendar(sla);
    const p = sla.priorities[priority]!;
    return {
      responseBy: addWorkingMinutes(from, p.responseMinutes, cal),
      resolutionBy: addWorkingMinutes(from, p.resolutionMinutes, cal),
    };
  }
  ...
  private async calendar(sla: SlaRow): Promise<Calendar> {
    const holidays = sla.holidayListId
      ? (
          await this.database.db
            .select({ d: holiday.holidayDate })
            .from(holiday)
            .where(eq(holiday.holidayListId, sla.holidayListId))
        ).map((h) => h.d)
      : [];
  ...
  private async update(id: string, set: Partial<TicketRow>): Promise<TicketRow> {
    return (
      await this.database.db.update(ticket).set(set).where(eq(ticket.id, id)).returning()
    )[0]!;
  }
}

 ...
 */
export const project = projectsSchema.table(
  'project',
  {
  ...
  (t) => [
    ...
    check(
      'project_method_valid',
      sql`${t.percentCompleteMethod} in ('manual', 'task_completion', 'task_progress', 'task_weight')`,
    ),
    check(
      'project_frequency_valid',
      sql`${t.reportFrequency} in ('none', 'daily', 'weekly', 'monthly')`,
    ),
    check(
      'project_manual_range',
      sql`${t.manualPercentComplete} is null or (${t.manualPercentComplete} >= 0 and ${t.manualPercentComplete} <= 100)`,
    ),
  ],
);

export const projectTask = projectsSchema.table(
  'task',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const emailOutbox = projectsSchema.table(
  'email_outbox',
  {
    id: uuid('id').primaryKey().defaultRandom(),
  ...
  (t) => [
    check(
      ...
      sql`${t.status} in ('queued', 'sent', 'failed', 'no_transport')`,
    ),

 ...
 */
export async function sendMail(url: string, from: string, to: string[], subject: string, body: string, timeoutMs = 15_000): Promise<void> {
  const u = new URL(url);
  const secure = u.protocol === 'smtps:';
  const port = Number(u.port || (secure ? 465 : 25));
  const socket: Socket = secure ? tlsConnect({ host: u.hostname, port, servername: u.hostname }) : netConnect({ host: u.hostname, port });
  ...
  const reply = (): Promise<string> => Promise.race([new Promise<string>((res) => waiters.push(res)), failed]);
  const expect = async (code: string, send?: string): Promise<void> => {
    const pending = reply();
    if (send !== undefined) socket.write(`${send}\r\n`);
    const r = await pending;
    if (!r.split('\r\n').filter(Boolean).at(-1)!.startsWith(code)) throw new Error(`SMTP: expected ${code}, got "${r.trim()}"`);
  };
  ...
  try {
    ...
    const headers = [
      `From: ${from}`, `To: ${to.join(', ')}`, `Subject: =?UTF-8?B?${Buffer.from(subject).toString('base64')}?=`,
      'MIME-Version: 1.0', 'Content-Type: text/plain; charset=UTF-8', 'Content-Transfer-Encoding: base64', `Date: ${new Date().toUTCString()}`,
    ];

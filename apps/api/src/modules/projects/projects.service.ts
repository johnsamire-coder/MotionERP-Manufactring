...
} from '../accounting/accounting.schema';
...
export interface Profitability {
  ...
  costVsEstimatePercent: string | null;
}
const EMAIL = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
...
export class ProjectsService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    ...
    if (minutes > 0) {
      this.timer = setInterval(() => {
        this.sendDueReports().catch((e: unknown) =>
          this.logger.error(`project reports failed: ${String(e)}`),
        );
      }, minutes * 60_000);
      this.timer.unref();
    }
  ...
  async create(input: {
    ...
    startDate?: string;
    endDate?: string;
    percentCompleteMethod?: PercentMethod;
    ...
    if (!input.code?.trim() || !input.name?.trim())
      throw new ProjectsValidationError('code and name are required');
    if (input.customerId && this.crm) {
      await this.crm.getCustomer(input.customerId).catch((e) => {
        if (e instanceof CrmNotFoundError)
          throw new ProjectsNotFoundError(`customer ${input.customerId} does not exist`);
        throw e;
      });
    }
    ...
    if (
      (
        await this.database.db
          ...
          .from(project)
          .where(eq(project.code, input.code.trim()))
          .limit(1)
      )[0]
    ) {
    ...
    if (input.startDate && input.endDate && input.endDate < input.startDate)
      throw new ProjectsValidationError('endDate is before startDate');
    ...
    if (this.dimensions) {
      ...
      const dim =
        dims.find((d) => d.code === 'PROJECT') ??
        (await this.dimensions.createDimension({
          orgNodeId: input.orgNodeId,
          code: 'PROJECT',
          name: 'المشروع',
        }));
      dimensionValueId = (
        await this.dimensions.addDimensionValue(dim.id, {
    ...
    return (
      await this.database.db
        ...
        .values({
          ...
          percentCompleteMethod: input.percentCompleteMethod ?? 'task_completion',
          estimatedCost: input.estimatedCost ?? null,
          contractValue: input.contractValue ?? null,
          reportFrequency: input.reportFrequency ?? 'none',
  ...
  async update(
    ...
    if (input.manualPercentComplete !== undefined)
      set.manualPercentComplete = input.manualPercentComplete.toFixed(2);
    ...
    return (
      await this.database.db.update(project).set(set).where(eq(project.id, id)).returning()
    )[0]!;
  }
    ...
    return Promise.all(
      rows.map(async (p) => ({
        ...
        percentComplete: percentComplete(
          p.percentCompleteMethod as PercentMethod,
          await this.tasksOf(p.id),
          p.manualPercentComplete === null ? null : Number(p.manualPercentComplete),
        ),
      })),
    );
  }
  ...
  async get(
    ...
    ProjectRow & { percentComplete: number; tasks: TaskRow[]; profitability: Profitability }
  > {
    ...
    return {
      ...p,
      tasks,
      profitability: await this.profitability(p),
      percentComplete: percentComplete(
        p.percentCompleteMethod as PercentMethod,
        tasks.map((t) => ({
          status: t.status,
  ...
  async addTask(
    projectId: string,
    input: { subject: string; weight?: number; expectedEnd?: string },
  ): Promise<TaskRow> {
    ...
    return (
      await this.database.db
        ...
        .values({
          ...
          weight: (input.weight ?? 1).toFixed(2),
          expectedEnd: input.expectedEnd ? new Date(input.expectedEnd) : null,
        })
        .returning()
    )[0]!;
  }
  ...
  async updateTask(
    taskId: string,
    input: { status?: string; progress?: number; weight?: number },
  ): Promise<TaskRow> {
    const t = (
      await this.database.db.select().from(projectTask).where(eq(projectTask.id, taskId)).limit(1)
    )[0];
  ...
  async profitability(p: ProjectRow): Promise<Profitability> {
    ...
    if (p.dimensionValueId) {
      const rows = await this.database.db
        ...
        .where(
          and(
            eq(journalLineDimension.valueId, p.dimensionValueId),
      ...
      for (const r of rows) {
        ...
        else if (r.typeCode === 'cogs' || r.typeCode === 'expense')
          costs += Number(r.debit) - Number(r.credit);
      }
    }
  ...
  async statusReport(id: string, now = new Date()): Promise<typeof emailOutbox.$inferSelect> {
    ...
    if (p.reportRecipients.length === 0)
      throw new ProjectsValidationError(`المشروع ${p.code} ملوش مستلمين للتقرير`);
    const live = p.tasks.filter((t) => t.status !== 'cancelled');
    const overdue = live.filter(
      (t) => t.status !== 'completed' && t.expectedEnd && t.expectedEnd < now,
    );
    const body = [
      `تقرير حالة المشروع ${p.code} — ${p.name}`,
      `التاريخ: ${now.toISOString().slice(0, 10)}`,
      '',
      `الحالة: ${p.status}`,
      `نسبة الإنجاز: ${p.percentComplete}% (الطريقة: ${p.percentCompleteMethod})`,
      `المهام: ${live.filter((t) => t.status === 'completed').length} خلصت من ${live.length}`,
      overdue.length > 0
        ? `مهام متأخرة: ${overdue.map((t) => t.subject).join('، ')}`
        : 'مفيش مهام متأخرة',
      '',
      `الإيرادات: ${p.profitability.revenue}`,
      `التكاليف: ${p.profitability.costs}`,
      `الربح: ${p.profitability.profit}${p.profitability.marginPercent ? ` (هامش ${p.profitability.marginPercent}%)` : ''}`,
      p.profitability.costVsEstimatePercent
        ? `التكاليف = ${p.profitability.costVsEstimatePercent}% من التقدير`
        : '',
    ]
    ...
    const row = (
      await this.database.db
        .insert(emailOutbox)
        .values({
          recipients: p.reportRecipients,
          subject: `تقرير حالة المشروع ${p.code}`,
          body,
          sourceType: 'project_status_report',
          sourceId: p.id,
        })
        .returning()
    )[0]!;
  ...
  async sendDueReports(now = new Date()): Promise<string[]> {
    const due = (
      await this.database.db
        .select()
        .from(project)
        .where(and(eq(project.status, 'open'), ne(project.reportFrequency, 'none')))
    ).filter(
      (p) => p.reportRecipients.length > 0 && reportDue(p.reportFrequency, p.lastReportAt, now),
    );
  ...
  async queueEmail(
    ...
    sourceType: string,
    sourceId?: string,
  ): Promise<typeof emailOutbox.$inferSelect> {
    this.checkRecipients(recipients);
    if (recipients.length === 0)
      throw new ProjectsValidationError('at least one recipient is required');
    const row = (
      await this.database.db
        .insert(emailOutbox)
        .values({ recipients, subject, body, sourceType, sourceId: sourceId ?? null })
        .returning()
    )[0]!;
    return this.deliver(row.id);
  }
  ...
  async deliver(outboxId: string): Promise<typeof emailOutbox.$inferSelect> {
    ...
    return (
      await db.update(emailOutbox).set(set).where(eq(emailOutbox.id, outboxId)).returning()
    )[0]!;
  }
  ...
  private async tasksOf(
    projectId: string,
  ): Promise<Array<{ status: string; progress: number; weight: number }>> {
    return (
      await this.database.db.select().from(projectTask).where(eq(projectTask.projectId, projectId))
    ).map((t) => ({ status: t.status, progress: Number(t.progress), weight: Number(t.weight) }));
  }

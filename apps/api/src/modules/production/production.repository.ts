...
const columns = {
  ...
  jobOrderReference: materialRequest.jobOrderReference,
  orgNodeId: materialRequest.orgNodeId,
  itemId: materialRequest.itemId,
  warehouseId: materialRequest.warehouseId,
  plannedQuantity: materialRequest.plannedQuantity,
...
export class ProductionRepository {
  ...
  async listRequests(jobOrderReference?: string): Promise<MaterialRequestRecord[]> {
    const rows = jobOrderReference
      ? await this.database.db
          ...
          .where(eq(materialRequest.jobOrderReference, jobOrderReference))
          .orderBy(asc(materialRequest.createdAt))
      : await this.database.db
          .select(columns)
          .from(materialRequest)
          .orderBy(asc(materialRequest.createdAt));
  ...
  async closeRequest(id: string, actualUsedQuantity: string): Promise<MaterialRequestRecord> {
    const rows = await this.database.db
      .update(materialRequest)
      .set({ actualUsedQuantity, status: 'closed' })
      .where(eq(materialRequest.id, id))
      .returning(columns);
    return toRecord(rows[0]!);
  }

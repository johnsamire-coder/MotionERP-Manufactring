...
describe('Year-end closing lines (plan item 36)', () => {
  ...
  const row = (accountId: string, typeCode: string, debit: number, credit: number) => ({
    accountId,
    code: accountId,
    name: accountId,
    typeCode,
  ...
  it('1. zeroes revenue, COGS and expenses; the profit goes to retained earnings', () => {
    const r = buildClosingLines(
      [
        row('sales', 'revenue', 0, 1000),
        row('sales', 'revenue', 50, 0), // returns
        row('cogs', 'cogs', 600, 0),
        row('rent', 'expense', 100, 0),
  ...
  it('2. a loss debits retained earnings', () => {
    const r = buildClosingLines(
      [row('sales', 'revenue', 0, 100), row('rent', 'expense', 300, 0)],
      re,
    );

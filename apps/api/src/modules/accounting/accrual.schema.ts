...
export const accruedExpense = accrualSchema.table('accrued_expense', {
...
export const prepaidExpense = accrualSchema.table('prepaid_expense', {
  id: uuid('id').primaryKey().defaultRandom(),
  voucherNumber: text('voucher_number').notNull().unique(),
  orgNodeId: uuid('org_node_id')
    .notNull()
  ...
  check('prepaid_expense_status_valid', sql`${t.status} in ('active', 'fully_consumed', 'cancelled')`),
...
export const warrantyProvision = accrualSchema.table('warranty_provision', {
  id: uuid('id').primaryKey().defaultRandom(),
  ...
  check('warranty_provision_status_valid', sql`${t.status} in ('active', 'fully_utilized', 'expired', 'cancelled')`),
  check('warranty_provision_rate_valid', sql`${t.provisionRate} > 0 AND ${t.provisionRate} <= 100`),

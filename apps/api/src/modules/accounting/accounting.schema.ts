...
export const accountType = accountingSchema.table(
  'account_type',
  {
...
export const chartOfAccounts = accountingSchema.table(
  'chart_of_accounts',
  {
    id: uuid('id').primaryKey().defaultRandom(),
  ...
  (t) => [
    ...
    check(
      'chart_of_accounts_balance_must_be_valid',
      sql`${t.balanceMustBe} is null or ${t.balanceMustBe} in ('debit', 'credit')`,
    ),
    check(
      'chart_of_accounts_role_valid',
      sql`${t.accountRole} is null or ${t.accountRole} in ('accumulated_depreciation', 'asset_received_but_not_billed', 'bank', 'cash', 'chargeable', 'capital_work_in_progress', 'cost_of_goods_sold', 'current_asset', 'current_liability', 'depreciation', 'direct_expense', 'direct_income', 'equity', 'expense_account', 'expenses_included_in_asset_valuation', 'expenses_included_in_valuation', 'fixed_asset', 'income_account', 'indirect_expense', 'indirect_income', 'liability', 'payable', 'receivable', 'round_off', 'service_received_but_not_billed', 'stock', 'stock_adjustment', 'stock_received_but_not_billed', 'tax', 'temporary')`,
    ),
...
export const fiscalYear = accountingSchema.table(
  'fiscal_year',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const accountingPeriod = accountingSchema.table(
  'accounting_period',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const costCenter = accountingSchema.table(
  'cost_center',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    orgNodeId: uuid('org_node_id')
    ...
    code: text('code').notNull(),
    name: text('name').notNull(),
    parentId: uuid('parent_id').references((): AnyPgColumn => costCenter.id, {
      onDelete: 'set null',
    }),
...
export const companyAccountingConfig = accountingSchema.table(
  'company_accounting_config',
  {
    ...
    defaultGrniAccountId: uuid('default_grni_account_id').references(() => chartOfAccounts.id, {
      onDelete: 'set null',
    }),
    ...
    defaultMfgVarianceAccountId: uuid('default_mfg_variance_account_id').references(
      () => chartOfAccounts.id,
      { onDelete: 'set null' },
    ),
    defaultPayableAccountId: uuid('default_payable_account_id').references(
      () => chartOfAccounts.id,
      { onDelete: 'set null' },
    ),
    defaultReceivableAccountId: uuid('default_receivable_account_id').references(
      () => chartOfAccounts.id,
      { onDelete: 'set null' },
    ),
    defaultInputTaxAccountId: uuid('default_input_tax_account_id').references(
      () => chartOfAccounts.id,
      { onDelete: 'set null' },
    ),
    defaultOutputTaxAccountId: uuid('default_output_tax_account_id').references(
      () => chartOfAccounts.id,
      { onDelete: 'set null' },
    ),
    defaultScrapAccountId: uuid('default_scrap_account_id').references(() => chartOfAccounts.id, {
      onDelete: 'set null',
    }),
    defaultStockAdjustmentAccountId: uuid('default_stock_adjustment_account_id').references(
      () => chartOfAccounts.id,
      { onDelete: 'set null' },
    ),
    defaultOhAppliedAccountId: uuid('default_oh_applied_account_id').references(
      () => chartOfAccounts.id,
      { onDelete: 'set null' },
    ),
    ...
    defaultRoundOffAccountId: uuid('default_round_off_account_id').references(
      () => chartOfAccounts.id,
      { onDelete: 'set null' },
    ),
    defaultWriteOffAccountId: uuid('default_write_off_account_id').references(
      () => chartOfAccounts.id,
      { onDelete: 'set null' },
    ),
    defaultExchangeGainLossAccountId: uuid('default_exchange_gain_loss_account_id').references(
      () => chartOfAccounts.id,
      { onDelete: 'set null' },
    ),
    ...
    defaultAdvanceReceivedAccountId: uuid('default_advance_received_account_id').references(
      () => chartOfAccounts.id,
      { onDelete: 'set null' },
    ),
    defaultAdvancePaidAccountId: uuid('default_advance_paid_account_id').references(
      () => chartOfAccounts.id,
      { onDelete: 'set null' },
    ),
    /** Plan item 41: filled into revenue / expense lines that come without a cost center. */
    defaultCostCenterId: uuid('default_cost_center_id').references(() => costCenter.id, {
      onDelete: 'set null',
    }),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
  ...
  (t) => [
    check(
      'company_config_valuation_valid',
      sql`${t.inventoryValuationMethod} in ('weighted_average', 'fifo', 'standard')`,
    ),
  ],
);

export const accountDetermination = accountingSchema.table(
  ...
  (t) => [
    check(
      'account_det_type_valid',
      sql`${t.determinationType} in ('item_category', 'warehouse', 'default')`,
    ),
    check(
      'account_det_purpose_valid',
      sql`${t.accountPurpose} in ('inventory', 'cogs', 'revenue', 'wip', 'purchase', 'scrap', 'stock_adjustment', 'input_tax', 'output_tax', 'payable', 'receivable')`,
    ),
...
export const journalEntry = accountingSchema.table(
  'journal_entry',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    reversalOfEntryId: uuid('reversal_of_entry_id').references((): AnyPgColumn => journalEntry.id, {
      onDelete: 'restrict',
    }),
  ...
  (t) => [
    ...
    check(
      'journal_entry_voucher_type_valid',
      sql`${t.voucherType} in ('journal_entry', 'inter_company_journal_entry', 'bank_entry', 'cash_entry', 'credit_card_entry', 'debit_note', 'credit_note', 'contra_entry', 'excise_entry', 'write_off_entry', 'opening_entry', 'depreciation_entry', 'exchange_rate_revaluation', 'exchange_gain_or_loss', 'deferred_revenue', 'deferred_expense', 'reversal_of_itc')`,
    ),
    check(
      'journal_entry_not_own_reversal',
      sql`${t.reversalOfEntryId} is null or ${t.reversalOfEntryId} <> ${t.id}`,
    ),
...
export const journalLine = accountingSchema.table(
  ...
  (t) => [
    check(
      'journal_line_amounts_valid',
      sql`${t.debitAmount} >= 0 AND ${t.creditAmount} >= 0 AND (${t.debitAmount} > 0 OR ${t.creditAmount} > 0)`,
    ),
    check(
      'journal_line_party_type_valid',
      sql`${t.partyType} is null or ${t.partyType} in ('customer', 'supplier')`,
    ),
...
/**
 * Legacy fixed-asset register. Retired: migration 0100 copied its rows (and depreciation_entry
 * history) into the assets module, which now owns fixed assets. Kept read-only for audit.
 */
export const fixedAsset = accountingSchema.table(
  'fixed_asset',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    totalDepreciated: numeric('total_depreciated', { precision: 14, scale: 4 })
      .notNull()
      .default('0'),
  ...
  (t) => [
    ...
    check(
      'fixed_asset_status_valid',
      sql`${t.status} in ('active', 'fully_depreciated', 'disposed')`,
    ),
...
export const depreciationEntry = accountingSchema.table(
  'depreciation_entry',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    accumulatedAmountAfter: numeric('accumulated_amount_after', {
      precision: 14,
      scale: 4,
    }).notNull(),
 ...
 */
export const budget = accountingSchema.table(
  'budget',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    orgNodeId: uuid('org_node_id')
  ...
  (t) => [
    unique('budget_fy_account_cc_unique')
      .on(t.fiscalYearId, t.accountId, t.costCenterId)
      .nullsNotDistinct(),
...
export const accountingDimension = accountingSchema.table(
...
export const accountingDimensionValue = accountingSchema.table(
  'accounting_dimension_value',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    dimensionId: uuid('dimension_id')
      .notNull()
      .references(() => accountingDimension.id, { onDelete: 'cascade' }),
    code: text('code').notNull(),
    name: text('name').notNull(),
    isActive: boolean('is_active').notNull().default(true),
  },
...
export const journalLineDimension = accountingSchema.table(
  'journal_line_dimension',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    journalLineId: uuid('journal_line_id')
      .notNull()
      .references(() => journalLine.id, { onDelete: 'cascade' }),
    dimensionId: uuid('dimension_id')
      .notNull()
      .references(() => accountingDimension.id, { onDelete: 'restrict' }),
    valueId: uuid('value_id')
      .notNull()
      .references(() => accountingDimensionValue.id, { onDelete: 'restrict' }),
  },
  (t) => [

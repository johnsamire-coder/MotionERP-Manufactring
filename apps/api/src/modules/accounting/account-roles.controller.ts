...
export class AccountRolesController {
  ...
  async setRole(@Param('id', ParseUUIDPipe) id: string, @Body() dto: SetAccountRoleDto): Promise<{ accountId: string; role: AccountRole | null }> {
    ...
    if (!isAccountRole(dto.role)) throw new AccountingValidationError(`unknown account role "${dto.role}" — see GET accounting/account-roles`);

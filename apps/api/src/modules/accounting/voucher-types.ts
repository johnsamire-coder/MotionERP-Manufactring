...
export const VOUCHER_TYPES: Record<VoucherType, { label: string; rule: string }> = {
  ...
  inter_company_journal_entry: {
    label: 'قيد بين الشركات',
    rule: 'لازم مرجع للقيد المقابل في الشركة التانية',
  },
  ...
  exchange_rate_revaluation: {
    label: 'إعادة تقييم عملة',
    rule: 'سطر على حساب فروق العملة الافتراضي (لو متحدد)',
  },
  exchange_gain_or_loss: {
    label: 'أرباح/خسائر فروق عملة',
    rule: 'سطر على حساب فروق العملة الافتراضي (لو متحدد)',
  },
...
export const isVoucherType = (v: string): v is VoucherType =>
  Object.prototype.hasOwnProperty.call(VOUCHER_TYPES, v);

const INCOME: readonly AccountRole[] = ['income_account', 'direct_income', 'indirect_income'];
const EXPENSE: readonly AccountRole[] = [
  'expense_account',
  'direct_expense',
  'indirect_expense',
  'cost_of_goods_sold',
...
export function voucherTypeProblem(
  ...
  const any = (roles: readonly AccountRole[]): boolean =>
    lines.some((l) => l.role !== null && roles.includes(l.role));
  const need = (ok: boolean): string | null =>
    ok ? null : `${label}: ${VOUCHER_TYPES[type].rule}`;
  switch (type) {
    case 'bank_entry':
    case 'credit_card_entry':
      return need(any(['bank']));
    case 'cash_entry':
      return need(any(['cash']));
    case 'contra_entry':
      return need(lines.every((l) => l.role === 'bank' || l.role === 'cash'));
    case 'debit_note':
    case 'credit_note':
      return need(lines.some((l) => l.hasParty));
    case 'excise_entry':
    case 'reversal_of_itc':
      return need(any(['tax']));
    case 'opening_entry':
      return need(!any([...INCOME, ...EXPENSE]));
    case 'depreciation_entry':
      return need(any(['depreciation']) && any(['accumulated_depreciation']));
    case 'deferred_revenue':
      return need(any(INCOME));
    case 'deferred_expense':
      return need(any(EXPENSE));
    case 'inter_company_journal_entry':
      return need(Boolean(ctx.reference?.trim()));
    case 'write_off_entry':
      return need(
        !ctx.writeOffAccountId || lines.some((l) => l.accountId === ctx.writeOffAccountId),
      );
    case 'exchange_rate_revaluation':
    case 'exchange_gain_or_loss':
      return need(
        !ctx.exchangeAccountId || lines.some((l) => l.accountId === ctx.exchangeAccountId),
      );
    default:
      return null;
  }

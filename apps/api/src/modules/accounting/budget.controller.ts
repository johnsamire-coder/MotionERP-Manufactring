...
export class BudgetController {
  ...
  @Get()
  async list(@Query('fiscalYearId') fiscalYearId?: string): Promise<{ budgets: BudgetRecord[] }> {
    return { budgets: await this.service.list(fiscalYearId || undefined) };
  }

  @Put()
  async upsert(@Body() dto: UpsertBudgetDto): Promise<{ budget: BudgetRecord }> {
    return { budget: await this.service.upsert(dto) };
  }

  @Get('variance/:fiscalYearId')
  async variance(
    @Param('fiscalYearId', ParseUUIDPipe) id: string,
  ): Promise<{ variance: Awaited<ReturnType<BudgetService['variance']>> }> {

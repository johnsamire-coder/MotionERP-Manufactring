  ...
  beforeEach(async () => {
    ...
    mockRepo = {
      createAccrual: jest.fn().mockImplementation((data) => {
        const rec = { id: `acc-${Date.now()}`, ...data, createdAt: new Date(), updatedAt: new Date() };
        dbAccruals.push(rec);
        return rec;
      }),
      findAccrualById: jest.fn().mockImplementation((id) => dbAccruals.find(a => a.id === id) || null),
      ...
      createPrepaid: jest.fn().mockImplementation((data) => {
        const rec = { id: `pre-${Date.now()}`, ...data, createdAt: new Date(), updatedAt: new Date() };
        dbPrepaids.push(rec);
        return rec;
      }),
      findPrepaidById: jest.fn().mockImplementation((id) => dbPrepaids.find(p => p.id === id) || null),
      updatePrepaidAmortization: jest.fn().mockImplementation((id, consumedAmount, remainingAmount, status) => {
      ...
      createProvision: jest.fn().mockImplementation((data) => {
        const rec = { id: `prv-${Date.now()}`, ...data, createdAt: new Date(), updatedAt: new Date() };
        dbProvisions.push(rec);
        return rec;
      }),
      findProvisionById: jest.fn().mockImplementation((id) => dbProvisions.find(p => p.id === id) || null),

...
export class CloseYearDto { @IsUUID() retainedEarningsAccountId!: string; }
...
export class YearEndClosingController {
  ...
  async preview(@Param('fiscalYearId', ParseUUIDPipe) id: string, @Query('retainedEarningsAccountId') retained?: string): Promise<{ preview: YearEndPreview }> {
    return { preview: await this.service.preview(id, retained || undefined) };
  }

  @Post(':fiscalYearId/close') @HttpCode(200)
  async close(@Param('fiscalYearId', ParseUUIDPipe) id: string, @Body() dto: CloseYearDto): Promise<{ preview: YearEndPreview; closingEntry: JournalEntryRecord | null }> {

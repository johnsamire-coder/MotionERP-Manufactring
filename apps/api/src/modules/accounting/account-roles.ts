...
export const ACCOUNT_ROLES: Record<AccountRole, AccountRoleSpec> = {
  ...
  expenses_included_in_asset_valuation: {
    label: 'مصروفات محمّلة على تقييم الأصول',
    normalBalance: 'credit',
  },
  expenses_included_in_valuation: {
    label: 'مصروفات محمّلة على تقييم المخزون',
    normalBalance: 'credit',
  },
  ...
  stock_received_but_not_billed: {
    label: 'بضاعة مستلمة ولم تُفوتر (GRNI)',
    normalBalance: 'credit',
  },
...
export const isAccountRole = (v: string): v is AccountRole =>
  Object.prototype.hasOwnProperty.call(ACCOUNT_ROLES, v);
...
export function manualLineProblem(
  ...
  if (spec.systemOnly)
    return `الحساب ${account.code} (${spec.label}) بيتحدّث من ${spec.systemOnly} بس — مينفعش قيد يدوي عليه`;

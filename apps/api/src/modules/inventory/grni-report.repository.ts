  ...
  async receipts(orgNodeIds: string[] | null): Promise<ReceivedNotBilledRow[]> {
    const billed = this.database.db
      .select({
        ...
        billed:
          sql<string>`sum(${purchaseInvoiceLine.quantity} * ${purchaseInvoiceLine.unitCost})`.as(
            'billed',
          ),
      })
    ...
    const conditions = [
      ...
      or(
        isNull(stockMovement.sourceModule),
        notInArray(stockMovement.sourceModule, ['production', 'manufacturing', 'inventory']),
      ),
    ];
    if (orgNodeIds)
      conditions.push(
        inArray(
          warehouse.orgNodeId,
          orgNodeIds.length > 0 ? orgNodeIds : ['00000000-0000-0000-0000-000000000000'],
        ),
      );
    const rows = await this.database.db
      .select({
        ...
        received: stockMovement.totalValue,
    ...
    return rows.map((r) => {
      ...
      return {
        ...
        purchaseOrderId: r.sourceModule === 'purchase_order' ? r.sourceId : null,
        quantity: r.quantity,
        receivedValue: received.toFixed(4),
        billedValue: billedValue.toFixed(4),
        outstandingValue: Math.max(0, received - billedValue).toFixed(4),
      };
    });
  }

  /** Posted ledger balance (credit − debit) of the company's default GRNI account, or null if not configured. */
  async ledgerGrniBalance(
    orgNodeId: string,
  ): Promise<{ accountId: string; balance: number } | null> {
    const cfg = await this.database.db
      .select({ accountId: companyAccountingConfig.defaultGrniAccountId })
    ...
    const rows = await this.database.db
      .select({
        balance: sql<string>`coalesce(sum(${journalLine.creditAmount} - ${journalLine.debitAmount}), 0)`,
      })
      .from(journalLine)

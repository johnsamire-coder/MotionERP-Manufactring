...
export class UpsertReorderDto {
  ...
  @IsOptional() @IsIn(['purchase', 'transfer', 'manufacture']) requestType?: 'purchase' | 'transfer' | 'manufacture';
}
...
export class ReorderController {
  ...
  @Get('rules')
  async rules(): Promise<{ rules: ItemReorderRecord[] }> { return { rules: await this.service.list() }; }

  @Put('rules')
  async upsert(@Body() dto: UpsertReorderDto): Promise<{ rule: ItemReorderRecord }> { return { rule: await this.service.upsert(dto) }; }

  @Delete('rules/:id') @HttpCode(204)
  async remove(@Param('id', ParseUUIDPipe) id: string): Promise<void> { await this.service.remove(id); }

  @Get('suggestions')
  async suggestions(): Promise<{ suggestions: ReorderSuggestion[] }> { return { suggestions: await this.service.suggestions() }; }

  @Post('raise') @HttpCode(200)
  async raise(): Promise<{ raised: Awaited<ReturnType<ReorderService['raise']>> }> { return { raised: await this.service.raise() }; }
}

...
describe('Seven reservation / request types and the Bin view (plan item 13)', () => {
  ...
  beforeEach(() => {
    ...
    const repo = {
      ...
      findBalance: jest
        .fn()
        .mockImplementation(async () => ({ onHand: '100', reserved: String(reserved) })),
      listBalances: jest
        .fn()
        .mockImplementation(async () => [
          { itemId: 'i', warehouseId: 'wh', onHand: '100', reserved: String(reserved) },
        ]),
      ...
      applyReservedDelta: jest
        .fn()
        .mockImplementation(async (_i: string, _w: string, d: string) => {
          reserved += Number(d);
        }),
      listReservations: jest.fn().mockImplementation(async () => reservations),
      findReservationById: jest
        .fn()
        .mockImplementation(async (id: string) => reservations.find((r) => r.id === id) ?? null),
  ...
  const reserve = (
    reservationType: StockReservationRecord['reservationType'],
    quantity: string,
  ...
  it('1. reserving types lock stock; expected types (ordered/indented/planned) do not', async () => {
    ...
    expect(bin).toMatchObject({
      actualQty: '100.000000',
      reservedQty: '10.000000',
      reservedForProduction: '20.000000',
      reservedForSubcontract: '5.000000',
      reservedForProductionPlan: '15.000000',
      orderedQty: '40.000000',
      indentedQty: '7.000000',
      plannedQty: '3.000000',
      availableQty: '50.000000',
      projectedQty: '100.000000', // 100 + 50 expected − 50 reserved
    });
  });

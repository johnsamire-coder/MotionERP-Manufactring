...
export const warehouse = inventorySchema.table(
  'warehouse',
  {
    ...
    parentWarehouseId: uuid('parent_warehouse_id').references((): AnyPgColumn => warehouse.id, {
      onUpdate: 'cascade',
      onDelete: 'restrict',
    }),
  ...
  (t) => [
    ...
    check(
      'warehouse_not_own_parent',
      sql`${t.parentWarehouseId} is null or ${t.parentWarehouseId} <> ${t.id}`,
    ),
...
export const stockBalance = inventorySchema.table(
  'stock_balance',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    itemId: uuid('item_id')
...
export const stockMovement = inventorySchema.table(
  'stock_movement',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    itemId: uuid('item_id')
    ...
    batchId: uuid('batch_id').references((): AnyPgColumn => itemBatch.id, {
      onUpdate: 'cascade',
      onDelete: 'restrict',
    }),
  ...
  (t) => [
    check(
      'stock_movement_type_valid',
      sql`${t.movementType} in ('receipt', 'issue', 'transfer_in', 'transfer_out', 'adjustment')`,
    ),
    check('stock_movement_quantity_not_zero', sql`${t.quantity} <> 0`),
    check(
      'stock_movement_purpose_valid',
      sql`${t.purpose} in ('general', 'material_transfer_for_manufacture', 'manufacture_consumption')`,
    ),
...
export const stockReservation = inventorySchema.table(
  'stock_reservation',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    itemId: uuid('item_id')
  ...
  (t) => [
    ...
    check(
      'stock_reservation_type_valid',
      sql`${t.reservationType} in ('sales_order', 'production', 'subcontract', 'production_plan', 'purchase_order', 'material_request', 'work_order')`,
    ),
    index('stock_reservation_item_warehouse_idx').on(t.itemId, t.warehouseId),
  ],
);

export const stockLedgerEntry = inventorySchema.table(
  'stock_ledger_entry',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    itemId: uuid('item_id')
    ...
    stockValueChange: numeric('stock_value_change', { precision: 24, scale: 6 })
      .notNull()
      .default('0'),
    stockValueAfter: numeric('stock_value_after', { precision: 24, scale: 6 })
      .notNull()
      .default('0'),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  },
...
export const itemBatch = inventorySchema.table(
  'item_batch',
  {
    id: uuid('id').primaryKey().defaultRandom(),
  ...
  (t) => [
    ...
    check(
      'item_batch_status_valid',
      sql`${t.status} in ('active', 'expired', 'quarantined', 'recalled')`,
    ),
...
export const serialNumber = inventorySchema.table(
  'serial_number',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    batchId: uuid('batch_id').references(() => itemBatch.id, {
      onUpdate: 'cascade',
      onDelete: 'set null',
    }),
    orgNodeId: uuid('org_node_id')
  ...
  (t) => [
    ...
    check(
      'serial_number_status_valid',
      sql`${t.status} in ('active', 'delivered', 'under_maintenance', 'decommissioned')`,
    ),
...
export const batchBalance = inventorySchema.table(
  'batch_balance',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const stockMovementSerial = inventorySchema.table(
 ...
 */
export const itemReorder = inventorySchema.table(
  'item_reorder',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    itemId: uuid('item_id').notNull(),
  ...
  (t) => [
    ...
    check(
      'item_reorder_request_type_valid',
      sql`${t.requestType} in ('purchase', 'transfer', 'manufacture')`,
    ),
  ],
);
...
export const landedCostVoucher = inventorySchema.table(
  'landed_cost_voucher',
  {
    id: uuid('id').primaryKey().defaultRandom(),
  ...
  (t) => [
    ...
    check(
      'landed_cost_distribute_valid',
      sql`${t.distributeMethod} in ('by_amount', 'by_quantity')`,
    ),
...
export const landedCostItem = inventorySchema.table(
  'landed_cost_item',
  {
    id: uuid('id').primaryKey().defaultRandom(),
 ...
 */
export const pickList = inventorySchema.table(
  'pick_list',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    scopeWarehouseId: uuid('scope_warehouse_id').references(() => warehouse.id, {
      onUpdate: 'cascade',
      onDelete: 'restrict',
    }),
  ...
  (t) => [
    ...
    check(
      'pick_list_transfer_has_target',
      sql`${t.purpose} <> 'material_transfer' or ${t.targetWarehouseId} is not null`,
    ),
  ],
);

export const pickListLine = inventorySchema.table(
  'pick_list_line',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    pickListId: uuid('pick_list_id')
      .notNull()
      .references(() => pickList.id, { onUpdate: 'cascade', onDelete: 'cascade' }),
    itemId: uuid('item_id').notNull(),
    warehouseId: uuid('warehouse_id')
      .notNull()
      .references(() => warehouse.id, { onUpdate: 'cascade', onDelete: 'restrict' }),
  ...
  (t) => [
    ...
    check(
      'pick_list_line_picked_range',
      sql`${t.pickedQuantity} is null or (${t.pickedQuantity} >= 0 and ${t.pickedQuantity} <= ${t.quantity})`,
    ),

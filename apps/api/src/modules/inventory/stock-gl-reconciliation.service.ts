...
export interface AccountComparison {
  ...
  stockValue: string;
  difference: string;
  warehouses: Array<{ id: string; code: string; stockValue: string }>;
}
...
export function compareStockWithGl(
  warehouses: Array<{
    ...
    code: string;
    stockValue: number;
    accounts: { inventoryAccountId: string; contraAccountId: string } | null;
  }>,
  ...
  for (const w of warehouses) {
    if (!w.accounts) {
      if (Math.abs(w.stockValue) > TOLERANCE)
        unmapped.push({ id: w.id, code: w.code, stockValue: w.stockValue.toFixed(4) });
      continue;
    }
    const g = groups.get(w.accounts.inventoryAccountId) ?? {
      adjustment: w.accounts.contraAccountId,
      value: 0,
      warehouses: [],
    };
  ...
  for (const [accountId, g] of groups) {
    ...
    accounts.push({
      ...
      accountCode: accountCode(accountId),
      adjustmentAccountId: g.adjustment,
      glBalance: gl.toFixed(4),
      stockValue: g.value.toFixed(4),
  ...
  return {
    ...
    totalDifference: total.toFixed(4),
    balanced: accounts.every((a) => Math.abs(Number(a.difference)) <= TOLERANCE),
    suggestedLines,
  };
}
...
export class StockGlReconciliationService {
  ...
  async compare(orgNodeId: string): Promise<StockGlComparison> {
    if (!this.accounting || !this.postingEngine)
      throw new InventoryValidationError('accounting is not available');
    const warehouses = (await this.repository.listWarehouses()).filter(
      (w) => w.orgNodeId === orgNodeId && !w.isGroup,
    );
    ...
    const result = compareStockWithGl(
      ...
      (id) => {
        const b = gl.get(id);
        return b ? Number(b.totalDebit) - Number(b.totalCredit) : 0;
      },
  ...
  async postCorrection(
    ...
    if (!reason?.trim())
      throw new InventoryValidationError(
        'a reason is required to post a stock / ledger correction',
      );
    const comparison = await this.compare(orgNodeId);
    if (comparison.suggestedLines.length === 0)
      throw new InventoryValidationError('المخزون والدفاتر متطابقين — مفيش قيد تصحيح');
    ...
    try {
      ...
      if (err instanceof AccountingValidationError || err instanceof AccountingNotFoundError)
        throw new InventoryValidationError(err.message);
      throw err;
    }
  }
  ...
  private async post(
    ...
    const draft = await this.accounting!.createEntry({
      ...
      lines: comparison.suggestedLines.map((l) => ({
        ...l,
        description: `[Auto] تسوية مخزون/دفاتر`,
      })),
    });

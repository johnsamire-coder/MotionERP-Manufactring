...
describe('Inventory user restrictions (plan item 5.2)', () => {
  ...
  beforeEach(() => {
    ...
    movements.length = 0;
    const perms = {
      getRestrictions: jest.fn().mockImplementation(async () => restrictions),
    } as unknown as UserPermissionService;
    ...
    const repo = {
      ...
      findWarehouseById: jest
        .fn()
        .mockImplementation(async (id: string) => warehouses.find((w) => w.id === id) ?? null),
      ...
      findBalance: jest
        .fn()
        .mockResolvedValue({ onHand: '100', reserved: '0', averageCost: '1', totalValue: '100' }),
      ...
      listMovements: jest.fn().mockImplementation(async () => [
        { id: 'm1', warehouseId: 'wh-cai' },
        { id: 'm2', warehouseId: 'wh-alx' },
        { id: 'm3', warehouseId: 'wh-hq' },
      ]),
  ...
  it('1. anonymous or unrestricted users see every warehouse', async () => {
    expect(ids(await asUser(() => service.getWarehouses(), undefined))).toEqual([
      'wh-alx',
      'wh-cai',
      'wh-hq',
    ]);
    ...
    await expect(
      asUser(() =>
        service.transferStock({
          itemId: 'i',
          fromWarehouseId: 'wh-cai',
          toWarehouseId: 'wh-alx',
          quantity: '1',
        }),
      ),
    ).rejects.toThrow(InventoryForbiddenError);
  ...
  it('6. creating a warehouse: only inside allowed org nodes; warehouse-restricted users cannot', async () => {
    restrictions.orgNodeIds = ['CAI'];
    await asUser(() =>
      service.createWarehouse({ code: 'CAI-2', name: 'Cairo 2', orgNodeId: 'CAI' }),
    );
    await expect(
      asUser(() => service.createWarehouse({ code: 'ALX-2', name: 'Alex 2', orgNodeId: 'ALX' })),
    ).rejects.toThrow(InventoryForbiddenError);
    restrictions = { orgNodeIds: null, warehouseIds: ['wh-cai'] };
    await expect(
      asUser(() => service.createWarehouse({ code: 'X', name: 'X', orgNodeId: 'CAI' })),

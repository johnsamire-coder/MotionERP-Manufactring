...
export interface WarehouseTreeNode {
  id: string; code: string; name: string; isGroup: boolean; parentWarehouseId: string | null;
  /** Stock held directly in this warehouse (always 0 for a group). */
  ownOnHand: string; ownValue: string;
  /** Stock of this warehouse plus every warehouse under it. */
  totalOnHand: string; totalValue: string;
  children: WarehouseTreeNode[];
}

/** Builds the warehouse tree with rolled-up totals (pure). */
export function buildWarehouseTree(
  warehouses: Array<Pick<WarehouseRecord, 'id' | 'code' | 'name' | 'parentWarehouseId' | 'isGroup'>>,
  ...
  const build = (w: (typeof warehouses)[number]): WarehouseTreeNode => {
    ...
    return {
      id: w.id, code: w.code, name: w.name, isGroup: w.isGroup ?? false, parentWarehouseId: w.parentWarehouseId ?? null,
      ownOnHand: o.qty.toFixed(6), ownValue: o.value.toFixed(4),
      totalOnHand: totalQty.toFixed(6), totalValue: totalVal.toFixed(4), children,
    };
  };
...
export class WarehouseTreeService {
  constructor(private readonly repository: InventoryRepository, private readonly inventory: InventoryService) {}

  /** Only the warehouses the current user may see (plan item 5.2). */
  async tree(): Promise<WarehouseTreeNode[]> {
    const [warehouses, balances] = await Promise.all([this.inventory.getWarehouses(), this.inventory.getBalances()]);
  ...
  async setParent(id: string, parentWarehouseId: string | null): Promise<WarehouseRecord> {
    ...
    if (parentWarehouseId !== null) {
      if (parentWarehouseId === id) throw new InventoryValidationError('a warehouse cannot be its own parent');
      const parent = await this.mustFind(parentWarehouseId);
      if (!parent.isGroup) throw new InventoryValidationError(`المخزن ${parent.code} مش مجموعة — لازم الأب يكون مخزن مجموعة`);
      const all = await this.repository.listWarehouses();
      const parentOf = new Map(all.map((w) => [w.id, w.parentWarehouseId ?? null]));
      for (let cur: string | null = parentWarehouseId, guard = 0; cur && guard <= all.length; cur = parentOf.get(cur) ?? null, guard++) {
        if (cur === id) throw new InventoryValidationError(`نقل ${target.code} تحت ${parent.code} هيعمل حلقة في الشجرة`);
      }
    }
  ...
  async setGroup(id: string, isGroup: boolean): Promise<WarehouseRecord> {
    ...
    if (isGroup && !target.isGroup && (await this.repository.warehouseHasStockHistory(id))) {
      throw new InventoryValidationError(`مينفعش تحوّل ${target.code} لمجموعة — عليه رصيد أو حركات`);
    }
    if (!isGroup && target.isGroup) {
      const children = (await this.repository.listWarehouses()).filter((w) => w.parentWarehouseId === id);
      if (children.length > 0) {
        throw new InventoryValidationError(`مينفعش تلغي المجموعة ${target.code} — تحتها ${children.map((c) => c.code).join('، ')}`);
      }
    }

...
describe('Landed cost into per-batch cost (plan item 15)', () => {
  const voucher = (items: Array<{ receiptMovementId: string; allocatedExpense: string }>): LandedCostVoucherRecord => ({
    id: 'v', voucherNumber: 'LCV-1', orgNodeId: 'o', postingDate: '', totalExpenseAmount: '0', distributeMethod: 'by_amount',
    expenseAccountId: 'acc', status: 'draft', notes: null, createdAt: '', updatedAt: '',
    items: items.map((x, n) => ({
      id: `li${n}`, voucherId: 'v', itemId: 'item', warehouseId: 'wh', quantity: '10', originalRate: '100',
      newValuationRate: '110', createdAt: '', ...x,
    })),
  ...
  beforeEach(() => {
    batch = { id: 'bb', batchId: 'lot-a', itemId: 'item', warehouseId: 'wh', quantity: '10', valuationRate: '100', totalValue: '1000', updatedAt: '' };
    posted = null;
    const repo = {
      findLandedCostVoucherById: jest.fn().mockImplementation(async () => lcv),
      findMovementById: jest.fn().mockImplementation(async (id: string) => ({ id, batchId: id === 'batch-receipt' ? 'lot-a' : null })),
      findBatchBalance: jest.fn().mockImplementation(async () => batch),
      upsertBatchBalance: jest.fn().mockImplementation(async (b) => { batch = { ...batch!, ...b }; return batch; }),
      findBalance: jest.fn().mockResolvedValue({ onHand: '10', totalValue: '1000' }),
      applyValuation: jest.fn(),
      insertLedgerEntry: jest.fn(),
      setLandedCostVoucherStatus: jest.fn().mockImplementation(async (_id: string, s: string) => { posted = s; return lcv; }),
  ...
  it('1. adds the allocated expense to the receipt batch cost', async () => {
    ...
    expect(batch).toMatchObject({ totalValue: '1100.000000', valuationRate: '110.000000', quantity: '10' });

...
export interface ItemReorderRecord {
  ...
  reorderLevel: string;
  reorderQty: string;
  requestType: ReorderRequestType;
}
export interface ReorderSuggestion extends ItemReorderRecord {
  projectedQty: string;
  suggestedQty: string;
}
...
export class ReorderService {
  ...
  async list(): Promise<ItemReorderRecord[]> {
    return (
      await this.database.db.select().from(itemReorder).orderBy(asc(itemReorder.createdAt))
    ).map(toRecord);
  }
  ...
  async upsert(input: {
    ...
    reorderQty: string;
    requestType?: ReorderRequestType;
  }): Promise<ItemReorderRecord> {
    const level = Number(input.reorderLevel);
    const qty = Number(input.reorderQty);
    if (!(level >= 0) || !(qty > 0))
      throw new InventoryValidationError('reorderLevel must be ≥ 0 and reorderQty > 0');
    if (!(await this.repository.findWarehouseById(input.warehouseId)))
    ...
    if (this.catalog) {
      await this.catalog.getItem(input.itemId).catch((err) => {
        if (err instanceof CatalogNotFoundError)
          throw new InventoryNotFoundError(`item ${input.itemId} does not exist`);
        throw err;
      });
    }
    const values = {
      reorderLevel: level.toFixed(6),
      reorderQty: qty.toFixed(6),
      requestType: input.requestType ?? 'purchase',
      updatedAt: new Date(),
    };
    const rows = await this.database.db
      .insert(itemReorder)
      .values({ itemId: input.itemId, warehouseId: input.warehouseId, ...values })
      .onConflictDoUpdate({ target: [itemReorder.itemId, itemReorder.warehouseId], set: values })
      .returning();
  ...
  async suggestions(): Promise<ReorderSuggestion[]> {
    ...
    const projected = new Map(
      bins.map((b) => [`${b.itemId}|${b.warehouseId}`, Number(b.projectedQty)]),
    );
    ...
    for (const rule of await this.list()) {
      ...
      if (qty !== null)
        out.push({ ...rule, projectedQty: p.toFixed(6), suggestedQty: qty.toFixed(6) });
    }
    return out;
  }
   ...
   */
  async raise(): Promise<
    Array<{ itemId: string; warehouseId: string; quantity: string; reservationId: string }>
  > {
    const raised = [];
    for (const s of await this.suggestions()) {
      const r = await this.inventory.reserveStock({
        ...
        source: `reorder:${s.requestType}:${s.id}`,
        reservationType: 'material_request',
      });
      raised.push({
        itemId: s.itemId,
        warehouseId: s.warehouseId,

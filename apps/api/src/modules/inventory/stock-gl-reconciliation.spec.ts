...
describe('Stock ↔ ledger reconciliation (plan item 35)', () => {
  ...
  const run = (rows: Parameters<typeof compareStockWithGl>[0]) =>
    compareStockWithGl(
      rows,
      (id) => gl[id] ?? 0,
      (id) => id.toUpperCase(),
  ...
  it('1. groups warehouses by inventory account and reports each difference', () => {
    ...
    expect(r.accounts.find((a) => a.inventoryAccountId === 'inv1')).toMatchObject({
      glBalance: '900.0000',
      stockValue: '1000.0000',
      difference: '100.0000',
    });
  ...
  it('2. ledger above stock → credit the inventory account', () => {
    ...
    expect(r.suggestedLines[0]).toEqual({
      accountId: 'inv1',
      debitAmount: '0',
  ...
  it('3. warehouses without accounts are listed apart; matching books need no entry', () => {
    const r = run([
      { id: 'w1', code: 'W1', stockValue: 900, accounts: acc('inv1') },
      { id: 'w9', code: 'W9', stockValue: 70, accounts: null },
    ]);

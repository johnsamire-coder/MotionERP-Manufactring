...
import { StockGlReconciliationService, type StockGlComparison } from './stock-gl-reconciliation.service';

export class PostStockGlCorrectionDto { @IsString() @MaxLength(500) reason!: string; }
...
export class StockGlReconciliationController {
  ...
  @Get(':orgNodeId')
  async compare(@Param('orgNodeId', ParseUUIDPipe) orgNodeId: string): Promise<{ comparison: StockGlComparison }> {
    return { comparison: await this.service.compare(orgNodeId) };
  }

  @Post(':orgNodeId/post') @HttpCode(200)
  async post(@Param('orgNodeId', ParseUUIDPipe) orgNodeId: string, @Body() dto: PostStockGlCorrectionDto): Promise<{ comparison: StockGlComparison; entry: JournalEntryRecord }> {

...
describe('Lead → Contact → Prospect → Customer (plan item 25)', () => {
  ...
  beforeEach(() => {
    ...
    prospects = new Map();
    customers = [];
    const withLeads = (p: ProspectRecord): ProspectRecord => ({ ...p, leadIds: [...leads.values()].filter((l) => l.prospectId === p.id).map((l) => l.id) });
    const repo = {
      ...
      findDuplicate: jest.fn(async (email: string | null, phone: string | null) =>
        [...leads.values()].find((l) => !['converted', 'lost'].includes(l.status) && ((email && l.email === email) || (phone && l.phone === phone))) ?? null),
      insertLead: jest.fn(async (v: Partial<LeadRecord> & { id: string }) => {
        const rec = { status: 'new', prospectId: null, customerId: null, lostReason: null, lastContactedAt: null, activities: [], ...v } as LeadRecord;
        leads.set(v.id, rec);
        return rec;
      }),
      updateLead: jest.fn(async (id: string, f: Record<string, unknown>) => {
        const l = leads.get(id)!;
        Object.assign(l, f, f['lastContactedAt'] instanceof Date ? { lastContactedAt: (f['lastContactedAt'] as Date).toISOString() } : {});
        return l;
      }),
      updateLeads: jest.fn(async (ids: string[], f: Partial<LeadRecord>) => { ids.forEach((id) => Object.assign(leads.get(id)!, f)); }),
      insertActivity: jest.fn(async (id: string, activityType: 'call') => { leads.get(id)!.activities.push({ id: 'a', activityType, activityDate: '', note: null }); }),
      findProspect: jest.fn(async (id: string) => (prospects.has(id) ? withLeads(prospects.get(id)!) : null)),
      insertProspect: jest.fn(async (v: Partial<ProspectRecord> & { id: string }) => { prospects.set(v.id, { status: 'open', customerId: null, leadIds: [], ...v } as ProspectRecord); }),
      updateProspect: jest.fn(async (id: string, f: Partial<ProspectRecord>) => { Object.assign(prospects.get(id)!, f); return withLeads(prospects.get(id)!); }),
    } as unknown as LeadRepository;
    const crm = {
      createCustomer: jest.fn(async (c: { code: string; name: string }) => { customers.push(c); return { id: `cust-${c.code}`, ...c }; }),
  ...
  it('1. a lead needs a way to be contacted and cannot be registered twice', async () => {
    await expect(service.createLead({ personName: 'A', orgNodeId: 'org' })).rejects.toThrow(/phone or an email/);
  ...
  it('4. a prospect groups interested leads and converts them all into one customer', async () => {
    ...
    await expect(service.createProspect({ companyName: 'ACME', orgNodeId: 'org', leadIds: [a.id, b.id] })).rejects.toThrow(/must be "interested"/);
    const p = await service.createProspect({ companyName: 'ACME', orgNodeId: 'org', leadIds: [a.id] });
    ...
    expect(customers).toEqual([expect.objectContaining({ code: 'ACME', name: 'ACME', status: 'active', contactEmail: 'a@x.com' })]);
    ...
    await expect(service.convertProspect(p.id, { code: 'ACME2' })).rejects.toThrow(/already "converted"/);
  });

...
export class LeadService {
  ...
  async listLeads(status?: LeadStatus): Promise<LeadRecord[]> {
    return this.repository.listLeads(status);
  }
  ...
  async createLead(input: CreateLeadInput): Promise<LeadRecord> {
    ...
    if (!email && !phone)
      throw new CrmValidationError('a lead needs a phone or an email to be contacted');
    const dup = await this.repository.findDuplicate(email, phone);
    if (dup)
      throw new CrmValidationError(
        `العميل المحتمل ده مسجّل قبل كده: ${dup.leadNumber} (${dup.personName})`,
      );
    const n = (await this.repository.countLeads()) + 1;
    return this.repository.insertLead({
      id: randomUUID(),
      leadNumber: `LEAD-${new Date().getFullYear()}-${String(n).padStart(6, '0')}`,
      personName,
  ...
  async logActivity(
    ...
    if (CLOSED.includes(l.status))
      throw new CrmValidationError(`lead ${l.leadNumber} is "${l.status}" — reopen it first`);
    const at = activityDate ? new Date(activityDate) : new Date();
    if (Number.isNaN(at.getTime()))
      throw new CrmValidationError('activityDate is not a valid date');
    await this.repository.insertActivity(id, activityType, clean(note), at);
    if (activityType === 'note') return this.getLead(id);
    const last =
      l.lastContactedAt && new Date(l.lastContactedAt) > at ? new Date(l.lastContactedAt) : at;
    return this.repository.updateLead(id, {
  ...
  async act(id: string, action: LeadAction, reason?: string): Promise<LeadRecord> {
    ...
    if (!ACTIONS_FROM[action].includes(l.status)) {
      const hint =
        action === 'interested' && l.status === 'new'
          ? ' — لازم يتسجّل تواصل فعلي (مكالمة/زيارة/إيميل) الأول'
          : '';
      throw new CrmValidationError(
        `cannot "${action}" lead ${l.leadNumber} while it is "${l.status}"${hint}`,
      );
    }
    if (action === 'lost') {
    ...
    if (action === 'reopen') {
      return this.repository.updateLead(id, {
        status: l.lastContactedAt ? 'contacted' : 'new',
        lostReason: null,
      });
    }
  ...
  async listProspects(): Promise<ProspectRecord[]> {
    return this.repository.listProspects();
  }
  ...
  async createProspect(input: CreateProspectInput): Promise<ProspectRecord> {
    ...
    if (ids.length === 0)
      throw new CrmValidationError('a prospect needs at least one interested lead');
    ...
    await this.repository.insertProspect({
      id,
      prospectNumber: `PRS-${new Date().getFullYear()}-${String(n).padStart(6, '0')}`,
      companyName,
  ...
  async convertProspect(
    id: string,
    input: { code: string; creditLimit?: string | null },
    ...
    const customer = await this.crm.createCustomer({
      ...
      contactPhone: leads.find((l) => l.phone)?.phone ?? undefined,
      contactEmail: leads.find((l) => l.email)?.email ?? undefined,
    });
    await this.repository.updateLeads(p.leadIds, { status: 'converted', customerId: customer.id });
    return {
      prospect: await this.repository.updateProspect(id, {
        status: 'converted',
  ...
  async convertLead(
    ...
    if (l.prospectId)
      throw new CrmValidationError(
        `lead ${l.leadNumber} belongs to a prospect — convert the prospect instead`,
      );
    if (l.status !== 'interested')
      throw new CrmValidationError(
        `only an interested lead can become a customer (lead ${l.leadNumber} is "${l.status}")`,
      );
    const customer = await this.crm.createCustomer({
      code: input.code,
      name: l.companyName ?? l.personName,
      orgNodeId: l.orgNodeId,
      status: 'active',
      creditLimit: input.creditLimit ?? null,
      contactPhone: l.phone ?? undefined,
      contactEmail: l.email ?? undefined,
    });
    return {
      lead: await this.repository.updateLead(id, { status: 'converted', customerId: customer.id }),
      customer,
    };
  }
  ...
  async loseProspect(id: string, reason: string): Promise<ProspectRecord> {
    const p = await this.openProspect(id);
    if (!clean(reason))
      throw new CrmValidationError('a reason is required to mark a prospect lost');
  ...
  private async openProspect(id: string): Promise<ProspectRecord> {
    ...
    if (p.status !== 'open')
      throw new CrmValidationError(`prospect ${p.prospectNumber} is already "${p.status}"`);
    return p;
  }
  ...
  private async mustBeInterested(leadId: string): Promise<LeadRecord> {
    ...
    if (l.status !== 'interested' || l.prospectId) {
      throw new CrmValidationError(
        `lead ${l.leadNumber} must be "interested" and not already in a prospect (it is "${l.status}")`,
      );
    }
    return l;
  }

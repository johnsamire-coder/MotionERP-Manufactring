...
const MANUAL_MOVES: Record<OpportunityStage, OpportunityStage[]> = {
  open: ['qualified', 'lost'], qualified: ['open', 'lost'], quoted: ['lost'], won: [], lost: ['open'],
};
...
export class OpportunityService {
  ...
  async list(customerId?: string): Promise<OpportunityRecord[]> { return this.repository.list(customerId); }
  ...
  async create(input: CreateOpportunityInput): Promise<OpportunityRecord> {
    ...
    if (!Number.isInteger(p) || p < 0 || p > 100) throw new CrmValidationError('probability must be 0–100');
    ...
    return this.repository.insert({ ...input, title: input.title.trim(), id: randomUUID(), opportunityNumber });
  }

  async moveStage(id: string, stage: OpportunityStage, lostReason?: string): Promise<OpportunityRecord> {
    const opp = await this.get(id);
    if (!MANUAL_MOVES[opp.stage].includes(stage)) {
      throw new CrmValidationError(`cannot move opportunity ${opp.opportunityNumber} from "${opp.stage}" to "${stage}"`);
    }
    if (stage === 'lost' && !lostReason?.trim()) throw new CrmValidationError('lostReason is required to mark an opportunity lost');
    return this.repository.update(id, { stage, lostReason: stage === 'lost' ? lostReason!.trim() : null });
  }
  ...
  async markQuoted(id: string, quotationId: string): Promise<OpportunityRecord> {
    ...
    if (opp.stage !== 'open' && opp.stage !== 'qualified') {
      throw new CrmValidationError(`opportunity ${opp.opportunityNumber} is "${opp.stage}" and cannot be quoted`);
    }
    return this.repository.update(id, { stage: 'quoted', quotationId, probability: Math.max(opp.probability, 50) });
  }

...
export class OpportunityRepository {
  ...
  async list(customerId?: string): Promise<OpportunityRecord[]> {
    ...
    const rows = customerId
      ? await q.where(eq(opportunity.customerId, customerId)).orderBy(asc(opportunity.createdAt))
      : await q.orderBy(asc(opportunity.createdAt));
  ...
  async findByQuotation(quotationId: string): Promise<OpportunityRecord | null> {
    const rows = await this.database.db
      ...
      .from(opportunity)
      .where(eq(opportunity.quotationId, quotationId))
      .limit(1);
    return rows[0] ? this.findById(rows[0].id) : null;
  }

  async insert(
    input: CreateOpportunityInput & { id: string; opportunityNumber: string },
    ...
    if (input.items && input.items.length > 0) {
      await this.database.db.insert(opportunityItem).values(
        input.items.map((i) => ({
          opportunityId: input.id,
          itemId: i.itemId,
          quantity: i.quantity,
          expectedRate: i.expectedRate ?? null,
        })),
      );
    }

...
const supplierColumns = {
  ...
  orgNodeId: supplier.orgNodeId,
  status: supplier.status,
  holdType: supplier.holdType,
  holdReason: supplier.holdReason,
  holdReleaseDate: supplier.holdReleaseDate,
...
const interactionColumns = {
  ...
  customerId: customerInteraction.customerId,
  interactionType: customerInteraction.interactionType,
  interactionDate: customerInteraction.interactionDate,
  note: customerInteraction.note,
  createdAt: customerInteraction.createdAt,
};
...
function toSupplierRecord(row: SupplierRow): SupplierRecord {
  return {
    ...
    holdType: row.holdType as SupplierHoldType | null,
...
export class CrmRepository {
  ...
  async setCustomerGroup(id: string, customerGroupId: string | null): Promise<CustomerRecord> {
    const rows = await this.database.db
      .update(customer)
      .set({ customerGroupId, updatedAt: new Date() })
      .where(eq(customer.id, id))
      .returning(customerColumns);
    return toCustomerRecord(rows[0]!);
  }
  async setSupplierGroup(id: string, supplierGroupId: string | null): Promise<SupplierRecord> {
    const rows = await this.database.db
      .update(supplier)
      .set({ supplierGroupId, updatedAt: new Date() })
      .where(eq(supplier.id, id))
      .returning(supplierColumns);
    return toSupplierRecord(rows[0]!);
  }
  ...
  async listInteractions(customerId?: string): Promise<CustomerInteractionRecord[]> {
    const rows = customerId
      ? await this.database.db
          ...
          .where(eq(customerInteraction.customerId, customerId))
          .orderBy(asc(customerInteraction.interactionDate))
      : await this.database.db
          .select(interactionColumns)
          .from(customerInteraction)
          .orderBy(asc(customerInteraction.interactionDate));
  ...
  async insertInteraction(
    input: CreateInteractionInput & { id: string },
  ): Promise<CustomerInteractionRecord> {
    const rows = await this.database.db

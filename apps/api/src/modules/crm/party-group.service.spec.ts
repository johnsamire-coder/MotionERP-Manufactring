...
const g = (
  ...
  isGroup: boolean,
  defaultCreditLimit: string | null = null,
): PartyGroupRecord => ({
  id,
  groupType: 'customer',
  code: id.toUpperCase(),
...
describe('Customer / supplier group trees (plan item 28)', () => {
  ...
  const groups = [
    g('all', null, true, '50000.0000'),
    g('retail', 'all', true),
    g('cairo', 'retail', false),
    g('wholesale', 'all', false, '200000.0000'),
  ];
  ...
  describe('service rules', () => {
    ...
    beforeEach(() => {
      ...
      jest
        .spyOn(service, 'get')
        .mockImplementation(async (id: string) => groups.find((x) => x.id === id)!);
      setGroup = jest.fn(async (id: string, groupId: string | null) => ({
        id,
        customerGroupId: groupId,
      }));
      (service as unknown as { crm: Partial<CrmRepository> }).crm = {
        findCustomerById: jest.fn(async () => ({ id: 'c' }) as CustomerRecord),
        setCustomerGroup: setGroup,
      };
    });
    ...
    it('3. moving a group under its own descendant is refused', async () => {
      ...
      await expect(service.update('retail', { parentGroupId: 'wholesale' })).rejects.toThrow(
        /مش مجموعة أب/,
      );
    });

    it('4. credit limit: own first, else the nearest group up the tree', async () => {
      const c = (creditLimit: string | null, customerGroupId: string | null): CustomerRecord =>
        ({ creditLimit, customerGroupId }) as CustomerRecord;
      expect(await service.effectiveCreditLimit(c('1000', 'cairo'))).toEqual({
        limit: '1000',
        source: 'customer',
      });
      expect(await service.effectiveCreditLimit(c(null, 'cairo'))).toEqual({
        limit: '50000.0000',
        source: 'group',
        groupCode: 'ALL',
      });
      expect(await service.effectiveCreditLimit(c(null, 'wholesale'))).toMatchObject({
        limit: '200000.0000',
        groupCode: 'WHOLESALE',
      });
      expect(await service.effectiveCreditLimit(c(null, null))).toEqual({
        limit: null,
        source: null,
      });
    });

...
const doColumns = {
  id: deliveryOrder.id,
  jobOrderReference: deliveryOrder.jobOrderReference,
  orgNodeId: deliveryOrder.orgNodeId,
  deliveryNumber: deliveryOrder.deliveryNumber,
  scheduledDate: deliveryOrder.scheduledDate,
  actualDate: deliveryOrder.actualDate,
...
const instColumns = {
  ...
  actualStartDate: installation.actualStartDate,
  actualEndDate: installation.actualEndDate,
  status: installation.status,
  technicianNames: installation.technicianNames,
...
const drColumns = {
  ...
  deliveryOrderId: deliveryReceipt.deliveryOrderId,
  receiptNumber: deliveryReceipt.receiptNumber,
  signedBy: deliveryReceipt.signedBy,
  signatureImage: deliveryReceipt.signatureImage,
  receivedItems: deliveryReceipt.receivedItems,
  notes: deliveryReceipt.notes,
  signedAt: deliveryReceipt.signedAt,
};

const irColumns = {
  id: installationReport.id,
  installationId: installationReport.installationId,
  reportNumber: installationReport.reportNumber,
  performedBy: installationReport.performedBy,
  verifiedBy: installationReport.verifiedBy,
  completionNotes: installationReport.completionNotes,
  issuesFound: installationReport.issuesFound,
  correctiveActions: installationReport.correctiveActions,
  verifiedAt: installationReport.verifiedAt,
};
...
function toDoRecord(row: DoRow): DeliveryOrderRecord {
  return { ...row, status: row.status as DeliveryStatus };
}
function toInstRecord(row: InstRow): InstallationRecord {
  return { ...row, status: row.status as InstallationStatus };
}
function toDrRecord(row: DrRow): DeliveryReceiptRecord {
  return row;
}
function toIrRecord(row: IrRow): InstallationReportRecord {
  return row;
}
...
export class DeliveryRepository {
  ...
  async findDeliveryOrderByNumber(deliveryNumber: string): Promise<DeliveryOrderRecord | null> {
    const rows = await this.database.db
      .select(doColumns)
      .from(deliveryOrder)
      .where(eq(deliveryOrder.deliveryNumber, deliveryNumber))
      .limit(1);
    return rows[0] ? toDoRecord(rows[0]) : null;
  }
  ...
  async findDeliveryOrdersByJobOrder(jobOrderReference: string): Promise<DeliveryOrderRecord[]> {
    const rows = await this.database.db
      .select(doColumns)
      .from(deliveryOrder)
      .where(eq(deliveryOrder.jobOrderReference, jobOrderReference))
      .orderBy(asc(deliveryOrder.scheduledDate));
    return rows.map(toDoRecord);
  }

  async insertDeliveryOrder(
  ...
  async findInstallationsByDeliveryOrder(deliveryOrderId: string): Promise<InstallationRecord[]> {
    const rows = await this.database.db
      .select(instColumns)
      .from(installation)
      .where(eq(installation.deliveryOrderId, deliveryOrderId))
      .orderBy(asc(installation.scheduledDate));
    return rows.map(toInstRecord);
  }

  async insertInstallation(
    input: CreateInstallationInput & { id: string },
    ...
    const rows = await this.database.db
      ...
      .values({
        ...
        scheduledDate: input.scheduledDate,
        technicianNames: input.technicianNames,
  ...
  async insertDeliveryReceipt(
    input: CreateDeliveryReceiptInput & { id: string; receiptNumber: string },
  ...
  async insertInstallationReport(
    input: CreateInstallationReportInput & { id: string; reportNumber: string },

...
export const deliveryOrder = deliverySchema.table(
  'delivery_order',
  {
  ...
  (t) => [
    check(
      'delivery_order_status_valid',
      sql`${t.status} in ('scheduled', 'in_transit', 'delivered', 'cancelled')`,
    ),
...
export const installation = deliverySchema.table(
  'installation',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const deliveryReceipt = deliverySchema.table(
  'delivery_receipt',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    deliveryOrderId: uuid('delivery_order_id')
...
export const installationReport = deliverySchema.table(
  'installation_report',
  {
    id: uuid('id').primaryKey().defaultRandom(),

...
export const collection = financeSchema.table(
  'collection',
  {
    ...
    receivedInAccountId: uuid('received_in_account_id').references(() => chartOfAccounts.id, {
      onDelete: 'restrict',
    }),
  ...
  (t) => [
    check(
      'collection_payment_method_valid',
      sql`${t.paymentMethod} in ('cash', 'bank_transfer', 'check', 'credit_card')`,
    ),
...
export const retention = financeSchema.table(
  'retention',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    jobOrderReference: text('job_order_reference').notNull(),
  ...
  (t) => [
    check(
      'retention_status_valid',
      sql`${t.status} in ('active', 'released', 'expired', 'cancelled')`,
    ),
    check(
      'retention_amounts_valid',
      sql`${t.originalAmount} > 0 AND ${t.releasedAmount} >= 0 AND ${t.releasedAmount} <= ${t.originalAmount}`,
    ),
...
export const purchaseInvoice = financeSchema.table(
  'purchase_invoice',
  {
    id: uuid('id').primaryKey().defaultRandom(),
 ...
 */
export const purchaseInvoiceHold = financeSchema.table(
  'purchase_invoice_hold',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const purchaseInvoiceLine = financeSchema.table(
...
export const salesInvoice = financeSchema.table(
  'sales_invoice',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const salesInvoiceLine = financeSchema.table(
  'sales_invoice_line',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const payment = financeSchema.table(
  'payment',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    purchaseInvoiceId: uuid('purchase_invoice_id').references(() => purchaseInvoice.id, {
      onDelete: 'set null',
    }),
...
export const creditDebitNote = financeSchema.table(
  'credit_debit_note',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const creditDebitNoteLine = financeSchema.table(
  'credit_debit_note_line',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    noteId: uuid('note_id')
      .notNull()
  ...
  (t) => [index('idx_note_line_note').on(t.noteId), index('idx_note_line_item').on(t.itemId)],
);

// ==================== 6. التحويلات البنكية ومطابقة كشوف الحسابات (الجديد) ====================

export const bankTransfer = financeSchema.table(
  'bank_transfer',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const bankReconciliation = financeSchema.table(
  'bank_reconciliation',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    clearedBalance: numeric('cleared_balance', { precision: 14, scale: 4 })
      .notNull()
      .default('0.0000'),
    differenceAmount: numeric('difference_amount', { precision: 14, scale: 4 })
      .notNull()
      .default('0.0000'),
  ...
  (t) => [
    check(
      'bank_reconciliation_status_valid',
      sql`${t.status} in ('draft', 'reconciled', 'cancelled')`,
    ),
 ...
 */
export const ledgerHealth = financeSchema.table(
  'ledger_health',
  {
    id: uuid('id').primaryKey().defaultRandom(),
  ...
  (t) => [
    check(
      'ledger_health_check_type_valid',
      sql`${t.checkType} in ('debit_credit_mismatch', 'missing_gl_entry', 'orphan_gl_entry')`,
    ),
    uniqueIndex('ledger_health_open_issue_unique')
      .on(t.issueKey)
...
export const advanceAllocation = financeSchema.table(
  'advance_allocation',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    partyType: text('party_type').notNull(),
  ...
  (t) => [
    ...
    check(
      'advance_allocation_customer_shape',
      sql`${t.partyType} <> 'customer' or (${t.collectionId} is not null and ${t.salesInvoiceId} is not null)`,
    ),
    check(
      'advance_allocation_supplier_shape',
      sql`${t.partyType} <> 'supplier' or (${t.paymentId} is not null and ${t.purchaseInvoiceId} is not null)`,
    ),
...
export const invoiceInstallment = financeSchema.table(
  'invoice_installment',
  {
    id: uuid('id').primaryKey().defaultRandom(),
  ...
  (t) => [
    ...
    check(
      'invoice_installment_shape',
      sql`(${t.invoiceType} = 'sales' and ${t.salesInvoiceId} is not null and ${t.purchaseInvoiceId} is null) or (${t.invoiceType} = 'purchase' and ${t.purchaseInvoiceId} is not null and ${t.salesInvoiceId} is null)`,
    ),
    ...
    unique('invoice_installment_purchase_number_unique').on(
      t.purchaseInvoiceId,
      t.installmentNumber,
    ),
  ],

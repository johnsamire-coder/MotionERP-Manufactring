...
import type { BankReconciliationStatus, CreditDebitNoteType } from './finance.types';

const colColumns = {
  id: collection.id,
  jobOrderReference: collection.jobOrderReference,
  orgNodeId: collection.orgNodeId,
  collectionNumber: collection.collectionNumber,
  collectionDate: collection.collectionDate,
  amount: collection.amount,
  currencyCode: collection.currencyCode,
  paymentMethod: collection.paymentMethod,
  receivedInAccountId: collection.receivedInAccountId,
...
const retColumns = {
  id: retention.id,
  jobOrderReference: retention.jobOrderReference,
  orgNodeId: retention.orgNodeId,
  retentionNumber: retention.retentionNumber,
  originalAmount: retention.originalAmount,
  releasedAmount: retention.releasedAmount,
  currencyCode: retention.currencyCode,
  startDate: retention.startDate,
  releaseDate: retention.releaseDate,
  dueDate: retention.dueDate,
...
const piColumns = {
  ...
  systemNumber: purchaseInvoice.systemNumber,
  orgNodeId: purchaseInvoice.orgNodeId,
...
const pilColumns = {
  ...
  purchaseInvoiceId: purchaseInvoiceLine.purchaseInvoiceId,
  itemId: purchaseInvoiceLine.itemId,
  quantity: purchaseInvoiceLine.quantity,
  unitCost: purchaseInvoiceLine.unitCost,
  taxRate: purchaseInvoiceLine.taxRate,
  taxAmount: purchaseInvoiceLine.taxAmount,
  totalAmount: purchaseInvoiceLine.totalAmount,
  purchaseReceiptId: purchaseInvoiceLine.purchaseReceiptId,
  createdAt: purchaseInvoiceLine.createdAt,
};

const siColumns = {
  id: salesInvoice.id,
  invoiceNumber: salesInvoice.invoiceNumber,
  orgNodeId: salesInvoice.orgNodeId,
  customerId: salesInvoice.customerId,
  jobOrderReference: salesInvoice.jobOrderReference,
  ...
  netAmount: salesInvoice.netAmount,
...
const silColumns = {
  ...
  salesInvoiceId: salesInvoiceLine.salesInvoiceId,
  itemId: salesInvoiceLine.itemId,
  quantity: salesInvoiceLine.quantity,
  unitPrice: salesInvoiceLine.unitPrice,
  taxRate: salesInvoiceLine.taxRate,
  taxAmount: salesInvoiceLine.taxAmount,
  totalAmount: salesInvoiceLine.totalAmount,
  deliveryOrderId: salesInvoiceLine.deliveryOrderId,
  createdAt: salesInvoiceLine.createdAt,
};

const payColumns = {
  id: payment.id,
  paymentNumber: payment.paymentNumber,
  orgNodeId: payment.orgNodeId,
...
const noteColumns = {
  id: creditDebitNote.id,
  noteNumber: creditDebitNote.noteNumber,
  noteType: creditDebitNote.noteType,
  orgNodeId: creditDebitNote.orgNodeId,
  partyType: creditDebitNote.partyType,
  partyId: creditDebitNote.partyId,
  originalInvoiceNumber: creditDebitNote.originalInvoiceNumber,
  salesInvoiceId: creditDebitNote.salesInvoiceId,
  purchaseInvoiceId: creditDebitNote.purchaseInvoiceId,
  postingDate: creditDebitNote.postingDate,
  netAmount: creditDebitNote.netAmount,
  taxAmount: creditDebitNote.taxAmount,
  grandTotal: creditDebitNote.grandTotal,
  reason: creditDebitNote.reason,
  status: creditDebitNote.status,
  createdAt: creditDebitNote.createdAt,
  updatedAt: creditDebitNote.updatedAt,
};

const noteLineColumns = {
  id: creditDebitNoteLine.id,
  noteId: creditDebitNoteLine.noteId,
  itemId: creditDebitNoteLine.itemId,
  quantity: creditDebitNoteLine.quantity,
  unitPrice: creditDebitNoteLine.unitPrice,
  taxRate: creditDebitNoteLine.taxRate,
  taxAmount: creditDebitNoteLine.taxAmount,
  totalAmount: creditDebitNoteLine.totalAmount,
  createdAt: creditDebitNoteLine.createdAt,
};

const transferColumns = {
  id: bankTransfer.id,
  transferNumber: bankTransfer.transferNumber,
  orgNodeId: bankTransfer.orgNodeId,
  fromAccountId: bankTransfer.fromAccountId,
  toAccountId: bankTransfer.toAccountId,
...
const reconColumns = {
  ...
  reconciliationNumber: bankReconciliation.reconciliationNumber,
  orgNodeId: bankReconciliation.orgNodeId,
  bankAccountId: bankReconciliation.bankAccountId,
  statementDate: bankReconciliation.statementDate,
  statementBalance: bankReconciliation.statementBalance,
  clearedBalance: bankReconciliation.clearedBalance,
  differenceAmount: bankReconciliation.differenceAmount,
  status: bankReconciliation.status,
  notes: bankReconciliation.notes,
  createdAt: bankReconciliation.createdAt,
  updatedAt: bankReconciliation.updatedAt,
};
...
export class FinanceRepository {
  ...
  async listCollections(jobOrderReference?: string): Promise<CollectionRecord[]> {
    const rows = jobOrderReference
      ? await this.database.db
          .select(colColumns)
          .from(collection)
          .where(eq(collection.jobOrderReference, jobOrderReference))
          .orderBy(asc(collection.collectionDate))
      : await this.database.db
          .select(colColumns)
          .from(collection)
          .orderBy(asc(collection.collectionDate));
    return rows.map((r) => ({
      ...
      receivedInAccountId: r.receivedInAccountId,
  ...
  async listRetentions(jobOrderReference?: string): Promise<RetentionRecord[]> {
    const rows = jobOrderReference
      ? await this.database.db
          .select(retColumns)
          .from(retention)
          .where(eq(retention.jobOrderReference, jobOrderReference))
          .orderBy(asc(retention.dueDate))
      : await this.database.db.select(retColumns).from(retention).orderBy(asc(retention.dueDate));
    return rows.map((r) => ({
      id: r.id,
      jobOrderReference: r.jobOrderReference,
      orgNodeId: r.orgNodeId,
      retentionNumber: r.retentionNumber,
  ...
  async insertRetention(
    input: CreateRetentionInput & { id: string; retentionNumber: string; orgNodeId: string | null },
    ...
    const rows = await this.database.db
      ...
      .values({
        ...
        jobOrderReference: input.jobOrderReference,
        orgNodeId: input.orgNodeId,
        retentionNumber: input.retentionNumber,
  ...
  async listPurchaseInvoices(
    ...
    for (const r of rows) {
      const lines = await this.database.db
        ...
        .where(eq(purchaseInvoiceLine.purchaseInvoiceId, r.id));
      results.push({
        id: r.id,
  ...
  async setPurchaseInvoiceStatus(
    id: string,
  ...
  async listSalesInvoices(orgNodeId?: string, customerId?: string): Promise<SalesInvoiceRecord[]> {
    ...
    for (const r of rows) {
      const lines = await this.database.db
        ...
        .where(eq(salesInvoiceLine.salesInvoiceId, r.id));
      results.push({
        id: r.id,
        invoiceNumber: r.invoiceNumber,
        orgNodeId: r.orgNodeId,
  ...
  async insertPayment(
    input: CreatePaymentInput & { id: string; paymentNumber: string },
  ...
  async listCreditDebitNotes(
    ...
    for (const r of rows) {
      const lines = await this.database.db
        .select(noteLineColumns)
        .from(creditDebitNoteLine)
        .where(eq(creditDebitNoteLine.noteId, r.id));
      results.push({
        id: r.id,
        noteNumber: r.noteNumber,
        noteType: r.noteType as CreditDebitNoteType,
        orgNodeId: r.orgNodeId,
        partyType: r.partyType as 'customer' | 'supplier',
        partyId: r.partyId,
        ...
        grandTotal: r.grandTotal,
        reason: r.reason,
        status: r.status as CreditDebitNoteStatus,
        createdAt: r.createdAt.toISOString(),
        updatedAt: r.updatedAt.toISOString(),
        lines: lines.map((l) => ({
          id: l.id,
  ...
  async findCreditDebitNoteById(id: string): Promise<CreditDebitNoteRecord | null> {
    const rows = await this.database.db
      ...
      .where(eq(creditDebitNote.id, id))
      .limit(1);
    if (!rows[0]) return null;
    const r = rows[0];
    const lines = await this.database.db
  ...
  async insertCreditDebitNote(
    ...
    return {
      ...
      noteType: insertedNote.noteType as CreditDebitNoteType,
      orgNodeId: insertedNote.orgNodeId,
      partyType: insertedNote.partyType as 'customer' | 'supplier',
      ...
      status: insertedNote.status as CreditDebitNoteStatus,
  ...
  async setCreditDebitNoteStatus(
    id: string,
  ...
  async listBankTransfers(orgNodeId?: string): Promise<BankTransferRecord[]> {
    ...
    const rows = orgNodeId
      ? await query
          .where(eq(bankTransfer.orgNodeId, orgNodeId))
          .orderBy(desc(bankTransfer.transferDate))
  ...
  async insertBankTransfer(
    input: CreateBankTransferInput & { id: string; transferNumber: string },
  ...
  async sumBilledForReceipt(purchaseReceiptId: string): Promise<number> {
    const rows = await this.database.db
      .select({
        total: sql<string>`coalesce(sum(${purchaseInvoiceLine.quantity} * ${purchaseInvoiceLine.unitCost}), 0)`,
      })
      .from(purchaseInvoiceLine)
      .innerJoin(purchaseInvoice, eq(purchaseInvoice.id, purchaseInvoiceLine.purchaseInvoiceId))
      .where(
        and(
          eq(purchaseInvoiceLine.purchaseReceiptId, purchaseReceiptId),
          ne(purchaseInvoice.status, 'cancelled'),
        ),
      );
  ...
  async findPurchaseInvoiceBySupplierNumber(
    supplierId: string,
    invoiceNumber: string,
  ): Promise<{ systemNumber: string; invoiceDate: Date; status: string } | null> {
    const rows = await this.database.db
      .select({
        systemNumber: purchaseInvoice.systemNumber,
        invoiceDate: purchaseInvoice.invoiceDate,
      ...
      .where(
        and(
          eq(purchaseInvoice.supplierId, supplierId),
          eq(purchaseInvoice.invoiceNumber, invoiceNumber),
        ),
      )
      .limit(1);
    return rows[0] ?? null;

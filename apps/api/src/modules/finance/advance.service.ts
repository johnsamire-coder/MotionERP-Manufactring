...
import { advanceAllocation, collection, payment, purchaseInvoice, salesInvoice } from './finance.schema';

export interface AdvanceAccounts { receivedAccountId: string | null; paidAccountId: string | null; }
export interface OpenAdvance {
  partyType: 'customer' | 'supplier'; documentId: string; documentNumber: string; partyReference: string | null;
  advance: string; allocated: string; remaining: string;
}

/** How much of a customer receipt settles invoices and how much is an advance (pure). */
export function splitReceipt(amount: number, invoiced: number, alreadyCovered: number): { settle: number; advance: number } {
...
const sum = (rows: Array<{ v: string | null }>): number => rows.reduce((s, r) => s + Number(r.v ?? 0), 0);
...
export class AdvanceService {
  ...
  async accounts(orgNodeId: string | null): Promise<AdvanceAccounts | null> {
    ...
    return { receivedAccountId: c.defaultAdvanceReceivedAccountId ?? null, paidAccountId: c.defaultAdvancePaidAccountId ?? null };
  }

  /** Split for a new collection against a job order: what posted invoices still leave open is settled, the rest is advance. */
  async splitCollection(jobOrderReference: string, amount: number): Promise<{ settle: number; advance: number }> {
    const db = this.database.db;
    const invoices = await db.select({ id: salesInvoice.id, v: salesInvoice.grandTotal }).from(salesInvoice)
      .where(and(eq(salesInvoice.jobOrderReference, jobOrderReference), eq(salesInvoice.status, 'posted')));
    const settledBefore = await db.select({ v: sql<string>`${collection.amount} - ${collection.advanceAmount}` }).from(collection)
      .where(and(eq(collection.jobOrderReference, jobOrderReference), ne(collection.status, 'cancelled')));
    let allocated = 0;
    for (const inv of invoices) allocated += sum(await db.select({ v: advanceAllocation.amount }).from(advanceAllocation).where(eq(advanceAllocation.salesInvoiceId, inv.id)));
  ...
  async setCollectionAdvance(id: string, advance: number): Promise<void> {
    await this.database.db.update(collection).set({ advanceAmount: advance.toFixed(4) }).where(eq(collection.id, id));
  }

  async setPaymentAdvance(id: string, advance: number): Promise<void> {
    await this.database.db.update(payment).set({ advanceAmount: advance.toFixed(4) }).where(eq(payment.id, id));
  }
  ...
  async allocateCollection(collectionId: string, salesInvoiceId: string, amountRaw: string): Promise<{ allocationId: string; journalEntryId: string | null }> {
    ...
    if (!col || col.status === 'cancelled') throw new FinanceNotFoundError(`collection ${collectionId} does not exist`);
    const inv = (await db.select().from(salesInvoice).where(eq(salesInvoice.id, salesInvoiceId)).limit(1))[0];
    ...
    if (acc?.receivedAccountId && ar && col.orgNodeId && this.accounting) {
      const e = await this.accounting.createEntry({
        orgNodeId: col.orgNodeId, description: `[Auto] تسوية دفعة مقدمة ${col.collectionNumber} على الفاتورة ${inv.invoiceNumber}`,
        reference: col.collectionNumber, isAutoGenerated: true, sourceEventType: 'advance_allocation',
        idempotencyKey: `advance-alloc-${col.id}-${inv.id}-${Date.now()}`,
        lines: [
          { accountId: acc.receivedAccountId, debitAmount: amount.toFixed(4), creditAmount: '0', partyType: 'customer', partyId: inv.customerId ?? undefined, description: '[Auto] Advance applied' },
          { accountId: ar, debitAmount: '0', creditAmount: amount.toFixed(4), partyType: 'customer', partyId: inv.customerId ?? undefined, description: '[Auto] AR settled by advance' },
        ],
      });
  ...
  async allocatePayment(paymentId: string, purchaseInvoiceId: string, amountRaw: string): Promise<{ allocationId: string; journalEntryId: string | null }> {
    ...
    const inv = (await db.select().from(purchaseInvoice).where(eq(purchaseInvoice.id, purchaseInvoiceId)).limit(1))[0];
    ...
    if (amount > Number(inv.grandTotal) - covered + 0.0001) throw new FinanceValidationError(`الفاتورة ${inv.systemNumber} متبقي منها ${(Number(inv.grandTotal) - covered).toFixed(4)} بس`);
    ...
    if (acc?.paidAccountId && ap && this.accounting) {
      const e = await this.accounting.createEntry({
        orgNodeId: p.orgNodeId, description: `[Auto] تسوية دفعة مقدمة ${p.paymentNumber} على الفاتورة ${inv.systemNumber}`,
        reference: p.paymentNumber, isAutoGenerated: true, sourceEventType: 'advance_allocation',
        idempotencyKey: `advance-alloc-${p.id}-${inv.id}-${Date.now()}`,
        lines: [
          { accountId: ap, debitAmount: amount.toFixed(4), creditAmount: '0', partyType: 'supplier', partyId: inv.supplierId ?? undefined, description: '[Auto] AP settled by advance' },
          { accountId: acc.paidAccountId, debitAmount: '0', creditAmount: amount.toFixed(4), partyType: 'supplier', partyId: inv.supplierId ?? undefined, description: '[Auto] Advance applied' },
        ],
      });
      journalEntryId = (await this.accounting.postEntry(e.id)).id;
    }
  ...
  private open(partyType: 'customer' | 'supplier', id: string, number: string, ref: string | null, advance: number, allocated: number): OpenAdvance {
    return { partyType, documentId: id, documentNumber: number, partyReference: ref, advance: advance.toFixed(4), allocated: allocated.toFixed(4), remaining: (advance - allocated).toFixed(4) };
  }
  ...
  private async partyAccount(orgNodeId: string | null, purpose: 'receivable' | 'payable'): Promise<string | null> {
    if (!orgNodeId || !this.accountingRepo) return null;
    const det = (await this.accountingRepo.listAccountDeterminations(orgNodeId)).find((d) => d.accountPurpose === purpose);
    const c = await this.accountingRepo.findCompanyConfig(orgNodeId);
    return det?.accountId ?? (purpose === 'receivable' ? c?.defaultReceivableAccountId : c?.defaultPayableAccountId) ?? null;
  }

  private async sameCustomer(collectionJo: string, invoiceCustomerId: string | null, invoiceJo: string | null): Promise<void> {
    if (invoiceJo && invoiceJo === collectionJo) return;
    const jo = this.sales ? (await this.sales.getJobOrders()).find((j) => j.jobOrderNumber === collectionJo) : undefined;
    if (!jo || jo.customerId !== invoiceCustomerId) throw new FinanceValidationError('الدفعة المقدمة والفاتورة لعملاء مختلفين');
  }
}

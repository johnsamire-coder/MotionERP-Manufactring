...
export class AllocationController {
  ...
  async salesOutstanding(
    @Param('id', ParseUUIDPipe) id: string,
  ): Promise<{ outstanding: InvoiceOutstanding }> {
    return { outstanding: await this.service.outstanding('sales', id) };
  }

  @Get('purchase-invoices/:id/outstanding')
  async purchaseOutstanding(
    @Param('id', ParseUUIDPipe) id: string,
  ): Promise<{ outstanding: InvoiceOutstanding }> {
    return { outstanding: await this.service.outstanding('purchase', id) };
  }

  @Put('sales-invoices/:id/installments')
  async salesInstallments(
    @Param('id', ParseUUIDPipe) id: string,
  ...
  @Post('collections/:id/allocations')
  @HttpCode(200)

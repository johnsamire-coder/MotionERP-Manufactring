...
export interface PurchaseInvoiceHoldRecord {
  purchaseInvoiceId: string; reason: string; releaseDate: string | null; createdAt: string; active: boolean;
}
...
export class PurchaseInvoiceHoldService {
  ...
  async get(purchaseInvoiceId: string): Promise<PurchaseInvoiceHoldRecord | null> {
    const rows = await this.database.db.select().from(purchaseInvoiceHold)
      .where(eq(purchaseInvoiceHold.purchaseInvoiceId, purchaseInvoiceId)).limit(1);
    const r = rows[0];
    if (!r) return null;
    const active = !r.releaseDate || r.releaseDate.getTime() > Date.now();
    return {
      purchaseInvoiceId: r.purchaseInvoiceId, reason: r.reason, releaseDate: r.releaseDate ? r.releaseDate.toISOString() : null,
      createdAt: r.createdAt.toISOString(), active,
    };
  }

  async hold(purchaseInvoiceId: string, reason: string, releaseDate?: string): Promise<PurchaseInvoiceHoldRecord> {
    const invoice = await this.finance.findPurchaseInvoiceById(purchaseInvoiceId);
    if (!invoice) throw new FinanceNotFoundError(`purchase invoice ${purchaseInvoiceId} does not exist`);
    if (invoice.status === 'cancelled') throw new FinanceValidationError(`purchase invoice ${invoice.systemNumber} is cancelled`);
    ...
    if (releaseDate) {
      ...
      if (Number.isNaN(release.getTime())) throw new FinanceValidationError('releaseDate is not a valid date');
      if (release.getTime() <= Date.now()) throw new FinanceValidationError('releaseDate must be in the future');
    }
    const row = { reason: reason.trim(), releaseDate: release };
    await this.database.db.insert(purchaseInvoiceHold).values({ purchaseInvoiceId, ...row })
  ...
  async release(purchaseInvoiceId: string): Promise<void> {
    await this.database.db.delete(purchaseInvoiceHold).where(eq(purchaseInvoiceHold.purchaseInvoiceId, purchaseInvoiceId));
  }

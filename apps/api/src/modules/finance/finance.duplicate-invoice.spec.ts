...
describe('Duplicate supplier invoice number (plan item 18)', () => {
  it('answers 400 with the existing invoice instead of hitting the unique constraint', async () => {
    const repo = {
      findPurchaseInvoiceBySupplierNumber: jest
        .fn()
        .mockImplementation(async (_s: string, n: string) =>
          n === 'INV-7'
            ? {
                systemNumber: 'PINV-2026-000001',
    ...
    await expect(
      service.createPurchaseInvoice({ ...input, invoiceNumber: 'INV-7' }),
    ).rejects.toThrow(/INV-7.*PINV-2026-000001 بتاريخ 2026-03-01 \(posted\)/);
    await expect(
      service.createPurchaseInvoice({ ...input, invoiceNumber: 'INV-8' }),

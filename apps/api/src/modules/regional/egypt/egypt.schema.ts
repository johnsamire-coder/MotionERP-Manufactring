...
export const etaIssuerConfig = regionalEgSchema.table('eta_issuer_config', {
  id: uuid('id').primaryKey().defaultRandom(),
  orgNodeId: uuid('org_node_id').notNull(),
  rin: text('rin').notNull(),
  name: text('name').notNull(),
  activityCode: text('activity_code').notNull(),
  address: jsonb('address').$type<{ branchID: string; country: string; governate: string; regionCity: string; street: string; buildingNumber: string }>().notNull(),
...
export const etaPartyProfile = regionalEgSchema.table('eta_party_profile', {
  id: uuid('id').primaryKey().defaultRandom(),
  customerId: uuid('customer_id').notNull(),
  receiverType: text('receiver_type').notNull(),
  taxId: text('tax_id'),
  name: text('name').notNull(),
  address: jsonb('address').$type<{ country: string; governate: string; regionCity: string; street: string; buildingNumber: string }>(),
}, (t) => [
  unique('eta_party_customer_unique').on(t.customerId),
  check('eta_party_type_valid', sql`${t.receiverType} in ('B', 'P', 'F')`),
]);

export const etaItemCode = regionalEgSchema.table('eta_item_code', {
  id: uuid('id').primaryKey().defaultRandom(),
...
export const etaDocument = regionalEgSchema.table('eta_document', {
  id: uuid('id').primaryKey().defaultRandom(),
  ...
  check('eta_document_status_valid', sql`${t.status} in ('draft', 'ready', 'signed', 'submitted', 'rejected')`),
]);

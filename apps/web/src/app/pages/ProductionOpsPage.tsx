...
export function ProductionOpsPage(): JSX.Element {
  ...
  const [items, _setItems] = useState<ItemRecord[]>([]);
  const [warehouses, _setWarehouses] = useState<WarehouseRecord[]>([]);
  const [showMaterialsForStep, setShowMaterialsForStep] = useState<string | null>(null);
  const [_stepMaterials, setStepMaterials] = useState<StepMaterialRecord[]>([]);
  const [_showMatForm, setShowMatForm] = useState(false);
  const [matItemId, _setMatItemId] = useState('');
  const [matReqQty, setMatReqQty] = useState('');
  const [matConsQty, setMatConsQty] = useState('0');
  const [matWhId, _setMatWhId] = useState('');
  ...
  async function loadJoData(joNumber: string): Promise<void> {
    try {
      const [stepsRes, costRes] = await Promise.all([
        api.get<{ steps: ProductionStepRecord[] }>(
          `/production-ops/steps?jobOrderReference=${joNumber}`,
        ),
  ...
  async function _openMaterialsForStep(stepId: string): Promise<void> {
    setShowMaterialsForStep(stepId);
    setShowMatForm(false);
    try {
      const res = await api.get<{ materials: StepMaterialRecord[] }>(
        `/production-ops/steps/${stepId}/materials`,
      );
  ...
  async function _handleAddMaterial(e: React.FormEvent): Promise<void> {
    e.preventDefault();
    ...
    try {
      ...
      const res = await api.get<{ materials: StepMaterialRecord[] }>(
        `/production-ops/steps/${showMaterialsForStep}/materials`,
      );
      setStepMaterials(res.materials);
      setShowMatForm(false);
      setMatReqQty('');
  ...
  const _whLabel = (id: string | null): string =>
  ...
  async function openLogsForStep(stepId: string): Promise<void> {
    ...
    try {
      const res = await api.get<{ timeLogs: TimeLogRecord[] }>(
        `/production-ops/steps/${stepId}/time-logs`,
      );
  ...
  const woLabel = (id: string | null): string =>
    id ? (workOrders.find((w) => w.id === id)?.workOrderNumber ?? id) : '—';
    ...
    <section className="module-page">
        ...
        {costSummary && (
          ...
          >
            <div>
              ...
              <p style={{ margin: '2px 0 0', fontWeight: 'bold', fontSize: 16 }}>
                {costSummary.totalStandardMinutes} min
              </p>
            </div>
            <div>
              <span style={labelStyle}>{t('pages.production_ops.cost.actTotalMins')}</span>
              <p style={{ margin: '2px 0 0', fontWeight: 'bold', fontSize: 16, color: '#0369a1' }}>
                {costSummary.totalActualMinutes} min
              </p>
            </div>
            <div>
              <span style={labelStyle}>{t('pages.production_ops.cost.stdTotalCost')}</span>
              <p style={{ margin: '2px 0 0', fontWeight: 'bold', fontSize: 16 }}>
                {Number(costSummary.totalStandardCost ?? 0).toFixed(2)} EGP
              </p>
            </div>
            <div>
              <span style={labelStyle}>{t('pages.production_ops.cost.actTotalCost')}</span>
              <p style={{ margin: '2px 0 0', fontWeight: 'bold', fontSize: 18, color: '#166534' }}>
                {Number(costSummary.totalActualCost ?? 0).toFixed(2)} EGP
              </p>
            </div>
          </div>
        )}
      ...
      <article className="panel module-panel" style={{ marginBottom: 20 }}>
        <div className="panel__head">
          ...
          <button className="filter-button" onClick={openWcForm}>
            <b>+</b> {t('pages.production_ops.workCenters.addCenter')}
          </button>
        </div>
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              <label style={labelStyle}>
                {t('pages.production_ops.workCenters.ratePerMinute')}
              </label>
              <input
                type="number"
                min="0"
                step="any"
                value={wcCostPerMin}
                onChange={(e) => setWcCostPerMin(e.target.value)}
                required
        ...
        <div className="placeholder-table">
          ...
          {workCenters.length === 0 && (
            <p style={{ padding: '20px 0', textAlign: 'center', color: '#94a3b8' }}>
              {t('pages.production_ops.form.empty')}
            </p>
          )}
          {workCenters.map((wc) => (
            <div className="placeholder-table__row" key={wc.id}>
              ...
              <span>{wc.name}</span>
              <span>
                <b>{wc.ratePerMinute ?? '0'} EGP / min</b>
              </span>
            </div>
          ))}
        </div>
      ...
      <article className="panel module-panel">
        <div className="panel__head">
          ...
          <button className="primary-button" onClick={() => setShowStepForm((v) => !v)}>
            <b>+</b> {t('pages.production_ops.steps.addStep')}
          </button>
        </div>
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <input
                ...
                step="any"
                value={stdMins}
                onChange={(e) => setStdMins(e.target.value)}
                required
                style={{ ...inputStyle, width: 120 }}
              />
            </div>
        ...
        <div className="placeholder-table">
          ...
          {steps.map((st) => (
            <div className="placeholder-table__row" key={st.id}>
              ...
              <span>
                <b>{st.operationName}</b>
              </span>
              <span>{wcLabel(st.workCenterId)}</span>
              <span>{woLabel(st.workOrderId)}</span>
              <span>{empLabel(st.operatorEmployeeId)}</span>
              <span>
                {st.completedQuantity}
                {st.forQuantity ? ` / ${st.forQuantity}` : ''}
              </span>
              <span>
                <span
                  className={`status status--${st.status === 'done' ? 'success' : st.status === 'in_progress' ? 'warning' : 'neutral'}`}
                >
                  <i />
                  {st.status}
                </span>
                ...
                {st.status === 'in_progress' && (
                  <>
                    <button
                      ...
                      style={{ fontSize: 12, padding: '4px 8px' }}
                      onClick={() => {
                        setActualMinsInput(st.standardTimeMinutes);
                        setShowCompleteModal(st.id);
                      }}
                    >
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 10 }}>
              <button className="filter-button" onClick={() => setShowCompleteModal(null)}>
                {t('pages.production_ops.form.cancel')}
              </button>
              <button
                className="primary-button"
                disabled={submitting}
                onClick={() => {
                  void handleCompleteStep(showCompleteModal);
                }}
              >
                {t('pages.production_ops.steps.complete')}
              </button>
      ...
      {showLogsForStep && (
        ...
        >
          ...
          >
            ...
            {timeLogs.length === 0 && (
              <p style={{ color: '#94a3b8', textAlign: 'center', padding: '10px 0' }}>
                {t('pages.production_ops.steps.noTimeLogs')}
              </p>
            )}
            {timeLogs.map((log) => (
              <div
                key={log.id}
                style={{ border: '1px solid #e2e8f0', borderRadius: 6, padding: 10, fontSize: 13 }}
              >
                <div>
                  <b>{t('pages.production_ops.steps.fromTime')}:</b>{' '}
                  {new Date(log.fromTime).toLocaleString()}
                </div>
                {log.toTime && (
                  <div>
                    <b>{t('pages.production_ops.steps.toTime')}:</b>{' '}
                    {new Date(log.toTime).toLocaleString()}
                  </div>
                )}
                {log.timeInMinutes && (
                  <div>
                    <b>{t('pages.production_ops.steps.stdMins')}:</b> {log.timeInMinutes} min
                  </div>
                )}
                {log.completedQuantity && (
                  <div>
                    <b>{t('pages.production_ops.steps.completedQtyLog')}:</b>{' '}
                    {log.completedQuantity}
                  </div>
                )}
              </div>
            ...
            {!showLogForm && (
              <button className="primary-button" onClick={() => setShowLogForm(true)}>
                + {t('pages.production_ops.steps.addTimeLog')}
              </button>
            )}
              ...
              >
                <div>
                  ...
                  <input
                    type="datetime-local"
                    value={logFromTime}
                    onChange={(e) => setLogFromTime(e.target.value)}
                    required
                    style={{ ...inputStyle, width: '100%' }}
                  />
                </div>
                <div>
                  ...
                  <input
                    type="datetime-local"
                    value={logToTime}
                    onChange={(e) => setLogToTime(e.target.value)}
                    style={{ ...inputStyle, width: '100%' }}
                  />
                </div>
                <div>
                ...
                <div>
                  <label style={labelStyle}>
                    {t('pages.production_ops.steps.completedQtyLog')}
                  </label>
                  <input
                    type="number"
                    min="0"

      ...
      <article className="panel module-panel">
        <div className="panel__head">
          ...
          <button className="primary-button" onClick={() => setShowForm((v) => !v)}>
            <b>+</b>
            {t('pages.material_request.createRequest')}
          </button>
        </div>
        ...
        {showForm && (
          ...
          >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                >
                  ...
                  <option value="material_transfer">
                    {t('pages.material_request.materialTransfer')}
                  </option>
            ...
            >
              ...
              >
                {t('pages.material_request.addLine')}
              </label>
              <div
                style={{
                  display: 'flex',
        ...
        <div className="placeholder-table">
          ...
          {requests.length === 0 && (
            <p style={{ padding: '20px 0', textAlign: 'center', color: '#94a3b8' }}>
              {t('pages.material_request.noRequests')}
            </p>
          )}
          {requests.map((mr) => (
            <div className="placeholder-table__row" key={mr.id}>
              <span>
                <b>{mr.requestNumber}</b>
              </span>
              <span>{purposeLabel(mr.purpose)}</span>
              <span>{orgLabel(mr.orgNodeId)}</span>
              <span>
                <span
                  className={`status status--${mr.status === 'submitted' ? 'success' : 'neutral'}`}
                >
                  <i />
                  {mr.status}
                </span>
              ...
              <span>
                {mr.status === 'draft' && (
                  <button
                    ...
                    onClick={() => {
                      void handleSubmitRequest(mr.id);
                    }}
                  >

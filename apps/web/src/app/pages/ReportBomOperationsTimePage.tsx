...
export function ReportBomOperationsTimePage(): JSX.Element {
  ...
  return (
    <section className="module-page">
      ...
      <p style={{ fontSize: 12, color: '#b45309', background: '#fffbeb', padding: 10, borderRadius: 6 }}>{t('pages.reports.note')}</p>
      ...
      >
        <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            <select value={filterBomId} onChange={(e) => setFilterBomId(e.target.value)} style={{ ...inputStyle, minWidth: 220 }}>
              {boms.map((b) => <option key={b.id} value={b.id}>{itemLabel(b.productItemId)} — v{b.version}</option>)}
            </select>
          </div>
        </div>

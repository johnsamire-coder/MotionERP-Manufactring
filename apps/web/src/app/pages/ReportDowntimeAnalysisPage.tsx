...
export function ReportDowntimeAnalysisPage(): JSX.Element {
  ...
  const filtered = useMemo(
    () => entries.filter((e) => !filterWc || e.workCenterId === filterWc),
    [entries, filterWc],
  );
    ...
    <section className="module-page">
      ...
      <ReportLayout
        ...
        exportHeaders={[
          ...
          t('pages.reports.stopReason'),
          t('pages.reports.duration'),
        ]}
        exportRows={filtered.map((e) => [
          wcLabel(e.workCenterId),
          e.stopReason,
          e.stoppageMinutes ?? '0',
        ])}
      >
        <div style={{ display: 'flex', gap: 12, marginBottom: 16, alignItems: 'end' }}>
          ...
          <div style={{ fontSize: 13, color: '#b45309', fontWeight: 'bold' }}>
            {t('pages.reports.totalDuration')}: {totalDuration.toFixed(1)}
          </div>
        </div>

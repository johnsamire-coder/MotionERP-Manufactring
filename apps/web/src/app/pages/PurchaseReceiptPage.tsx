  ...
  async function handleSubmit(e: React.FormEvent): Promise<void> {
    ...
    try {
      await api.post('/inventory/movements', {
        ...
        quantity: qty, movementDate: new Date().toISOString(),
        note: 'Purchase Receipt from ' + (suppliers.find(s => s.id === supplierId)?.name || '') + ': ' + (note || ''),
      });
      setFormSuccess('Purchase Receipt posted! Stock updated automatically.');
      setShowForm(false); setQty('1'); setNote(''); await loadAll();
    ...
    <section className="module-page">
      <div className="page-intro">
        <div>
          ...
          <p>Record incoming goods from suppliers. Stock balance is updated automatically on submission.</p>
        </div>
        <button className="primary-button" onClick={() => setShowForm(v => !v)}>{showForm ? 'Cancel' : '+ New Purchase Receipt'}</button>
      </div>
          ...
          <form onSubmit={(e) => { void handleSubmit(e); }} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                <select value={itemId} onChange={e => setItemId(e.target.value)} style={{ ...inputStyle, width: '100%' }} required>
                  {items.map(it => <option key={it.id} value={it.id}>{it.name} ({it.code})</option>)}
                </select>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                <label style={labelStyle}>Target Warehouse</label>
                <select value={warehouseId} onChange={e => setWarehouseId(e.target.value)} style={{ ...inputStyle, width: '100%' }} required>
                  {warehouses.map(wh => <option key={wh.id} value={wh.id}>{wh.name}</option>)}
                </select>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                <label style={labelStyle}>Quantity Received</label>
                <input type="number" min="0.001" step="any" value={qty} onChange={e => setQty(e.target.value)} style={{ ...inputStyle, width: '100%' }} required />
              </div>
            </div>
            <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Receipt notes..." style={{ ...inputStyle, minHeight: 50 }} />
            <button type="submit" disabled={submitting} className="primary-button" style={{ alignSelf: 'flex-start' }}>Submit Receipt & Update Stock</button>
          </form>
        </article>
      )}
    </section>

...
interface ProductionStepRecord { id: string; operationName: string; workCenterId: string; standardTimeMinutes: string; actualTimeMinutes: string | null; status: string; }
...
export function ReportJobCardSummaryPage(): JSX.Element {
  ...
  const filtered = useMemo(
    () => steps.filter((s) => (!filterStatus || s.status === filterStatus) && (!filterWc || s.workCenterId === filterWc)),
  ...
  return (
    <section className="module-page">
      ...
      <ReportLayout
        ...
        exportHeaders={[t('pages.reports.operation'), t('pages.reports.workCenter'), t('pages.reports.standardTime'), t('pages.reports.actualTime'), t('pages.reports.status')]}
        exportRows={filtered.map((s) => [s.operationName, wcLabel(s.workCenterId), s.standardTimeMinutes, s.actualTimeMinutes ?? '—', s.status])}
      >

...
export function CostingPage(): JSX.Element {
  ...
  const [accessories, setAccessories] = useState<DynamicRow[]>([
    {
      id: '1',
      name: 'طقم مجرى أدراج + كوالين ومقابض وبراغي',
      uomOrStation: 'طقم',
      qtyOrMins: 1,
      unitPriceOrRate: 450,
    },
  ...
  const [consumables, setConsumables] = useState<DynamicRow[]>([
    {
      id: '1',
      name: 'بودرة دهان إلكتروستاتيك ومستلزمات فرن',
      uomOrStation: 'لتر/كجم',
      qtyOrMins: 1,
      unitPriceOrRate: 500,
    },
  ]);
  const [operations, setOperations] = useState<DynamicRow[]>([
    {
      id: '1',
      name: 'قص ليزر فايبر + تشكيل ثناية CNC',
      uomOrStation: 'محطة الليزر',
      qtyOrMins: 30,
      unitPriceOrRate: 15,
    },
  ]);
    ...
    try {
      ...
      const [itemsRes, joRes] = await Promise.all([
        api
          ...
          .catch(() => ({ items: [] })),
        api
          .get<{ jobOrders: JobOrderRecord[] }>('/sales/job-orders')
          .catch(() => ({ jobOrders: [] })),
      ]);
  ...
  async function loadJoCostData(joNumber: string): Promise<void> {
    try {
      const res = await api.get<CostSummaryRecord & { summary?: CostSummaryRecord }>(
        `/cost/jobs/${joNumber}/summary`,
      );
  ...
  function addAccessoryRow(): void {
    setAccessories([
      ...accessories,
      { id: String(Date.now()), name: '', uomOrStation: 'عدد', qtyOrMins: 1, unitPriceOrRate: 0 },
    ]);
  }
  function addConsumableRow(): void {
    setConsumables([
      ...consumables,
      { id: String(Date.now()), name: '', uomOrStation: 'وحدة', qtyOrMins: 1, unitPriceOrRate: 0 },
    ]);
  }
  ...
  const totalAccessoriesCost = accessories.reduce(
    (sum, a) => sum + (Number(a.qtyOrMins) || 0) * (Number(a.unitPriceOrRate) || 0),
    0,
  );
  const totalConsumablesCost = consumables.reduce(
    (sum, c) => sum + (Number(c.qtyOrMins) || 0) * (Number(c.unitPriceOrRate) || 0),
    0,
  );
  const totalOperationsCost = operations.reduce(
    (sum, o) => sum + (Number(o.qtyOrMins) || 0) * (Number(o.unitPriceOrRate) || 0),
    0,
  );

  const initialDirectCost =
    totalMaterialsCost + totalAccessoriesCost + totalConsumablesCost + totalOperationsCost;
    ...
    <section className="module-page">
      <div className="page-intro">
        <div>
          ...
          <p>
            تقدير تكلفة الصاج الطبية، الإكسسوارات، الدهان، وأزمنة التشغيل ومقارنتها بالمنصرف الفعلي
            في صالة الإنتاج
          </p>
        </div>
      </div>
      ...
      {activeTab === 'estimator' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          >
            <label style={{ fontSize: 14, fontWeight: 'bold', color: '#0f172a' }}>
              اختر الصنف الفني من الكتالوج:
            </label>
            <select
              value={selectedItemId}
              onChange={(e) => setSelectedItemId(e.target.value)}
              style={{ ...inputStyle, minWidth: 240, fontWeight: 'bold', width: 'auto' }}
            >
              {items.map((it) => (
          ...
          >
            <div>
              ...
              <p style={{ margin: '4px 0 0', fontSize: 20, fontWeight: 'bold', color: '#38bdf8' }}>
                {initialDirectCost.toLocaleString('ar-EG', {
            ...
            <div>
              <span style={{ fontSize: 12, color: '#94a3b8' }}>
                مصروفات إشراف وإهلاك صناعية ({overheadPct}%)
              </span>
              <p style={{ margin: '4px 0 0', fontSize: 20, fontWeight: 'bold', color: '#facc15' }}>
                {overheadCost.toLocaleString('ar-EG', {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}{' '}
                ج.m
            ...
            <div>
              <span style={{ fontSize: 12, color: '#94a3b8' }}>
                التكلفة التقديرية النهائية للمنتج الواحد
              </span>
              <p style={{ margin: '4px 0 0', fontSize: 24, fontWeight: 'bold', color: '#4ade80' }}>
                {estimatedUnitCost.toLocaleString('ar-EG', {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}{' '}
                ج.م
          ...
          <article className="panel module-panel">
            ...
            >
              <h3 style={{ color: '#1e293b', margin: 0 }}>
                1. المواد المباشرة والخامات الرئيسية (مع احتساب الهالك)
              </h3>
              <button className="btn btn--sm" onClick={addMaterialRow}>
                + إضافة سطر صاج / خامة
              </button>
            </div>
                      ...
                      <tr key={m.id}>
                        ...
                        <td style={tdStyle}>
                          <input
                            ...
                            onChange={(e) => {
                              const updated = [...materials];
                              updated[idx]!.length = Number(e.target.value);
                              setMaterials(updated);
                            }}
                            style={inputStyle}
                          />
                        ...
                        <td style={tdStyle}>
                          <input
                            ...
                            onChange={(e) => {
                              const updated = [...materials];
                              updated[idx]!.width = Number(e.target.value);
                              setMaterials(updated);
                            }}
                            style={inputStyle}
                          />
                        ...
                        <td style={tdStyle}>
                          <input
                            ...
                            onChange={(e) => {
                              const updated = [...materials];
                              updated[idx]!.height = Number(e.target.value);
                              setMaterials(updated);
                            }}
                            style={inputStyle}
                          />
                        ...
                        <td style={tdStyle}>
                          <input
                            ...
                            onChange={(e) => {
                              const updated = [...materials];
                              updated[idx]!.scrapPct = Number(e.target.value);
                              setMaterials(updated);
                            }}
                            style={inputStyle}
                          />
                        ...
                        <td style={{ ...tdStyle, fontWeight: 'bold', color: '#0f172a' }}>
                          {linePrice.toLocaleString('ar-EG', {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                          })}{' '}
                          ج.م
          ...
          >
            <article className="panel module-panel">
              ...
              >
                <h3 style={{ color: '#1e293b', margin: 0 }}>
                  2. الإكسسوارات الطبية ومستلزمات الحركة
                </h3>
                <button className="btn btn--sm" onClick={addAccessoryRow}>
                  + بند إكسسوار
                </button>
              </div>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {accessories.map((a, idx) => (
                    <tr key={a.id}>
                      ...
                      <td style={tdStyle}>
                        <input
                          value={a.uomOrStation}
                          onChange={(e) => {
                            const updated = [...accessories];
                            updated[idx]!.uomOrStation = e.target.value;
                            setAccessories(updated);
                          }}
                          style={inputStyle}
                        />
                      ...
                      <td style={tdStyle}>
                        <input
                          ...
                          onChange={(e) => {
                            const updated = [...accessories];
                            updated[idx]!.qtyOrMins = Number(e.target.value);
                            setAccessories(updated);
                          }}
                          style={inputStyle}
                        />
                      ...
                      <td style={tdStyle}>
                        <input
                          type="number"
                          value={a.unitPriceOrRate}
                          onChange={(e) => {
                            const updated = [...accessories];
                            updated[idx]!.unitPriceOrRate = Number(e.target.value);
                            setAccessories(updated);
                          }}
                          style={inputStyle}
                        />
                      ...
                      <td style={{ ...tdStyle, fontWeight: 'bold' }}>
                        {(a.qtyOrMins * a.unitPriceOrRate).toFixed(2)} ج.م
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <article className="panel module-panel">
              ...
              >
                ...
                <button className="btn btn--sm" onClick={addConsumableRow}>
                  + إضافة مادة/دهان
                </button>
              </div>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {consumables.map((c, idx) => (
                    <tr key={c.id}>
                      ...
                      <td style={{ ...tdStyle, fontWeight: 'bold' }}>
                        {(c.qtyOrMins * c.unitPriceOrRate).toFixed(2)} ج.م
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <article className="panel module-panel">
            ...
            >
              <h3 style={{ color: '#1e293b', margin: 0 }}>
                4. أزمنة عمليات الماكينات والعمالة (ليزر، ثناية، تجميع)
              </h3>
              <button className="btn btn--sm" onClick={addOperationRow}>
                + إضافة عملية تشغيلية
              </button>
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {operations.map((o, idx) => (
                  <tr key={o.id}>
                    ...
                    <td style={{ ...tdStyle, fontWeight: 'bold', color: '#0f172a' }}>
                      {(o.qtyOrMins * o.unitPriceOrRate).toFixed(2)} ج.م
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <label style={{ fontSize: 14, fontWeight: 'bold', color: '#475569' }}>
                اختر أمر الشغل (Job Order):
              </label>
              <select
                value={selectedJO}
                onChange={(e) => {
                ...
                style={{ ...inputStyle, minWidth: 220, fontWeight: 'bold', width: 'auto' }}
              >
                {jobOrders.map((j) => (
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <input
                ...
                style={{ ...inputStyle, width: 80, fontWeight: 'bold' }}
              />
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <input
                ...
                style={{ ...inputStyle, width: 120, fontWeight: 'bold' }}
              />
            </div>
          </div>
          ...
          >
            ...
            <article className="panel" style={{ padding: 16 }}>
              <span style={{ fontSize: 12, color: '#64748b' }}>
                المنصرف والمنفذ الفعلي من صالة الإنتاج
              </span>
              <p
                style={{
                  margin: '4px 0 0',
                  fontSize: 20,
                  fontWeight: 'bold',
                  color: actualCostFromDb > joEstimatedTotal ? '#b91c1c' : '#166534',
                }}
              >
            ...
            <article className="panel" style={{ padding: 16 }}>
              <span style={{ fontSize: 12, color: '#64748b' }}>
                صافي الأرباح الفعلية المحققة للمصنع
              </span>
              <p
                style={{
                  margin: '4px 0 0',
                  fontSize: 20,
                  fontWeight: 'bold',
                  color: actualProfit < estimatedProfit ? '#b91c1c' : '#166534',
                }}
              >
          ...
          <article className="panel module-panel">
            <h3 style={{ marginBottom: 16, color: '#1e293b' }}>
              مقارنة المخطط بالمنفذ الفعلي لأمر الشغل ({selectedJO})
            </h3>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
              ...
              <tbody>
                <tr>
                  <td style={tdStyle}>
                    <b>التكلفة المباشرة المجمعة (خامات + عمالة + تشغيل)</b>
                  </td>
                  <td style={tdStyle}>
                    {(initialDirectCost * joBatchQty).toLocaleString('ar-EG', {
                      minimumFractionDigits: 2,
                    })}{' '}
                    ج.م
                  </td>
                  ...
                  <td
                    style={{
                      ...tdStyle,
                      color: varianceTotal > 0 ? '#b91c1c' : '#166534',
                      fontWeight: 'bold',
                    }}
                  >
                    {varianceTotal > 0
                      ? `+${varianceTotal.toLocaleString('ar-EG', { minimumFractionDigits: 2 })}`
                      : `${varianceTotal.toLocaleString('ar-EG', { minimumFractionDigits: 2 })}`}{' '}
                    ج.م
                  </td>
                  <td style={tdStyle}>
                    <span
                      className={`status-badge status-badge--${varianceTotal > 0 ? 'cancelled' : 'active'}`}
                    >
                      {varianceTotal > 0
                        ? 'تجاوز في الميزانية ⚠️'
                        : 'وفر حقيقي ومطابق لميزانيتك ✅'}
                    </span>
                  </td>
                </tr>
                  ...
                  <td style={tdStyle}>
                    {varianceTotal > 0 ? (
                      <span style={{ color: '#b91c1c', fontSize: 12 }}>
                        ⚠️ تأكل في الأرباح بمقدار {varianceTotal.toLocaleString()} ج.م
                      </span>
                    ) : (
                      <span style={{ color: '#166534', fontSize: 12 }}>
                        ✅ زيادة ممتازة في الأرباح التشغيلية
                      </span>
                    )}
                  </td>
                </tr>

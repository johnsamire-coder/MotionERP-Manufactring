...
export function ExpensesPage(): JSX.Element {
  ...
  const [expCategory, setExpCategory] = useState<
    'transport' | 'samples' | 'electricity' | 'packaging' | 'paint' | 'wages' | 'gratuity' | 'other'
  >('transport');
  ...
  const [expenses, setExpenses] = useState<ExpenseRow[]>([
    {
      id: '1',
      date: '2026-07-01',
      description: 'استريتش لزوم التغليف',
      amount: 408,
      category: 'packaging',
    ...
    },
    {
      id: '2',
      date: '2026-07-01',
      description: 'النقلات لنصحي - شراء خامات',
      amount: 25,
      category: 'transport',
      jobOrderReference: 'JO-2026-000001',
    },
    {
      id: '3',
      date: '2026-07-01',
      description: 'عمار تروسيكل - نقلة إلى فرن الدهان',
      amount: 100,
      category: 'transport',
      jobOrderReference: 'JO-2026-000001',
    },
    {
      id: '4',
      date: '2026-07-01',
      description: 'شعبان - باقي حساب النقل',
      amount: 1200,
      category: 'transport',
      jobOrderReference: 'JO-2026-000001',
    },
    ...
    },
    {
      id: '6',
      date: '2026-07-05',
      description: 'عيد خليفة - مصنع الدهان',
      amount: 30000,
      category: 'paint',
      jobOrderReference: 'JO-2026-000001',
    },
    {
      id: '7',
      date: '2026-07-08',
      description: 'سلك لحام وسلك سويدي للكهرباء',
      amount: 380,
    ...
    },
    {
      id: '8',
      date: '2026-07-13',
      description: 'أولاد يونس - نقل حديد',
      amount: 2350,
      category: 'transport',
      jobOrderReference: 'JO-2026-000001',
    },
    {
      id: '9',
      date: '2026-07-15',
      description: 'عيد خليفة - دفعة دهان أفرُن',
      amount: 40000,
      category: 'paint',
      jobOrderReference: 'JO-2026-000001',
    },
  ...
  function handleExportExcel(): void {
    ...
    for (const exp of expenses) {
      const row = [
        ...
        exp.category === 'other' || exp.category === 'gratuity' || exp.category === 'wages'
          ? exp.amount
          : 0,
      ];
    ...
    link.setAttribute(
      'download',
      `تحليل_المصروفات_الإنتاجية_${new Date().toISOString().split('T')[0]}.csv`,
    );
  ...
  const sumTransport = expenses
    .filter((x) => x.category === 'transport')
    .reduce((s, x) => s + x.amount, 0);
  const sumSamples = expenses
    .filter((x) => x.category === 'samples')
    .reduce((s, x) => s + x.amount, 0);
  const sumElectricity = expenses
    .filter((x) => x.category === 'electricity')
    .reduce((s, x) => s + x.amount, 0);
  const sumPackaging = expenses
    .filter((x) => x.category === 'packaging')
    .reduce((s, x) => s + x.amount, 0);
  const sumPaint = expenses.filter((x) => x.category === 'paint').reduce((s, x) => s + x.amount, 0);
  const _sumWages = expenses
    .filter((x) => x.category === 'wages')
    .reduce((s, x) => s + x.amount, 0);
  const sumOther = expenses
    .filter((x) => x.category === 'other' || x.category === 'gratuity')
    .reduce((s, x) => s + x.amount, 0);
    ...
    <section className="module-page">
      ...
      >
        ...
        <button
          ...
          style={{ background: '#166534', color: '#fff', fontWeight: 'bold' }}
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'end' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                <input
                  ...
                  required
                  placeholder="بيان المصروف والجهة..."
            ...
            <tbody>
              {expenses.map((exp) => (
                <tr key={exp.id}>
                  ...
                  <td style={tdStyle}>
                    <code>{exp.jobOrderReference}</code>
                  </td>
                  <td style={tdStyle}>
                    {exp.category === 'transport' ? (
                      <b>{exp.amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</b>
                    ) : (
                      '—'
                    )}
                  </td>
                  <td style={tdStyle}>
                    {exp.category === 'samples' ? (
                      <b>{exp.amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</b>
                    ) : (
                      '—'
                    )}
              ...
              >
                <td style={tdStyle} colSpan={2}>
                  <b>{t('pages.expenses.table.totals')}</b>
                </td>
                <td style={{ ...tdStyle, color: '#0f172a', fontSize: 14 }}>
                  {sumTotal.toLocaleString('en-US', { minimumFractionDigits: 2 })} EGP
                </td>
                <td style={tdStyle}>—</td>
                <td style={{ ...tdStyle, color: '#0369a1' }}>
                  {sumTransport.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </td>
                <td style={{ ...tdStyle, color: '#0369a1' }}>
                  {sumSamples.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </td>
                <td style={{ ...tdStyle, color: '#0369a1' }}>
                  {sumElectricity.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </td>
                <td style={{ ...tdStyle, color: '#0369a1' }}>
                  {sumPackaging.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </td>
                <td style={{ ...tdStyle, color: '#0369a1' }}>
                  {sumPaint.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </td>
                <td style={{ ...tdStyle, color: '#0369a1' }}>
                  {sumOther.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </td>
              </tr>
            </tbody>
          </table>

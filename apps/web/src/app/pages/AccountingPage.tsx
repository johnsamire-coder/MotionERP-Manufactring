...
export function AccountingPage(): JSX.Element {
  ...
  const [journalEntries, setJournalEntries] = useState<
    Array<JournalEntryRecord & { isAutoGenerated?: boolean }>
  >([]);
  ...
  const [defaultOrgNodeId, _setDefaultOrgNodeId] = useState('00000000-0000-0000-0000-000000000001');
  ...
  const loadData = async () => {
    try {
      ...
      const [accRes, fyRes, ccRes, jeRes, collRes, retRes] = await Promise.all([
        api.get<{ accounts: AccountRow[] }>('/accounting/accounts').catch(() => ({ accounts: [] })),
        api
          .get<{ fiscalYears: FiscalYearRow[] }>('/accounting/fiscal-years')
          .catch(() => ({ fiscalYears: [] })),
        api
          .get<{ costCenters: CostCenterRow[] }>('/accounting/cost-centers')
          .catch(() => ({ costCenters: [] })),
        accountingApi.getJournalEntries().catch(() => ({ entries: [] })),
        api
          .get<{ collections: CollectionRecord[] }>('/finance/collections')
          .catch(() => ({ collections: [] })),
        api
          .get<{ retentions: RetentionRow[] }>('/finance/retentions')
          .catch(() => ({ retentions: [] })),
      ]);
      ...
      setFiscalYears(fys);
      const firstFy = fys[0];
      if (firstFy && !selectedFyId) {
        setSelectedFyId(firstFy.id);
        void loadPeriods(firstFy.id);
      }
  ...
  const loadPeriods = async (fyId: string) => {
    try {
      const res = await api.get<{ periods: PeriodRow[] }>(
        `/accounting/fiscal-years/${fyId}/periods`,
      );
  ...
  return (
    <section className="module-page">
      ...
      >
        <button
          className={`btn ${activeTab === 'coa' ? 'btn--primary' : 'btn--secondary'}`}
          onClick={() => setActiveTab('coa')}
        >
          📊 دليل الحسابات ({accounts.length})
        </button>
        <button
          className={`btn ${activeTab === 'fiscal' ? 'btn--primary' : 'btn--secondary'}`}
          onClick={() => setActiveTab('fiscal')}
        >
          📅 السنوات والفترات المالية
        </button>
        <button
          className={`btn ${activeTab === 'cost_centers' ? 'btn--primary' : 'btn--secondary'}`}
          onClick={() => setActiveTab('cost_centers')}
        >
          🏭 مراكز التكلفة ({costCenters.length})
        </button>
        <button
          className={`btn ${activeTab === 'journals' ? 'btn--primary' : 'btn--secondary'}`}
            ...
            <table className="data-table">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={5}>لا توجد حسابات مسجلة</td>
                  </tr>
                ) : (
                  accounts.map((acc) => (
                    <tr key={acc.id}>
                      ...
                      <td>{acc.name}</td>
                      <td>
                        <code>{acc.accountType}</code>
                      </td>
                      <td>
                        <span
                          className={`status-badge status-badge--${acc.isLeaf ? 'active' : 'draft'}`}
                        >
          ...
          {showFyModal && (
            ...
            >
              ...
              <div className="form-actions">
                ...
                <button type="button" className="btn" onClick={() => setShowFyModal(false)}>
                  إلغاء
                </button>
              </div>
            </form>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  void loadPeriods(e.target.value);
                }}
          ...
          <div className="data-table-wrapper">
            <table className="data-table">
              ...
              <tbody>
                {periods.length === 0 ? (
                  <tr>
                    <td colSpan={6}>يرجى اختيار سنة مالية لعرض شهورها</td>
                  </tr>
                ) : (
                  periods.map((p) => (
                    <tr key={p.id}>
                      <td>
                        <b>M{p.periodNumber}</b>
                      </td>
                      <td>{p.name}</td>
                      ...
                      <td>
                        <span
                          className={`status-badge status-badge--${p.status === 'open' ? 'active' : 'cancelled'}`}
                        >
          ...
          {showCcModal && (
            ...
            >
              ...
              <div className="form-actions">
                ...
                <button type="button" className="btn" onClick={() => setShowCcModal(false)}>
                  إلغاء
                </button>
              </div>
            </form>
            ...
            <table className="data-table">
              ...
              <tbody>
                {costCenters.length === 0 ? (
                  <tr>
                    <td colSpan={4}>لا توجد مراكز تكلفة مسجلة</td>
                  </tr>
                ) : (
                  costCenters.map((cc) => (
                    <tr key={cc.id}>
            ...
            <table className="data-table">
              ...
              <tbody>
                {journalEntries.length === 0 ? (
                  <tr>
                    <td colSpan={6}>لا توجد قيود يومية مسجلة</td>
                  </tr>
                ) : (
                  journalEntries.map((je) => (
                    <tr key={je.id}>
                      ...
                      <td>
                        <span
                          className={`status-badge status-badge--${je.status === 'posted' ? 'active' : 'draft'}`}
                        >
              ...
              <table className="data-table">
                ...
                <tbody>
                  {retentions.map((r) => (
                    <tr key={r.id}>
                      <td>
                        <b>{r.retentionNumber}</b>
                      </td>
                      <td>{Number(r.originalAmount).toLocaleString('ar-EG')} ج.م</td>
                      <td>
                        <span className="status-badge status-badge--active">{r.status}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>

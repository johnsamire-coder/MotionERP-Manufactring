...
export function WorkOrderPage(): JSX.Element {
  ...
  const approvedBomsForItem = boms.filter(
    (b) => b.productItemId === productItemId && b.status === 'approved',
  );
  ...
  async function loadAll(): Promise<void> {
    ...
    try {
      ...
      if (!finishedGoodsWarehouseId && whRes.warehouses[0])
        setFinishedGoodsWarehouseId(whRes.warehouses[0].id);
    } catch (err) {
  ...
  async function handleTransition(
    id: string,
    action: 'start' | 'complete' | 'stop' | 'close',
  ): Promise<void> {
    setFormError(null);
    setFormSuccess(null);
    setSubmitting(true);
  ...
  const _orgLabel = (id: string): string => orgNodes.find((o) => o.id === id)?.name ?? id;
  const whLabel = (id: string | null): string =>
  ...
  const kpiCompleted = workOrders.filter(
    (w) => w.status === 'completed' || w.status === 'closed',
  ).length;
  const kpiPlannedQty = workOrders.reduce(
    (sum, w) => sum + parseFloat(w.qtyToManufacture || '0'),
    0,
  );

  if (loading)
    return <p style={{ padding: 40, textAlign: 'center' }}>{t('pages.technical.form.loading')}</p>;
      ...
      <article className="panel module-panel">
        <div className="panel__head">
          ...
          <button className="primary-button" onClick={() => setShowForm((v) => !v)}>
            <b>+</b>
            {t('pages.work_order.createWorkOrder')}
          </button>
        </div>
        ...
        {showForm && (
          ...
          >
            ...
            {approvedBomsForItem.length === 0 && (
              ...
              >
                {t('pages.work_order.noBomWarning')}
              </p>
            )}
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'end' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                <select
                  ...
                  onChange={(e) => setFinishedGoodsWarehouseId(e.target.value)}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                <select
                  value={wipWarehouseId}
                  onChange={(e) => setWipWarehouseId(e.target.value)}
                  style={{ ...inputStyle, minWidth: 180 }}
                >
                  <option value="">—</option>
                  {warehouses.map((w) => (
            ...
            >
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13 }}>
                <input
                  type="checkbox"
                  checked={considerScrapItems}
                  onChange={(e) => setConsiderScrapItems(e.target.checked)}
                />
                {t('pages.work_order.considerScrapItems')}
              </label>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                <label style={labelStyle}>
                  {t('pages.work_order.materialConsumptionPercentage')}
                </label>
                <input
                  type="number"
                  min="0"
                  max="200"
                  step="any"
                  value={materialConsumptionPercentage}
                  onChange={(e) => setMaterialConsumptionPercentage(e.target.value)}
                  style={{ ...inputStyle, width: 100 }}
                />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                <select
                  ...
                  onChange={(e) => setMaterialTransferMode(e.target.value)}
            ...
            <button
              type="submit"
              disabled={submitting || approvedBomsForItem.length === 0}
        ...
        >
          ...
          >
            <span style={{ fontSize: 12, color: '#b45309' }}>
              {t('pages.work_order.kpiInProgress')}
            </span>
            <div style={{ fontSize: 22, fontWeight: 'bold', color: '#b45309', marginTop: 4 }}>
              {kpiInProgress}
            </div>
          </div>
          <div
          ...
          >
            <span style={{ fontSize: 12, color: '#166534' }}>
              {t('pages.work_order.kpiCompleted')}
            </span>
            <div style={{ fontSize: 22, fontWeight: 'bold', color: '#15803d', marginTop: 4 }}>
              {kpiCompleted}
            </div>
          </div>
          <div
          ...
          >
            <span style={{ fontSize: 12, color: '#0369a1' }}>
              {t('pages.work_order.kpiPlannedQty')}
            </span>
            <div style={{ fontSize: 22, fontWeight: 'bold', color: '#0369a1', marginTop: 4 }}>
              {kpiPlannedQty.toLocaleString(undefined, {
              ...
              <span style={{ display: 'flex', gap: 6 }}>
                {wo.status === 'not_started' && (
                  <button
                    ...
                    onClick={() => {
                      void handleTransition(wo.id, 'start');
                    }}
                  >
                    {t('pages.work_order.start')}
                  </button>
                )}
                {wo.status === 'in_progress' && (
                  <>
                    <button
                      ...
                      style={{ fontSize: 12, padding: '4px 10px' }}
                      onClick={() => {
                        void handleTransition(wo.id, 'complete');
                      }}
                    >
                      {t('pages.work_order.complete')}
                    </button>
                    <button
                      className="filter-button"
                      style={{ fontSize: 12, padding: '4px 10px' }}
                      onClick={() => {
                        void handleTransition(wo.id, 'stop');
                      }}
                    >
                      {t('pages.work_order.stop')}
                    </button>
                  </>
                )}
                {(wo.status === 'completed' || wo.status === 'stopped') && (
                  <button
                    ...
                    style={{ fontSize: 12, padding: '4px 10px' }}
                    onClick={() => {
                      void handleTransition(wo.id, 'close');
                    }}
                  >
                    {t('pages.work_order.close')}
                  </button>
                )}
              </span>
            </div>

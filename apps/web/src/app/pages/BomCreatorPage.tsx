...
export function BomCreatorPage(): JSX.Element {
  ...
  const [rows, setRows] = useState<BcTreeRow[]>([
    { tempId: 1, parentTempId: null, componentItemId: '', quantity: '1', isSubAssembly: false },
  ]);
        ...
        setRows([
          {
            tempId: 1,
            parentTempId: null,
            componentItemId: itemsRes.items[0].id,
            quantity: '1',
            isSubAssembly: false,
          },
        ]);
  ...
  function removeRow(tempId: number): void {
    setRows(
      rows
        ...
        .map((r) => (r.parentTempId === tempId ? { ...r, parentTempId: null } : r)),
    );
  }
              ...
              {rows.map((row) => (
                ...
                >
                  <select
                    ...
                    onChange={(e) => updateRow(row.tempId, 'componentItemId', e.target.value)}
                  ...
                  >
                    ...
                    {rows
                      ...
                      .map((r) => (
                        <option key={r.tempId} value={r.tempId}>
                          {itemLabel(r.componentItemId)} (#{r.tempId})
                        </option>
                      ))}
                  </select>
                  <input
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={row.isSubAssembly}
                      onChange={(e) => updateRow(row.tempId, 'isSubAssembly', e.target.checked)}
                    />

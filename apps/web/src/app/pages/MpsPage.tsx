...
interface MpsLineInput {
  period: string;
  startDate: string;
  endDate: string;
  forecastQuantity: string;
}
interface MpsLineRecord {
  id: string;
  period: string;
  startDate: string;
  endDate: string;
...
export function MpsPage(): JSX.Element {
  ...
  const [lines, setLines] = useState<MpsLineInput[]>([
    { period: 'month', startDate: '', endDate: '', forecastQuantity: '100' },
  ]);
  ...
  async function loadAll(): Promise<void> {
    ...
    try {
      ...
      const [itemsRes, orgRes, whRes, mpsRes] = await Promise.all([
        ...
        api.get<{ masterProductionSchedules: MpsRecord[] }>(
          '/planning/master-production-schedules',
        ),
      ]);
  ...
  function addLine(): void {
    setLines([...lines, { period: 'month', startDate: '', endDate: '', forecastQuantity: '100' }]);
  }
  ...
  const _periodLabel = (p: string): string => t(`pages.mps.${p}`);
        ...
        {showForm && (
          ...
          >
            ...
            >
              ...
              {lines.map((line, idx) => (
                ...
                >
                  ...
                  <input
                    ...
                    onChange={(e) => updateLine(idx, 'startDate', e.target.value)}
                    required
                    style={inputStyle}
                  />
                  <input
                    type="date"
                    value={line.endDate}
                    onChange={(e) => updateLine(idx, 'endDate', e.target.value)}
                    required
                    style={inputStyle}
                  />
                  <input
              ...
              <button type="button" onClick={addLine} className="filter-button">
                + {t('pages.mps.addLine')}
              </button>
              <input
                type="number"
                min="0"
                step="any"
                placeholder={t('pages.mps.totalToDistribute')}
                value={mpsDistributeTotal}
                onChange={(e) => setMpsDistributeTotal(e.target.value)}
                style={{ ...inputStyle, width: 140, marginInlineStart: 8 }}
              />
              <button type="button" onClick={distributeMpsEvenly} className="filter-button">
                {t('pages.mps.distributeEvenly')}
              </button>
            </div>
        ...
        <div className="placeholder-table">
          ...
          {records.map((r) => (
            <div className="placeholder-table__row" key={r.id}>
              ...
              <span>
                <span
                  className={`status status--${r.status === 'submitted' ? 'success' : 'neutral'}`}
                >
                  <i />
                  {r.status}
                </span>
              ...
              <span style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                <button
                  ...
                  onClick={() => {
                    void handleGetProjected(r.id);
                  }}
                >
                  {t('pages.mps.getProjected')}
                </button>
                {r.status === 'draft' && (
                  <button
                    className="primary-button"

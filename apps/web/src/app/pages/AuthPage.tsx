    ...
    <section className="module-page">
      ...
      <article className="panel module-panel" style={{ marginBottom: 20 }}>
        <div className="panel__head">
          ...
          <button className="primary-button" onClick={() => setShowUserForm((v) => !v)}>
            <b>+</b> {t('pages.auth.users.addUser')}
          </button>
        </div>
      ...
      <article className="panel module-panel">
        <div className="panel__head">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="filter-button" onClick={() => setShowRoleForm((v) => !v)}>
              <b>+</b> {t('pages.auth.roles.addRole')}
            </button>
            <button className="primary-button" onClick={() => setShowPermForm((v) => !v)}>
              <b>+</b> {t('pages.auth.roles.addPermission')}
            </button>
          </div>
        </div>
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <input
                ...
                required
                placeholder="e.g. accounting.journal_entry"
                style={{ ...inputStyle, minWidth: 200 }}
              />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          {permissions.map((p) => (
            <div className="placeholder-table__row" key={p.id}>
              ...
              <span>
                <b>
                  {p.maxValueLimit
                    ? `${Number(p.maxValueLimit).toLocaleString()} EGP`
                    : 'بلا حد أقصى'}
                </b>
              </span>
            </div>

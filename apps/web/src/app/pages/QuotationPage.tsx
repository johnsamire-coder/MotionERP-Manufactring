...
export function QuotationPage(): JSX.Element {
  ...
  async function loadAll(): Promise<void> {
    ...
    try {
      ...
      if (!customerId && custRes.customers && custRes.customers[0])
  ...
  async function handleItemChange(index: number, itemId: string): Promise<void> {
    ...
    try {
      ...
      const priceRes = await api.get<{ itemPrice: { price: string } | null }>(
        `/catalog/items/${itemId}/prices/latest?type=selling`,
      );
  ...
  async function addLine(): Promise<void> {
    ...
    if (defaultItem) {
      try {
        const pRes = await api.get<{ itemPrice: { price: string } | null }>(
          `/catalog/items/${defaultItem}/prices/latest?type=selling`,
        );
        if (pRes.itemPrice) defaultPrice = pRes.itemPrice.price;
      } catch {
        // No price on record: keep the line price empty for manual entry.
      }
    }
  ...
  async function handleApprove(id: string): Promise<void> {
    ...
    try {
      ...
      setPoRefInput('');
      setFormSuccess(
        `Quotation approved! Sales Order (Job Order ${res.jobOrder?.jobOrderNumber || ''}) generated automatically!`,
      );
      await loadAll();
    } catch (err) {
  ...
  const custLabel = (id: string | null): string =>
    id ? (customers.find((c) => c.id === id)?.name ?? id) : '—';
  const _orgLabel = (id: string | null): string =>
    id ? (orgNodes.find((o) => o.id === id)?.name ?? id) : '—';
    ...
    <section className="module-page">
      <div className="page-intro">
        <div>
          ...
          <p>
            Create commercial quotations, calculate margins from BOM prices, and convert to Sales
            Orders.
          </p>
        </div>
      ...
      {formSuccess && (
        <p style={{ color: '#166534', padding: '8px 0', fontWeight: 'bold' }}>{formSuccess}</p>
      )}
          ...
          >
            ...
            >
                ...
                return (
                  ...
                  >
                    <div style={{ flex: 3 }}>
                      <select
                        value={line.itemId}
                        onChange={(e) => void handleItemChange(idx, e.target.value)}
                        style={{ ...inputStyle, width: '100%' }}
                        required
                      >
                        {items.map((it) => (
                    ...
                    <div
                      style={{ flex: 1.5, fontWeight: 'bold', color: '#0369a1', paddingLeft: 8 }}
                    >
              ...
              >
                ...
                <div style={{ fontSize: 16, fontWeight: 'bold', color: '#0f172a' }}>
                  Grand Total:{' '}
                  <span style={{ color: '#166534', fontSize: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <textarea
                ...
                placeholder="e.g. Delivery in 15 days, 50% advance..."
          ...
          {quotations.map((q) => {
            ...
            return (
              <div className="placeholder-table__row" key={q.id}>
                ...
                <span>
                  <b>
                    {qTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })} {q.currency}
                  </b>
                </span>
                <span>
                  <span
                    className={`status status--${q.status === 'approved' ? 'success' : q.status === 'sent' ? 'warning' : 'neutral'}`}
                  >
                    <i />
                ...
                <span style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  ...
                  {q.status === 'sent' && (
                    <>
                      <button
                        className="primary-button"
                        style={{ fontSize: 12, padding: '4px 8px', background: '#166534' }}
                        onClick={() => setApproveModalId(q.id)}
                      >
                        ✓ Approve & Order
                      </button>
                      <button
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: 13, color: '#64748b', margin: 0 }}>
              Approving this quotation will automatically generate a <b>Sales Order (Job Order)</b>{' '}
              to initiate production planning.
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              <label style={labelStyle}>Customer PO Reference (Optional)</label>
              <input
                type="text"
                placeholder="e.g. PO-HOSP-2026-99"
                value={poRefInput}
                onChange={(e) => setPoRefInput(e.target.value)}
                style={{ ...inputStyle, width: '100%' }}
              />
            </div>
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 8 }}>
              <button className="filter-button" onClick={() => setApproveModalId(null)}>
                Cancel
              </button>
              <button
                className="primary-button"
                disabled={submitting}
                onClick={() => {
                  void handleApprove(approveModalId);
                }}
              >

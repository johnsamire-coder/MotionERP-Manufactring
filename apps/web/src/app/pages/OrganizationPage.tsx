    ...
    <section className="module-page">
      ...
      <article className="panel module-panel">
        <div className="panel__head">
          ...
          <button className="primary-button" onClick={() => setShowNodeForm((v) => !v)}>
            <b>+</b> {t('pages.organization.tree.addNode')}
          </button>
        </div>
        ...
        {showNodeForm && (
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              >
                {nodeTypes.map((nt) => (
                  <option key={nt.nodeType} value={nt.nodeType}>
                    {nt.nodeType}
                  </option>
                ))}
              </select>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <input
                ...
                required
                placeholder="e.g. LaboSystem Facility #1"
            ...
            <button
              ...
              style={{ height: 38 }}
            >
              {t('pages.organization.form.save')}
            </button>
          </form>
        )}
        ...
        <div className="placeholder-table">
          ...
          {flatList.length === 0 && (
            <p style={{ padding: '20px 0', textAlign: 'center', color: '#94a3b8' }}>
              {t('pages.organization.form.empty')}
            </p>
          )}
          ...
          {flatList.map((node) => (
            <div
              ...
              key={node.id}
              style={{ paddingRight: node.depth * 20 }}
            >
              <span>
              ...
              <span>
                <span
                  className={`status status--${node.status === 'active' ? 'success' : 'neutral'}`}
                >
                  <i />
                  {node.status}
                </span>
              ...
              <span>
                {node.status === 'active' && node.depth > 0 && (
                  <button
                    ...
                    onClick={() => {
                      void handleArchiveNode(node.id);
                    }}
                  >

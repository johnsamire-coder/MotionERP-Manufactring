...
export function SalesPage(): JSX.Element {
  ...
  async function loadAll(): Promise<void> {
    ...
    try {
      ...
      if (!selectedCustomerId && custRes.customers[0])
  ...
  const grandTotal = quoteLines.reduce(
    ...
    <section className="module-page">
      ...
      <article className="panel module-panel">
        <div className="panel__head">
          ...
          <button className="primary-button" onClick={openCustomerForm}>
            <b>+</b>
            {t('pages.sales.customers.addCustomer')}
          </button>
        </div>
        ...
        <div className="placeholder-table">
          ...
          {customers.map((c) => (
            <div className="placeholder-table__row" key={c.id}>
              ...
              <span>{c.name}</span>
              <span>{c.phone ?? '—'}</span>
              <span className={`status status--${c.status === 'active' ? 'success' : 'neutral'}`}>
                <i />
                {c.status}
              </span>
            </div>
      ...
      <article className="panel module-panel" style={{ marginTop: 20 }}>
        <div className="panel__head">
          ...
          <button className="primary-button" onClick={() => setShowQuotationForm((v) => !v)}>
            <b>+</b>
            {t('pages.sales.quotations.addQuotation')}
          </button>
        </div>
        ...
        {showQuotationForm && (
          ...
          >
            ...
            >
              ...
              >
                {t('pages.sales.quotations.lines')}
              </label>
              ...
              >
                ...
                <span style={{ width: 90, textAlign: 'end' }}>
                  {t('pages.sales.quotations.total')}
                </span>
              </div>
              ...
              {quoteLines.map((line, idx) => {
                ...
                return (
                  ...
                  >
                    <select
                      value={line.itemId}
                      onChange={(e) => updateQuoteLine(idx, 'itemId', e.target.value)}
                    ...
                    <input
                      ...
                      onChange={(e) => updateQuoteLine(idx, 'unitPrice', e.target.value)}
                      required
              ...
              >
                <button type="button" onClick={addQuoteLine} className="filter-button">
                  + {t('pages.sales.quotations.addItem')}
                </button>
          ...
          {quotations.map((q) => (
            <div className="placeholder-table__row" key={q.id}>
              <span>
                ...
                <small>{q.customerPoReference ? `PO: ${q.customerPoReference}` : ''}</small>
              </span>
              ...
              <span style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <span
                  className={`status status--${q.status === 'approved' ? 'success' : 'neutral'}`}
                >
                  <i />
                  {q.status}
                </span>
                {q.status === 'draft' && (
                  <button
                    ...
                    onClick={() => setShowApproveModal(q.id)}
                  >
        ...
        >
          ...
          >
            ...
            <input
              ...
              placeholder="e.g. PO-CUST-2026-99"
              style={{ ...inputStyle, width: '100%' }}
            />
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 10 }}>
              <button className="filter-button" onClick={() => setShowApproveModal(null)}>
                {t('pages.sales.form.cancel')}
              </button>
              <button
                className="primary-button"
                disabled={submitting}
                onClick={() => {
                  void handleApproveQuotation(showApproveModal);
                }}
              >
                {t('pages.sales.form.save')}
              </button>
          ...
          {jobOrders.map((jo) => (
            <div className="placeholder-table__row" key={jo.id}>
              <span>
                ...
                <small>{customerLabel(jo.customerId)}</small>
              </span>
              ...
              <span>
                {jo.financialReviewPassed ? (
                  <span className="status status--success">
                    <i />
                    {t('pages.sales.jobOrders.passed')}
                  </span>
                ) : (
                  <button
                    className="filter-button"
                    style={{ fontSize: 12, padding: '4px 8px' }}
                    onClick={() => {
                      void handlePassFinancialReview(jo.id);
                    }}
                  >
              ...
              <span style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <span
                  className={`status status--${jo.status === 'approved' ? 'success' : 'neutral'}`}
                >
                  <i />
                  {jo.status}
                </span>
                {jo.status === 'draft' && jo.financialReviewPassed && (
                  <button
                    ...
                    style={{ fontSize: 12, padding: '4px 8px' }}
                    onClick={() => {
                      void handleApproveJobOrder(jo.id);
                    }}
                  >

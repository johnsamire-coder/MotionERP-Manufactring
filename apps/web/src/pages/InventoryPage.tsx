...
export const InventoryPage: React.FC = () => {
  const stockItems = [
    { code: 'RAW-SS-304-15', name: 'صاج ستانلس 304 طبي 1.5 مم', warehouse: 'مخزن الخامات الرئيسي', qty: '1,250 كجم', avgCost: '185 ج.م', totalValue: '231,250 ج.م', batch: 'LOT-2026-MED-0941' },
    { code: 'RAW-PIPE-125', name: 'مواسير ستانلس 304 قطر 1.25 بوصة', warehouse: 'مخزن الخامات الرئيسي', qty: '480 متر', avgCost: '140 ج.م', totalValue: '67,200 ج.م', batch: 'LOT-2026-MED-0941' },
    { code: 'ACC-HYD-400W', name: 'مواتير وبساتم هيدروليكية 400W', warehouse: 'مخزن الإكسسوارات الطبية', qty: '35 طقم', avgCost: '4,500 ج.م', totalValue: '157,500 ج.م', batch: 'CE-MED-2026-78' },
    { code: 'FG-ICU-BED-01', name: 'سرير عناية مركزة كهربائي 5 حركات', warehouse: 'مخزن المنتج التام', qty: '10 أسرة', avgCost: '15,033 ج.م', totalValue: '150,330 ج.م', batch: 'BATCH-2026-09-ICU' },
  ];
    ...
    <div className="p-6 space-y-6 bg-slate-50 min-h-screen text-slate-800" dir="rtl">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-3">
          <div className="p-3 bg-amber-600 text-white rounded-xl shadow-md"><Boxes className="w-7 h-7" /></div>
          <div>
            <h1 className="text-2xl font-bold text-slate-900">دليل الأصناف وأرصدة المخازن التراكمية</h1>
            <p className="text-sm text-slate-500">متابعة أرصدة الصاج الإكسسوارات الطبية والمنتج التام مع حساب المتوسط المتحرك</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <p className="text-xs font-semibold text-slate-500 uppercase">إجمالي قيمة المخزون الدفتري</p>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-200 bg-slate-50"><h3 className="font-bold text-slate-900">جدول أرصدة المخازن الحالية</h3></div>
            ...
            <tbody className="divide-y divide-slate-100">
              {stockItems.map((item) => (
                <tr key={item.code} className="hover:bg-slate-50 transition">
                  ...
                  <td className="p-4 font-mono text-xs text-teal-700 bg-teal-50 px-2 py-0.5 rounded border border-teal-200 w-max">{item.batch}</td>
                </tr>
              ))}
            </tbody>
          </table>

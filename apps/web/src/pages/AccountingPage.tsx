        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">
              شجرة الحسابات ودفتر اليومية العامة الحقيقي
            </h1>
            <p className="text-sm text-slate-500">
              الدليل المحاسبي الفعلي ومحرك الترحيل الآلي للقيود من الداتابيز
            </p>
          </div>
        </div>
        <button
          onClick={fetchData}
          className="flex items-center gap-1.5 px-4 py-2 border border-slate-300 rounded-xl text-xs font-bold hover:bg-slate-50 transition cursor-pointer"
        >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-200 flex gap-2 bg-slate-50">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-bold transition cursor-pointer ${activeTab === 'coa' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-600'}`}
          >
            دليل الحسابات الفعلي (Chart of Accounts)
          </button>
          <button
            onClick={() => setActiveTab('journals')}
            className={`px-4 py-2 rounded-xl text-sm font-bold transition cursor-pointer ${activeTab === 'journals' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-600'}`}
          >
        ...
        {loading ? (
          <div className="p-12 text-center text-slate-500 font-bold">
            جاري تحميل البيانات الحقيقية من السيرفر...
          </div>
        ) : activeTab === 'coa' ? (
                ...
                {coa.map((acc) => (
                  <tr key={acc.code} className="hover:bg-slate-50 transition">
                    ...
                    <td className="p-4 font-mono font-bold text-slate-900">
                      {(parseFloat(acc.balance) || 0).toLocaleString()} EGP
                    </td>
                    <td className="p-4">
                      <span className="text-xs text-emerald-700 font-bold bg-emerald-50 px-2 py-0.5 rounded flex items-center gap-1 w-max">
                        <CheckCircle2 className="w-3.5 h-3.5" /> نشط وموصول بالداتابيز
                      </span>
                    </td>
                  </tr>
                ))}
                {coa.length === 0 && (
                  <tr>
                    <td colSpan={5} className="p-8 text-center text-slate-400">
                      لا توجد حسابات مسجلة في قاعدة البيانات حاليا.
                    </td>
                  </tr>
                )}
              </tbody>
            ...
            <table className="w-full text-right text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                ...
                {journals.length === 0 && (
                  <tr>
                    <td colSpan={5} className="p-8 text-center text-slate-400">
                      لا توجد قيود مرحلة في قاعدة البيانات حاليا.
                    </td>
                  </tr>
                )}
              </tbody>

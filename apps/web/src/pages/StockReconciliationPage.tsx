...
export const StockReconciliationPage: React.FC = () => {
  const reconciliations = [
    { id: 'RECON-2026-03', warehouse: 'مخزن الخامات الرئيسي', item: 'صاج ستانلس 304 طبي 1.5 مم', bookQty: 548, physicalQty: 548, diff: 0, status: 'matched' },
    { id: 'RECON-2026-04', warehouse: 'مخزن الإكسسوارات الطبية', item: 'مواتير وبساتم هيدروليكية', bookQty: 35, physicalQty: 35, diff: 0, status: 'matched' },
  ];
    ...
    <div className="p-6 space-y-6 bg-slate-50 min-h-screen text-slate-800" dir="rtl">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-3">
          <div className="p-3 bg-teal-600 text-white rounded-xl shadow-md"><Scale className="w-7 h-7" /></div>
          <div>
            <h1 className="text-2xl font-bold text-slate-900">الجرد الفعلي وتسويات عجز وزيادة المخازن</h1>
            <p className="text-sm text-slate-500">مقارنة الأرصدة الدفترية بالرصيد الفعلي وتوليد قيود تسوية العجز والزيادة آليا</p>
          </div>
        </div>
      </div>

      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-200 bg-slate-50 font-bold text-slate-900">سجل تسويات الجرد المخزني الحالية</div>
            ...
            <tbody className="divide-y divide-slate-100">
              {reconciliations.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50 transition">
                  ...
                  <td className="p-4 font-mono text-emerald-700 font-bold">{r.diff === 0 ? '0 (مطابق)' : r.diff}</td>
                  <td className="p-4">

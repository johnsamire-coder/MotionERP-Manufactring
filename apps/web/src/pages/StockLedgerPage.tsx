...
export const StockLedgerPage: React.FC = () => {
  const ledger = [
    ...
    {
      id: 'SLE-1003',
      item: 'سرير عناية مركزة كهربائي 5 حركات',
      date: '2026-10-11',
      voucher: 'STE-2026-0812',
      inQty: 10,
      outQty: 0,
      balanceQty: 10,
      rate: 15033,
    ...
    <div className="p-6 space-y-6 bg-slate-50 min-h-screen text-slate-800" dir="rtl">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">
              السجل المالي التراكمي للمخزون (Stock Ledger)
            </h1>
            <p className="text-sm text-slate-500">
              تتبع القيمة المالية والرصيد التراكمي بالمتوسط المتحرك الآلي لكل حركة
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-200 bg-slate-50 font-bold text-slate-900">
          سجل القيود المالية الحركية للمخزون
        </div>
              ...
              {ledger.map((row) => (
                <tr key={row.id} className="hover:bg-slate-50 transition">
                  ...
                  <td className="p-4 font-mono font-bold text-indigo-700">
                    {row.balanceVal.toLocaleString()} ج.م
                  </td>
                </tr>
              ))}
            </tbody>

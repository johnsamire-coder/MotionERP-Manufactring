...
export const medicalTraceApi = {
  ...
  async getPurchaseBatches(filter?: BatchFilter) {
    try {
      const params = new URLSearchParams(filter as Record<string, string> | undefined).toString();
  ...
  async traceMedicalDevice(serialNumber: string) {
    const res = await fetch(
      `${API_BASE}/v1/sales/serials/trace/${encodeURIComponent(serialNumber)}`,
    );
  ...
  async traceBatchOrigin(batchNumber: string) {
    const res = await fetch(
      `${API_BASE}/v1/inventory/purchase-batches/trace?batchNumber=${encodeURIComponent(batchNumber)}`,
    );

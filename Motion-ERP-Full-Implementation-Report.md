...
- **تعريف سير العمل:**
  ...
  - سير عمل واحد نشط لكل نوع مستند.
- **فحص التعريف وقت الحفظ:**
  - الحالة الابتدائية موجودة، وفيه حالة نهائية.
  - مفيش حالة "مسدودة" (مش نهائية ومفيش منها طريق).
  - الحالة النهائية ملهاش انتقالات.
  - نفس الإجراء ميتكررش من نفس الحالة.
  - الشرط مكتوب صح.
- **لغة الشروط:** JSON آمن من غير أي تنفيذ كود.
  - مقارنات: يساوي، ومش يساوي، وأكبر، وأصغر، وضمن قائمة، وموجود، ويحتوي.
  - تجميع: الكل (all)، أو أي واحد (any)، أو عكس (not).
  - مسارات منقّطة زي `customer.segment`.
- **الشروط الديناميكية:** مع كل إجراء ممكن تبعت **بيانات المستند الحالية**، والشرط بيتقيّم عليها وقتها.
- **الأدوار:** دور المستخدم اللي داخل (من التذكرة، بند 5). لو الإجراء محتاج دور والمستخدم مش داخل أو دوره مختلف، بيرجع **403**.
- **سجل كامل:** كل انتقال بيتسجّل: من، ولـ، والإجراء، والمستخدم، والتعليق، والوقت.
...
- `POST workflow/instances/:id/available-actions`
- `POST workflow/instances/:id/act`
- Migration 0092 (`CREATE SCHEMA workflow` + 5 جداول، ومسجّلة في الفهرس المركزي).

**الاختبارات:** 3 اختبارات وحدة (الشروط، والشروط الغلط، وفحص التعريف).
...
- **اتفاقيات مستوى خدمة (SLA):**
  ...
  - قائمة إجازات.
  - **وقت رد ووقت حل لكل أولوية** (منخفضة، ومتوسطة، وعالية، وعاجلة) بدقائق عمل.
  - عدد أيام الإغلاق التلقائي، وSLA افتراضية.
  - الفحص: وقت الرد مايزيدش عن وقت الحل، والساعات صحيحة، والمنطقة الزمنية موجودة.
- **محرك وقت العمل** (دوال صافية من غير مكتبات): "إضافة X دقيقة عمل" و"دقايق العمل بين وقتين". بيعدّي الليل، والويك إند، والإجازات، وبيبدأ من أول الدوام لو التذكرة جت بدري.
- **التذاكر:**
  - الموضوع، والوصف، والعميل (بيتفحص في CRM)، والأولوية، والـ SLA.
  - **موعد الرد** و**موعد الحل** بيتحسبوا وقت الإنشاء.
  - أول رد من الدعم بيسجّل هل الرد اتعمل في الميعاد ولا لأ، والحالة بتبقى "اتردّ عليها". ورد العميل بيرجّعها "مفتوحة".
  - **الإيقاف المؤقت (on hold) بيوقف العدّاد:** لما التذكرة ترجع، المواعيد اللي لسه ماتحققتش **بتتأخر بدقايق العمل** اللي عدّت وهي موقوفة.
  - الحل والقفل بيسجّلوا هل الحل اتعمل في الميعاد.
- **تقسيم التذكرة:** التعليقات اللي بتختارها بتتنقل لتذكرة جديدة مربوطة بالأصلية، بنفس العميل والأولوية، وبعدّاد SLA جديد.
- **فحص دوري (sweep):**
  - يعلّم المواعيد اللي فاتت "فشل".
  - **يقفل تلقائيًا** التذكرة اللي مستنية رد العميل أو اتحلّت، بعد عدد الأيام المحدد.
  - بيشتغل يدوي، أو كل كذا دقيقة بمتغيّر `SUPPORT_SWEEP_INTERVAL_MINUTES`.
...
- `sweep`
- `sla-report`
- Migration 0093 (`CREATE SCHEMA support`، ومسجّلة في الفهرس المركزي).

**الاختبارات:** 5 اختبارات وحدة لمحرك وقت العمل.
...
- **ربحية مستقلة لكل مشروع:**
  ...
  - أي سطر قيد عليه البُعد ده بيتحسب على المشروع: **الإيرادات − التكاليف = الربح**، والهامش، والتكاليف كنسبة من التقدير.
- **تقرير حالة دوري بالإيميل:**
  - التكرار: يومي أو أسبوعي أو شهري، ومستلمين (الإيميلات بتتفحص).
  - التقرير بالعربي: نسبة الإنجاز، والمهام اللي خلصت، **والمهام المتأخرة**، والربحية.
  - **صندوق صادر** (`email_outbox`): كل إيميل بيتسجّل وحالته (اتبعت / فشل / مفيش إعدادات)، ويتعاد إرساله.
  - **مرسل SMTP صغير** من غير أي مكتبة (smtp:// وsmtps:// وAUTH LOGIN، والنص UTF-8).
  - تشغيل دوري بمتغيّر `PROJECT_REPORT_INTERVAL_MINUTES`، أو يدوي.
...
- `:id/status-report`
- `reports/send-due`
- `outbox`، و`outbox/:id/send`
- Migration 0094 (`CREATE SCHEMA projects`).

**الاختبارات:** 5 اختبارات للطرق الأربعة والتكرار، و2 لمرسل SMTP (على سيرفر وهمي جوه الاختبار). المجموع 333.
...
- **أصل تحت التنفيذ (CWIP):**
  ...
  - **الرسملة** بتنقل كل المتجمّع لحساب الأصل وبتبدأ الإهلاك.
...
- `depreciate-due`
- Migration 0095 (`CREATE SCHEMA assets`).

**الاختبارات:** 5 اختبارات وحدة للطرق الأربعة وإعادة الحساب.
...
- **منع طباعة المسودة والملغي:** الطباعة بترجع 400 "رحّله الأول" أو "مستند ملغي"، إلا لو القالب نفسه مسموح له (`allowDraft` / `allowCancelled`).
- **لغة قوالب آمنة** (من غير تنفيذ كود):
  - حقول ومسارات منقّطة، وتكرار على السطور مع الترقيم والوصول للأب، وشروط بـ "وإلا".
  - فلاتر: `date` و`money` و`number`.
  - **كل القيم بتتأمّن ضد HTML/JS** (اتجرّب `<script>` واتعرض كنص).
  - القالب المكسور بيترفض وقت الحفظ.
- **مصادر مستندات جاهزة:** فاتورة البيع، وفاتورة الشراء، والدفعة (من `finance`)، والسطور بتتكمّل **بكود واسم الصنف** من الكتالوج. وأي مستند تاني ممكن يتبعت كبيانات في الطلب.
- **النتيجة:** صفحة HTML كاملة جاهزة للطباعة (A4، واتجاه عربي، والتذييل ثابت تحت).
...
**الاختبارات:** 5 اختبارات وحدة للغة القوالب.
...
### بند 48 — طبقة إقليمية منفصلة للفوترة الإلكترونية المصرية
...
- **بناء مستند مصلحة الضرائب** من فاتورة البيع المرحّلة: السطور بكود الصنف، والضريبة T1، والإجماليات (مقرّبة لـ 5 أرقام زي الـ SDK).
- **فحص قبل الإرسال:**
  - الرقم الضريبي للشركة وللعميل الشركة.
  - **الرقم القومي للفرد لو الفاتورة ≥ 50,000**.
  - كود كل صنف، وعنوان العميل.
- **التسلسل المعياري (Canonical)** حسب مواصفة المصلحة، و**UUID المستند = SHA-256** للتسلسل (من غير التوقيعات).
- **الحالات:**
  - مسودة (فيها مشاكل)
  - جاهز
  - موقّع
  - اتبعت / مرفوض
- **التوقيع:** بيجي من فلاشة التوقيع الإلكتروني عند الممول (برّه السيستم)، ويتسجّل على المستند. **نسخة 1.0 مبتتبعتش من غير توقيع.**
...
- `customers/:id`
- `items/:id`
- `invoices/:id/prepare`، و`/signature`، و`/submit`
- `documents`
- Migration 0097.

**الاختبارات:** 4 اختبارات وحدة (الإجماليات، والتسلسل المعياري، والـ UUID، والفحوصات).
...
- **حركة الأصل:**
  ...
  - أي حركة مفيهاش تغيير بتترفض، والأصل المدموج أو المستبعد مبيتحركش.
  - **سجل كامل** لكل حركة: من فين لفين، ومن مين لمين، والملاحظة، والوقت.
- **تأمين الأصل:**
  - وثائق تأمين: شركة التأمين، ورقم الوثيقة (مبيتكررش لنفس الشركة)، وقيمة التأمين، والقسط، والتواريخ.
  - **تنبيهات:** الوثائق اللي هتنتهي خلال N يوم، والأصول اللي في الاستخدام **من غير أي وثيقة سارية النهارده**.
- **الأصل المركّب:**
  - بيتبني كأصل تحت التنفيذ، ومكوّناته:
    - **أصول تانية:** قيمتها الدفترية بتتنقل للأصل المركّب بقيد (من الأصول تحت التنفيذ + مجمع إهلاكها، إلى حساب الأصل بتاعها)، والأقساط اللي لسه ماترحّلتش بتتلغي، والأصل بيبقى **"مدموج"** ومربوط بالأصل المركّب.
    - **أصناف مخزون:** بتتصرف من المخزن فعلًا، وتكلفتها بتتنقل من حساب الصرف لحساب الأصول تحت التنفيذ.
  - الرسملة بعدها زي بند 46، ومجموع المكوّنات بيبقى قيمة الأصل وبيتعمله جدول إهلاك.
- Migration 0098.
...
1. **طباعة بالجملة في الخلفية** (`printing.print_job`):
   ...
   - متابعة الحالة (في الطابور / شغالة / خلصت / فشلت) وعدد اللي خلص.
2. **طابعة شبكية** (`printing.printer`): IP وبورت (9100 افتراضيًا، بروتوكول RAW/JetDirect)، والمهمة بتتبعت للطابعة مباشرة. **القيد:** بيتبعت **نص** (العناوين والجداول والأرقام)، لأن مفيش محرك PDF في السيرفر. الطباعة الكاملة بالتنسيق على طابعة شبكية محتاجة محرك PDF، وده قرار للمالك. الطباعة بالتنسيق من المتصفح شغالة من ملف النتيجة.
3. **مهام تلقائية مع انتقالات سير العمل** (بند 42):
   - لكل إجراء في سير العمل ممكن تربط مهام:
     - **إيميل** (بعنوان ونص فيهم `{{...}}` من بيانات المستند)
     - **Webhook** (إرسال بيانات الانتقال لنظام خارجي)
     - **طباعة** على طابعة شبكية
   - المهام **بتشتغل في الخلفية بعد الانتقال**، ومبتوقفهوش أبدًا لو فشلت.
   - كل تشغيل بيتسجّل (نجح أو فشل، والتفاصيل).
...
- `GET workflow/instances/:id/task-runs`
- Migration 0099.

**الاختبارات:** 3 اختبارات للطابعة (تحويل النص، والإرسال على سوكيت وهمي، والطابعة اللي مش بترد)، واختبار للـ placeholders. **المجموع 351 نجحوا.**
...
- **الاختبارات (بعد المرحلة التالتة):** API 366، وweb 5، وshared 7، وe2e 18. كلهم ناجحين.
...
## 6. المرحلة التالتة: قرارات المالك على الأسئلة الأربعة

| السؤال               | القرار      | اللي اتعمل                                                                                                                                           |
| -------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| ترحيل مخازن الفروع   | تمام        | `findAccountingOrgNode` بتطلع في شجرة المؤسسة لأقرب مستوى ليه إعدادات محاسبية، والترحيل والفحص بيستخدموه                                             |
| الضرايب والاستحقاقات | اربطها      | تسوية وسداد ضريبة القيمة المضافة، والإفراج الجمركي، ورسملة الجمارك (سند تكلفة استيراد حقيقي)، والاستحقاقات ومخصص الضمان، وتوزيع الأعباء: كلها بترحّل |
| شاشة التصنيع بالباطن | لازم نعملها | `SubcontractingPage`: إنشاء، وترحيل، واستلام جزئي أو كامل، وربط الفاتورة                                                                             |
| الشاشات الوهمية      | كمل         | **كل** الشاشات الوهمية اتشالت، واتحط مكانها الحقيقية أو شاشات حقيقية جديدة. مفيش علامة "تجريبي" في القائمة خالص                                      |

### الشاشات الوهمية: اتعمل فيها إيه بالظبط

- **13 شاشة كان ليها نسخة حقيقية في الكود:** القائمة بقت بتفتح الحقيقية (المحاسبة، والمخازن، والسجل المالي، والجرد، وفواتير المبيعات، وأوامر الشراء، والسندات، والتكاليف، والميزانية، والجودة، وحركات المخازن، وعمليات الورشة، ومنشئ الـ BOM).
- **شاشات حقيقية كانت موجودة ومش في القائمة:** اتضافت (عروض الأسعار، والتسليم، وأسعار الأصناف، وأذون الاستلام، وصرف الخامات، والمستندات الفنية، وقائمة الدخل، والمصروفات، والمستخدمين والأدوار، والموارد البشرية، والإعدادات).
- **شاشات اتعملت جديدة لأن مكانش ليها حقيقية:**
  - **الضرايب والجمارك ونموذج 41** (`TaxCustomsPage`): الإقرار بيتملي من الدفاتر، والتقديم، والسداد، والخصم تحت حساب الضريبة، والإفراج الجمركي والرسملة.
  - **سجل التدقيق** (`AuditLogPage`): كل تغيير ناجح بيتسجل **تلقائيًا** من `AuditInterceptor` (مين، وإمتى، ومنين، والقيم، والباسوردات متخفية). والتسجيل اليدوي `POST audit/log` اتشال عشان محدش يزوّر.
  - **تتبع اللوطات والسيريال، وغرفة الاستدعاء** (`BatchTracePage`): `GET inventory/batches/:id/trace` بيرجّع اللوط وكل حركاته ورصيده في كل مخزن وسيريالاته. الحجز أو الاستدعاء بيمنع الصرف (اتجرّب: صرف من لوط محجوز رجع 400).
  - **المخطط مقابل الفعلي** (`PlannedVsActualPage`): التكلفة التقديرية مقابل الفعلية لكل عنصر، والخامات المخططة مقابل المطلوبة والمصروفة والمستهلكة، متقيّمة بسعر الصرف الفعلي (`GET production/job-orders/:ref/material-variance`).
- **شاشات اتشالت لأنها تكرار:** كارت التكلفة الفعلي، وربحية أوامر الشغل (موجودين في "تكلفة وربحية أوامر الشغل" الحقيقية)، وانحرافات المواد بـ 4 مستويات (اتدمجت في "المخطط مقابل الفعلي")، ومصفوفة الصلاحيات (موجودة في "المستخدمين والأدوار").
- **الـ endpoints اللي كانت بترجّع أرقام ثابتة** (`cost/job-cost-sheet`، و`cost/standard-vs-actual`، و`cost/material-variance`، و`calculateBomStandardCost` اللي كانت بترجّع صفر دايمًا): اتشالت هي واختباراتها.

### تصحيح محاسبي اتعمل في السكة

- **صرف الخامات لأمر الشغل** كان بيترحّل على حساب "تسويات المخزون". بقى بيترحّل **مدين إنتاج تحت التشغيل / دائن المخزون** (اتجرّب حي)، وبيتربط بطلب الصرف وقيمته الفعلية (migration `0107`، ومعاها تعبئة للطلبات القديمة من نص الملاحظة).

### الـ migrations الجديدة في المرحلة دي

`0105` (حسابات الضرايب والجمارك)، و`0106` (ترحيل توزيع الأعباء)، و`0107` (ربط صرف الخامات بالحركة). وإجمالي migrations بعد قرارات المالك: من `0100` لـ `0107`.

### الفحوصات

- `prettier --check` نضيف، و`lint` صفر أخطاء، و`typecheck` نضيف في الـ 3 باكدجات.
- API 363، وweb 5، وshared 7، وe2e 18، على قاعدة فاضية.
- **كل شاشات القائمة (59) اتفتحت بالمتصفح بعد تسجيل الدخول:** صفر أعطال.

### commits المرحلة التالتة

```
0bd60f4 fix(accounting): branch warehouses post to their company's books
e219106 fix(accounting): tax, customs and accruals post real journal entries
d9d84b4 chore(cost): drop leftovers of the removed overhead tests
abd5b3a feat(cost): overhead allocation posts Dr WIP / Cr applied overhead
603b2e5 feat(web): subcontracting screen (plan item 45)
4ad668d docs: status report — phase three
2ddbf5e feat(web): the menu opens the real screens instead of placeholder ones
343d5e2 style(web): prettier on the fixed invoice screens
ba8d174 feat(web): real tax and customs screen; VAT report ignores settlement entries
5f51d3d feat(audit): every change is recorded automatically; real audit log screen
b3627dc feat(inventory): real batch trace and recall screen
8f8629c feat(production,cost): real planned-vs-actual report; drop hardcoded cost screens
```

### حاجات لسه مفتوحة (مش أعطال، بس تستاهل قرار بعدين)

- **سجلّين موازيين للوطات والسيريال** (`purchase-batch-link` و`sales-serial-link`) جداولهم فاضية، وشغالين جنب اللوطات والسيريال الحقيقية في المخازن. ينفع يتشالوا بعدين.

### قرار المالك: صرف الخامات = تكلفة فعلية على أمر الشغل (تمام)

- **أي صرف خامات لأمر شغل** بيتسجل لوحده كتكلفة خامات **فعلية** في كارت تكلفة أمر الشغل، بقيمة الصرف الحقيقية (المتوسط المرجّح وقت الصرف). والقيد المحاسبي زي ما هو: مدين إنتاج تحت التشغيل / دائن المخزون.
- **مبيتكررش:** القيد مربوط بطلب الصرف (`material-request:<id>`). ولو تسجيل التكلفة فشل لأي سبب، الصرف نفسه بيكمل (المخزون اتحرك واترحّل)، والمشكلة بتتكتب في الـ log.
- **الطلبات اللي اتصرفت قبل كده** اتسجلت تكلفتها بـ migration `0108` (من غير تكرار).
- **اتجرّب حي:** كارت أمر الشغل JO-2026-000001 كان فيه 358.86 خامات فعلية. بعد صرف جديد بـ 143.55 بقى 502.41 لوحده.
- **تنبيه:** متسجّلش الخامات المصروفة يدوي كمان من شاشة التكاليف، وإلا هتتحسب مرتين. الإدخال اليدوي بقى للخامات اللي مش بتتصرف من المخزن بس.
- migrations المرحلة دي بقت لحد `0108`.
